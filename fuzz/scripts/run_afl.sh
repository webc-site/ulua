...
# 产物目录不硬写 ./target：build.target-dir 可被 ~/.cargo/config.toml 全局重定向，
# 届时下面喂给 afl-fuzz 的二进制路径根本不存在。问 cargo 要真实目录（默认配置下
# 它返回本工作区的 target）。
TARGET_DIR="$(cargo metadata --format-version 1 --no-deps 2>/dev/null | sed -n 's/.*"target_directory":"\([^"]*\)".*/\1/p')"
TARGET_DIR="${TARGET_DIR:-${FUZZ_DIR}/target}"
...
if [[ "${IN_DIR}" == corpus/* && "${AFL_NO_CMIN:-0}" != "1" ]]; then
  ...
  if [[ ! -d "${CMIN_DIR}" || -z "$(ls -A "${CMIN_DIR}" 2>/dev/null)" ]]; then
    ...
    AFL_NO_FORKSRV=1 AFL_NO_UI=1 cargo afl cmin -t "${CMIN_T:-5000}" \
      -i "${IN_DIR}" -o "${CMIN_DIR}" -- "${TARGET_DIR}/release/${TARGET}" || true
...
cargo afl fuzz -i "${IN_DIR}" -o "${OUT_DIR}" "${TIME_ARGS[@]}" "${MAXLEN_ARGS[@]}" "$@" "${TARGET_DIR}/release/${TARGET}"
