...
use std::{env::var, process::abort, rc::Rc, thread::Builder};
...
fn split_top_level_statements(src: &str) -> Vec<String> {
  ...
  let mut out = Vec::new();
  for stat in body {
    // Nodes 的元素 Node<T> 恒非空（from_raw 构造即拦截 null 槽），
    // 旧 is_null 死检查随 Node 句柄化删除；get() 借用读 location，免裸解引
    let loc = stat.get().base.location;
