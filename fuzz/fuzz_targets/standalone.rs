...
fn standalone_main(exercise: fn(&[u8])) {
  ...
  // 完全限定 `std::result::Result`：include! 会把本文件并入各 target 模块，
  // 部分宿主（run/splice/api/gcstress/host/structured）已有
  // `use ulua_rt::Result`（单泛型参数别名），裸 `Result` 名字冲突且元数不符。
