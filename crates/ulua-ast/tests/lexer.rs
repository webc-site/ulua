...
// two comment cases (tests/Lexer.test.cpp:406, :418); renamed here to keep Rust
// test names unique and accurate.
#[test]
...
// 回归测试：`\u{...}` 恰好 16 位十六进制数字 + '}' 必须合法。
// C++ `fixupQuotedString` 中 16 次循环耗尽后再检查 `data[i] == '}'`，
// 高位回绕（`unsigned int` 32 位）后仍 ≤ 0x10FFFF 的值照常解码。
#[test]
fn fixup_quoted_bytes_allows_16_digit_unicode_escape() {
...
#[test]
fn fixup_quoted_bytes_rejects_16_digit_unicode_escape_without_brace() {
  // 16 位数字后没有 '}' → 仍然非法
  let mut data = b"\\u{000000000010FFFF".to_vec();
  assert!(!Lexer::fixup_quoted_bytes(&mut data));
}
