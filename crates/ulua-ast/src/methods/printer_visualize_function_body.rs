...
use crate::records::{
  ...
  cst_generic_type_pack::CstGenericTypePack, printer::Printer, writer::Writer,
};
...
impl<'a, W: Writer> Printer<'a, W> {
  pub fn visualize_function_body<F: IntoAstExprFunctionMut>(&mut self, func: F) {
    ...
    if func.generics.size > 0 || func.generic_packs.size > 0 {
      ...
      let mut comma = CommaSeparatorInserter::new(comma_position);
    ...
    let mut comma = CommaSeparatorInserter::new(args_comma_pos);
    ...
    if func.vararg {
      ...
      if self.write_types && !func.vararg_annotation.is_null() {
        ...
        unsafe {
          self.visualize_type_pack_annotation(&mut *func.vararg_annotation, true, true, false);
        }
      }
    }
