...
use crate::{
  ...
  functions::{
    ...
    optional_node::{slot_opt, slot_ref},
  },
  records::{
    ast_array::AstArray, ast_expr::AstExpr, ast_expr_call::AstExprCall, ast_stat::AstStat,
    ast_stat_expr::AstStatExpr, lexeme::lexeme_name_is, node_handle::Node, parser::Parser,
  },
  rtti::ast_node_is,
};
  ...
  pub fn parse_stat(&mut self) -> *mut AstStat {
    match self.lexer.current().r#type {
      ...
      Type::RESERVED_FUNCTION => {
        return self.parse_function_stat(&AstArray::EMPTY).cast::<AstStat>();
      }
      Type::RESERVED_LOCAL => {
    ...
    if ast_node_is::<AstExprCall>(slot_ref(expr)) {
      return self.alloc_stat(AstStatExpr::new(expr_location, Node::from_raw(expr)));
    }

    let current_type = self.lexer.current().r#type;
    if current_type == Type::COMMA || current_type == Type::EQUAL_SIGN {
    ...
    let ident = get_identifier(slot_opt(expr));
