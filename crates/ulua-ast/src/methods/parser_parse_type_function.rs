...
impl Parser {
  pub fn parse_type_function(
    ...
    let node = self.alloc_stat(AstStatTypeFunction::new(
      Location::new(start.begin, unsafe {
        // Safety: body 为 parse_function_body 返回的存活 AstStatBlock（内部 alloc_stat 恒非空，失败中止）；(*body).base.base.location 沿两层 #[repr(C)] 基类前缀（AstStat→AstNode）读取 location，仅取 end 坐标拷贝。
        (*body).base.base.location.end
      }),
      fn_name.name,
      fn_name.location,
      body,
