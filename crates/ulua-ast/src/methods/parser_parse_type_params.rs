        ...
        if should_parse_type_pack(&mut self.lexer) {
          ...
          if !type_pack.is_null() {
              ...
              && is_type_follow(self.lexer.current().r#type)
            {
              ...
              parameters.push(AstTypeOrPack {
                r#type: self
                  .parse_type_suffix(NonNull::new(type_group as *mut AstType), &begin_loc),
                type_pack: null_mut(),
              });
            } else {
              parameters.push(AstTypeOrPack {
            ...
            parameters.push(AstTypeOrPack {
              r#type: self.parse_type_suffix(NonNull::new(type_), &begin_loc),
              type_pack: null_mut(),
            });
          }
