...
pub fn parse_double(data: &str) -> (ConstantNumberParseResult, f64) {
  ...
  let value = match data.parse::<f64>() {
    Ok(v) => v,
    Err(_) => return (ConstantNumberParseResult::Malformed, 0.0),
  };
  ...
  if value >= (1u64 << 53) as f64 && bytes.iter().all(u8::is_ascii_digit) {
    ...
    if repr != data {
      return (ConstantNumberParseResult::Imprecise, value);
    }
  }

  (ConstantNumberParseResult::Ok, value)
}
