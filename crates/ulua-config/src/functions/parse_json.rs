...
/// 解析回调以 `Some(错误描述)` 表示失败、`None` 表示成功；此处转成 `Result`
/// 供 `?` 传播（`?` 对 `Option` 的语义正好相反）。
#[inline]
fn apply(
  action: &mut impl FnMut(&[String], String) -> Option<String>,
  keys: &[String],
  value: String,
) -> Result<(), String> {
...
pub(crate) fn parse_json<Action>(contents: &str, mut action: Action) -> Option<String>
where
  Action: FnMut(&[String], String) -> Option<String>,
{
