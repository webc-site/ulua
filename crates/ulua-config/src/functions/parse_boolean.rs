...
/// `Result<bool, ConfigError>`（`Err` 对应 cpp 的 `Error`）；解析失败时调用方
/// 保持字段原值，与 cpp 语义一致。
pub(crate) fn parse_boolean(value: &str) -> Result<bool, ConfigError> {
  match value {
    SETTING_TRUE => Ok(true),
    SETTING_FALSE => Ok(false),
    _ => Err(ConfigError::Message(bad_setting(value, OPT_BOOL))),
  }
}
