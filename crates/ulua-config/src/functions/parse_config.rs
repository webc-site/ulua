...
pub fn parse_config(
  ...
  parse_json(contents, |keys, value| {
    match keys {
      [key] if key == "languageMode" => parse_mode_string(&value, options.compat)
        .map(|mode| config.mode = mode)
        .err(),
      ...
      [key] if key == "lintErrors" => parse_boolean(&value).map(|v| config.lint_errors = v).err(),
      [key] if key == "typeErrors" => parse_boolean(&value).map(|v| config.type_errors = v).err(),
      [key] if key == "globals" => {
      ...
      [key0, key1] if options.compat && key0 == "language" && key1 == "mode" => {
        parse_mode_string(&value, options.compat)
          .map(|mode| config.mode = mode)
          .err()
      }
