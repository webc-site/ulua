...
pub fn extract_luau_config_from_bytecode(
  ...
  let Some(state) = new_sandbox() else {
    return Some(LUA_MEMERRMSG.to_string_lossy().into_owned());
  };
  let l = state.0;
  ...
  match execute_and_extract(l, &callbacks) {
    Ok(config_table) => parse_luau_config_table(&config_table, config, alias_options),
