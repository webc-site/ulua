...
pub(crate) unsafe fn serialize_table(l: *mut lua_State) -> Result<ConfigTable, String> {
  // 处理完后把表弹出栈
  ...
  while unsafe { lua_next(l, -2) } != 0 {
    ...
    let key = match unsafe { lua_type(l, -2) } {
      ...
      _ => {
        return Err(String::from(
          "configuration table keys must be strings or numbers",
        ));
      }
    };
    ...
    match unsafe { lua_type(l, -1) } {
      ...
      T_TABLE => {
        ...
        let nested = unsafe { serialize_table(l)? };
