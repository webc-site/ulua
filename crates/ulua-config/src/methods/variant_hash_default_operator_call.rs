...
use ulua_common::{
  records::dense_hash_table::DenseHasher, type_aliases::dense_hash_default::DenseHashDefault,
};
...
impl DenseHasher<ConfigTableKey> for VariantHashDefault {
  fn hash(&self, key: &ConfigTableKey) -> usize {
    match key {
      ConfigTableKey::String(value) => DenseHashDefault::<String>::default().hash(value),
      ConfigTableKey::F64(value) => {
