...
use crate::{
  functions::{check_script::run_check, run_code::run_code},
  util::{NOT_ENOUGH_MEMORY, PRINT_NAME, cstr_cow, lua_str_to_string},
};
...
unsafe extern "C-unwind" fn capturing_print(l: *mut lua_State) -> c_int {
  unsafe {
    ...
    for i in 1..=n {
      ...
      line.push_str(&lua_str_to_string(s, len));
      lua_pop(l, 1);
    }
    line.push('\n');
...
pub fn run(source: &str) -> RunResult {
  ...
  if l.is_null() {
    return RunResult {
      output: String::new(),
      error: NOT_ENOUGH_MEMORY.to_string_lossy().into_owned(),
    };
  }
  ...
  let error = unsafe {
    ...
    lua_l_openlibs(l);
    lua_pushcclosurek(l, Some(capturing_print), PRINT_NAME.as_ptr(), 0, None);
    lua_setglobal(l, PRINT_NAME.as_ptr());
