...
//!    `register_flags()` 清单 == `pub use _inner` 别名表，四者逐项相同。新增
//!    flag 漏登记 / 漏别名，这里红。
...
fn rust_table() -> Vec<FlagEntry> {
  rust_flags! {
    ...
    fflag::LUAU_CLOSURE_USAGE_COUNTER => LuauClosureUsageCounter false,
    fflag::LUAU_CODE_GEN_CALL_WRAPPER_EMIT_INST => LuauCodeGenCallWrapperEmitInst false,
    ...
    fflag::LUAU_CODEGEN_EXTRA_TABLE_OPTS => LuauCodegenExtraTableOpts false,
    fflag::LUAU_CODEGEN_FIX_BUFFER_LEN_CHECK => LuauCodegenFixBufferLenCheck false,
    fflag::LUAU_CODEGEN_FORWARD_REMATERIALIZE => LuauCodegenForwardRematerialize false,
    fflag::LUAU_CODEGEN_FREE_BLOCKS => LuauCodegenFreeBlocks false,
    fflag::LUAU_CODEGEN_INTEGER3 => LuauCodegenInteger3 false,
    fflag::LUAU_CODEGEN_INTEGER_COMPARE => LuauCodegenIntegerCompare false,
    fflag::LUAU_CODEGEN_LINEAR_SETUP_ENTRY_STATE3 => LuauCodegenLinearSetupEntryState3 false,
    ...
    fflag::LUAU_CODEGEN_RECORD_ALL_BLOCK_EXIT_INFO => LuauCodegenRecordAllBlockExitInfo false,
    fflag::LUAU_CODEGEN_REG_TAG2 => LuauCodegenRegTag2 false,
    ...
    fflag::LUAU_COMPILE_UDATA_DIRECT => LuauCompileUdataDirect false,
...
fn registered_idents(text: &str, module: &str) -> BTreeSet<String> {
  let marker = "pub fn register_flags()";
  let start = text
    .find(marker)
...
/// `pub use _inner::{ A as B, ..., * }` 的别名对。
fn alias_pairs(text: &str, module: &str) -> BTreeSet<(String, String)> {
  let marker = "pub use _inner::{";
  let start = text
    .find(marker)
    .unwrap_or_else(|| panic!("{module}.rs 缺 {marker}"));
  ...
  let end = rest
    .find("};")
    .unwrap_or_else(|| panic!("{module}.rs 的 {marker} 未闭合"));
  let mut out = BTreeSet::new();
  for entry in split_top_level(&rest[..end]) {
    // 折叠换行与缩进，使 `A as B` 无论是否被 rustfmt 折行都同形。
    let entry: String = entry.split_whitespace().collect::<Vec<_>>().join(" ");
    if entry.is_empty() || entry == "*" {
      continue;
    }
    let parts: Vec<&str> = entry.split(" as ").collect();
    let [rust, cpp] = *parts.as_slice() else {
      panic!("{module}.rs `pub use` 条目形态异常: {entry}");
    };
    out.insert((rust.to_owned(), cpp.to_owned()));
  }
  out
}
...
fn listing_covers_module_tables() {
  ...
  for module in MODULES {
    ...
    let aliases = alias_pairs(&text, module);
    assert_eq!(
      aliases, source,
      "{module}.rs: `pub use _inner` 别名表与宏定义不一致"
    );
  }
}
