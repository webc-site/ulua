...
#[test]
fn op_length_table_matches_cpp_exhaustively() {
  // cpp `getOpLength` 的 2 字长 case 全集，逐一比对定表；其余须为 1。
  ...
  for op in WIDE {
    assert_eq!(get_op_length(op), 2, "{op:?} 应为 2 字长");
  }
  // 遍历全部判别式：不在 WIDE 内者（含 NOP 与 LOP__COUNT 哨兵）须为 1。
  for raw in 0..=LuauOpcode::LOP__COUNT as u8 {
    let op = LuauOpcode::from(raw);
    let expected = if WIDE.contains(&op) { 2 } else { 1 };
    assert_eq!(get_op_length(op), expected, "{op:?}");
  }
}
...
mod experimental_flags {
  ...
  fn list_matches_experimental_flags_h() {
    assert!(is_analysis_flag_experimental("LuauInstantiateInSubtyping"));
    assert!(is_analysis_flag_experimental(
      "LuauFixIndexerSubtypingOrdering"
    ));
    assert!(is_analysis_flag_experimental("LuauSolverV2"));
    assert!(is_analysis_flag_experimental(
      "UseNewLuauTypeSolverDefaultEnabled"
    ));
    assert!(is_analysis_flag_experimental(
      "LuauRefactorStringSemanticSubtyping"
    ));
    assert!(!is_analysis_flag_experimental("LuauConstraintGraph"));
  }
}
