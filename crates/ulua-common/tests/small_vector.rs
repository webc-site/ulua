...
fn upstream_basic_push_index_clear() {
  ...
  assert!(sv.iter().copied().eq(0..10)); // 逐元素对照区间
  sv.clear();
  assert!(sv.empty());
}
...
fn drops_every_element_exactly_once() {
  ...
  let witness = Rc::new(Cell::new(0));
  {
    ...
    sv.extend((0..50).map(|_| Rc::clone(&witness)));
...
/// `N = 0`：无 inline 槽位，首次 `push_back` 即落堆，直接走 grow 路径；
/// 上面的用例都是 N>=2，碰不到这条边界。
#[test]
fn zero_inline_capacity_grows_immediately() {
...
#[test]
fn traits_and_conversions() {
  use std::borrow::{Borrow, BorrowMut};

  // From [T; N]
  let mut sv: SmallVector<i32, 4> = SmallVector::from([10, 20, 30, 40]);
  assert_eq!(sv.len(), 4);
  assert!(!sv.is_empty());
  assert_eq!(sv.as_ref(), &[10, 20, 30, 40]);
  assert_eq!(Borrow::<[i32]>::borrow(&sv), &[10, 20, 30, 40]);

  // AsMut / BorrowMut
  sv.as_mut()[0] = 11;
  assert_eq!(sv[0], 11);
  BorrowMut::<[i32]>::borrow_mut(&mut sv)[1] = 22;
  assert_eq!(sv[1], 22);
  ...
  assert_eq!(sv.as_slice(), &[22, 44, 60, 80]);

  // IntoIterator consuming
  let v: Vec<i32> = sv.into_iter().collect();
  assert_eq!(v, vec![22, 44, 60, 80]);

  // From Vec<T>
  let sv2: SmallVector<i32, 2> = SmallVector::from(vec![1, 2, 3]);
  assert_eq!(sv2.len(), 3);
  assert_eq!(sv2.as_slice(), &[1, 2, 3]);

  // From / Into SmallVec
  let raw: smallvec::SmallVec<[i32; 2]> = sv2.into();
  assert_eq!(raw.as_slice(), &[1, 2, 3]);
  let sv3: SmallVector<i32, 2> = SmallVector::from(raw);
  assert_eq!(sv3.as_slice(), &[1, 2, 3]);
}
