...
use crate::enums::luau_bytecode_type::LuauBytecodeType;
...
impl LuauBuiltinFunction {
  ...
  /// bfid → `LuauBuiltinFunction`：校验后再转换，消除 `as` 静默截断。
  ///
  /// 合法域：`LOP_FASTCALL` 的 A 字节（0..=255）或 `get_builtin_function_id` 结果。
  /// LBF 为 `#[repr(u8)]` 连续判别值（0..=132），仅 `LbfNone = 0` 显式标注；
  /// 越界 id（损坏字节码/非 builtin）返回 `None`，调用方归入 LBF_NONE 语义，
  /// 与 C++ `switch (LuauBuiltinFunction(bfid))` 落 default → no builtin 对齐。
  #[inline]
  pub fn from_id(bfid: i32) -> Option<Self> {
    u8::try_from(bfid).ok().and_then(Self::from_repr)
  }

  /// 内建调用的确定返回字节码类型；若不确定（如 table/any）或无返回值则返回 None。
  pub const fn result_bytecode_type(self) -> Option<LuauBytecodeType> {
    use LuauBytecodeType as T;
    match self {
      Self::LbfMathAbs
      | Self::LbfMathAcos
      | Self::LbfMathAsin
      ...
      | Self::LbfMathTanh
      | Self::LbfMathTan
      | Self::LbfBit32Arshift
      | Self::LbfBit32Band
      | Self::LbfBit32Bnot
      | Self::LbfBit32Bor
      | Self::LbfBit32Bxor
      | Self::LbfBit32Btest
      | Self::LbfBit32Extract
      | Self::LbfBit32Lrotate
      | Self::LbfBit32Lshift
      | Self::LbfBit32Replace
      | Self::LbfBit32Rrotate
      | Self::LbfBit32Rshift
      | Self::LbfStringByte
      | Self::LbfStringLen
      | Self::LbfMathClamp
      | Self::LbfMathSign
      | Self::LbfMathRound
      | Self::LbfBit32Countlz
      | Self::LbfBit32Countrz
      | Self::LbfRawlen
      | Self::LbfBit32Extractk
      | Self::LbfTonumber
      | Self::LbfBit32Byteswap
      | Self::LbfBufferReadi8
      | Self::LbfBufferReadu8
      | Self::LbfBufferReadi16
      | Self::LbfBufferReadu16
      | Self::LbfBufferReadi32
      | Self::LbfBufferReadu32
      | Self::LbfBufferReadf32
      | Self::LbfBufferReadf64
      | Self::LbfVectorMagnitude
      | Self::LbfVectorDot
      | Self::LbfMathLerp
      | Self::LbfIntegerTonumber => Some(T::LBC_TYPE_NUMBER),

      Self::LbfType
      | Self::LbfStringChar
      | Self::LbfTypeof
      | Self::LbfStringSub
      | Self::LbfTostring => Some(T::LBC_TYPE_STRING),

      Self::LbfMathIsnan
      | Self::LbfMathIsinf
      | Self::LbfMathIsfinite
      | Self::LbfRawequal
      | Self::LbfIntegerLt
      | Self::LbfIntegerLe
      | Self::LbfIntegerGt
      ...
      | Self::LbfIntegerUgt
      | Self::LbfIntegerUge
      | Self::LbfIntegerBtest => Some(T::LBC_TYPE_BOOLEAN),

      Self::LbfVector
      | Self::LbfVectorNormalize
      | Self::LbfVectorCross
      | Self::LbfVectorFloor
      | Self::LbfVectorCeil
      | Self::LbfVectorAbs
      ...
      | Self::LbfVectorMin
      | Self::LbfVectorMax
      | Self::LbfVectorLerp => Some(T::LBC_TYPE_VECTOR),
      ...
      | Self::LbfIntegerBnot
      | Self::LbfIntegerBxor
      | Self::LbfIntegerLshift
      | Self::LbfIntegerRshift
      | Self::LbfIntegerArshift
      | Self::LbfIntegerLrotate
      | Self::LbfIntegerRrotate
      | Self::LbfIntegerExtract
      | Self::LbfIntegerCountlz
      | Self::LbfIntegerCountrz
      | Self::LbfIntegerBswap
      | Self::LbfIntegerClamp
      | Self::LbfIntegerNeg
      | Self::LbfIntegerCreate
      | Self::LbfBufferReadinteger => Some(T::LBC_TYPE_INTEGER),

      _ => None,
    }
  }

  /// 是否为 integer fastcall 族函数（受 `LuauIntegerFastcalls` 旗标控制）
  #[inline]
  pub const fn is_integer_fastcall(self) -> bool {
    let id = self as u8;
    id >= Self::LbfIntegerCreate as u8 && id <= Self::LbfBufferReadinteger as u8
  }
}
