...
/// C++ `grow` 的容量上限断言（`newSize < 0x40000000`）。
const MAX_GROW_CAPACITY: u32 = 0x4000_0000;
...
impl<T, const N: usize> SmallVector<T, N> {
  ...
  pub fn front(&self) -> &T {
    self.as_slice().first().expect("front on empty SmallVector")
  }

  pub fn back(&self) -> &T {
    self.as_slice().last().expect("back on empty SmallVector")
  }
  ...
  fn grow(&mut self, new_size: u32) {
    ...
    assert!(new_size < MAX_GROW_CAPACITY);
