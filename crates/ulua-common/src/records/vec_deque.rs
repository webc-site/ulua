...
impl<T: Clone> Clone for VecDeque<T> {
  ...
  fn clone_from(&mut self, other: &Self) {
    ...
    // SAFETY：capacity ≥ queue_size（上面已按需重分配），两段元素写入
    // [0, queue_size) 区间的未初始化内存，逐元素克隆。
    unsafe {
      for (i, val) in head_part.iter().enumerate() {
        ptr::write(buf.as_ptr().add(i), val.clone());
      }
      for (i, val) in tail_part.iter().enumerate() {
        ptr::write(buf.as_ptr().add(head_part.len() + i), val.clone());
      }
    }
  }
}
