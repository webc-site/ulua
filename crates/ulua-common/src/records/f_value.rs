...
/// 在任一链表遍历入口调用；返回前保证所有 'static flag 已注册。
fn with_registered_flags<R>(f: impl FnOnce() -> R) -> R {
  ...
  pub fn set_flag_by_name(name: &str, value: T) -> bool {
    with_registered_flags(|| unsafe {
      ...
      while !cur.is_null() {
        ...
        // `<name><version>` 形式：后缀须是"无前导零的十进制数"（与
        // `version.to_string()` 逐字节等价），数值相等才命中。
        let version = flag.version();
        if version != 0
          && let Some(suffix) = name.strip_prefix(flag.name)
          && !suffix.is_empty()
