...
pub fn compute_cfg_dominance_tree_children(function: &mut IrFunction) {
  ...
  for &dom_parent in function.cfg.idoms.iter().take(block_count) {
    if dom_parent != !0u32 {
  ...
  for (block_idx, &dom_parent) in function.cfg.idoms.iter().enumerate().take(block_count) {
    if dom_parent != !0u32 {
  ...
  // offsets[i] = offsets[i-1]（i 从 1 起）：整体右移一格，原首槽归零
  if block_count > 1 {
    function
      .cfg
      .dom_children_offsets
      .copy_within(0..block_count - 1, 1);
