...
pub fn emit_return_assembly_builder_a_64_module_helpers(
  ...
  build.ldr(X0, mem(R_STATE, offset_of!(lua_State, ci) as i32));
  build.ldr(W3, mem(X0, offset_of!(CallInfo, nresults) as i32));
  ...
  build.str(W4, mem(X1, offset_of!(TValue, tt) as i32));
  ...
  build.ldr(X1, mem(X2, offset_of!(CallInfo, top) as i32));
  build.set_label_label(&mut skip_fixed_ret_top);

  build.str(X2, mem(R_STATE, offset_of!(lua_State, ci) as i32));
  build.ldr(R_BASE, mem(X2, offset_of!(CallInfo, base) as i32));
  build.str(R_BASE, mem(R_STATE, offset_of!(lua_State, base) as i32));

  build.str(X1, mem(R_STATE, offset_of!(lua_State, top) as i32));

  if LuauClosureUsageCounter.get() {
    build.ldr(X4, mem(R_CLOSURE, offset_of!(Closure, usage) as i32));
    build.sub_register_a_64_register_a_64_u16(X4, X4, 1);
    build.str(X4, mem(R_CLOSURE, offset_of!(Closure, usage) as i32));
  }

  build.ldr(W4, mem(X0, offset_of!(CallInfo, flags) as i32));
  build.tbnz(
    W4,
  ...
  build.ldr(W4, mem(X2, offset_of!(CallInfo, flags) as i32));
  build.tbz(
    W4,
  ...
  build.ldr(R_CLOSURE, mem(X2, offset_of!(CallInfo, func) as i32));
  build.ldr(
    R_CLOSURE,
    mem(
      R_CLOSURE,
