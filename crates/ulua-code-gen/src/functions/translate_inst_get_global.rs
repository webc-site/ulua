...
pub unsafe fn translate_inst_get_global(build: &mut IrBuilder, pc: *const Instruction, pcpos: i32) {
  let insn = unsafe { *pc };
  ...
  let reg_ra = build.vm_reg(ra);
  let aux_op = build.vm_const(aux);
  build.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::FallbackGetglobal, pcpos_op, reg_ra, aux_op);
