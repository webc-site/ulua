use crate::{
  ...
  functions::{
    ...
    substitute::{substitute, substitute_at},
  },
  records::{ir_block::IrBlock, ir_function::IrFunction, ir_inst::IrInst, ir_op::IrOp},
  type_aliases::ir_ops::IrOps,
};

/// 脏高位时替换为 TRUNCATE_UINT，否则 substitute；
/// 返回是否发生了 TRUNCATE_UINT 替换（调用方据此决定是否记录 CSE）。
...
/// `substitute_with_truncated_uint` 的索引化变体：只借 `&mut function`，
