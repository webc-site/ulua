  ...
  match inst.cmd {
    ...
    IrCmd::GetSlotNodeAddr => {
      for i in 0..state.get_slot_node_cache.len() {
        ...
        if op_a(&mut prev) == op_a(inst) && op_c_ref(&prev) == op_c_ref(inst) {
    ...
    IrCmd::GetArrAddr => {
      for prev_idx in state.get_arr_addr_cache.iter().copied() {
        ...
        if op_a(&mut prev) == op_a(inst) && op_b_ref(&prev) == op_b_ref(inst) {
          substitute_at(function, index,
            IrOp::ir_op_ir_op_kind_u32(IrOpKind::Inst, prev_idx),
          );
          return;
