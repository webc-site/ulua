...
use ulua_common::{
  ...
  fflag::{DebugLuauAbortingChecks, LuauCodegenExtraTableOpts},
...
use crate::{
  ...
  macros::{
    ...
    op_b_ref::op_b_ref, op_c::op_c, op_c_ref::op_c_ref, op_d_ref::op_d_ref, op_e_ref::op_e_ref,
    op_f_ref::op_f_ref, op_g_ref::op_g_ref, opt_op_b_ref::opt_op_b_ref, opt_op_c_ref::opt_op_c_ref,
    opt_op_d_ref::opt_op_d_ref,
  },
  records::{
...
fn replace_with_jump(function: &mut IrFunction, block: &mut IrBlock, index: u32, target: IrOp) {
...
fn substitute_with(
  ...
  replace_ir_function_ir_block_u32_ir_inst(function, block, index, const_prop_make_inst(cmd, ops));

  if LUAU_CODEGEN_SUBSTITUTE_REPLACEMENTS.get() {
...
fn forward_buffer_int_store(
  ...
  let src = function.as_inst_op(op_c_ref(inst));
  // SAFETY：src 非空且指向 function.instructions 存活指令，仅只读借用
  if let Some(src_inst) = (!src.is_null()).then(|| unsafe { &*src }) {
    let int_src_b = function.as_int_op(opt_op_b_ref(src_inst));

    if is_inverse(src_inst, int_src_b) {
      let replacement = op_a_ref(src_inst);
      replace_ir_function_ir_op_ir_op(function, &mut inst.ops[2], replacement);
    }
  }

  state.forward_buffer_store_to_load(inst, load_cmd, size);
}
...
fn store_numeric(
  ...
  if target.kind() == IrOpKind::VmReg {
    ...
    if value.kind() == IrOpKind::Constant {
      ...
      state.forward_vm_reg_store_to_load(inst, load_cmd);
    }
  }
}
...
fn reuse_node_load(
  ...
  if prev.cmd == IrCmd::LoadTvalue {
    ...
    let arg = function.as_inst_op(op_b_ref(&prev));
    if !arg.is_null() && unsafe { (*arg).use_count } != 0 {
      substitute_at(function, index, op_b(prev));
    }
  }
}
...
pub fn const_prop_in_inst(
  ...
  match inst.cmd {
    ...
    IrCmd::LoadDouble => {
      load_numeric(state, build, function, index, inst, |f, v| {
        f.as_double_op(v).is_some()
      });
    }
    IrCmd::LoadInt => {
      load_numeric(state, build, function, index, inst, |f, v| {
        f.as_int_op(v).is_some()
      });
    }
    IrCmd::LoadInt64 => {
      load_numeric(state, build, function, index, inst, |f, v| {
        f.as_int_64_op(v).is_some()
      });
    }
    IrCmd::LoadTvalue => {
      let source = op_a(inst);
      if source.kind() == IrOpKind::VmReg {
        ...
        if source_inst.cmd == IrCmd::GetSlotNodeAddr {
          ...
          *state.hash_value_cache.get_or_insert(source.index()) = index;
          ...
          let offset_op =
            state.get_combined_array_load_offset_op(&mut source_addr, opt_op_b_ref(inst));
    ...
    IrCmd::LoadFloat => {
      let source = op_a(inst);
      if source.kind() == IrOpKind::VmReg {
        let offset = function.int_op(op_b_ref(inst));
    ...
    IrCmd::StoreDouble => store_numeric(state, function, index, inst, IrCmd::LoadDouble),
    IrCmd::StoreInt => {
      let stored = op_b_ref(inst);
      let stored_inst = function.as_inst_op(stored);
      if !stored_inst.is_null() {
        // SAFETY：指向 function.instructions 存活指令；op_a_ref 只读，
        // 借用在 replace 前结束（IrOp 为 Copy）
        let src_inst = unsafe { &*stored_inst };
      ...
      store_numeric(state, function, index, inst, IrCmd::LoadInt);
    }
    IrCmd::StoreInt64 => store_numeric(state, function, index, inst, IrCmd::LoadInt64),
    IrCmd::StoreVector => {
      let target = op_a(inst);
      if target.kind() == IrOpKind::VmReg {
        state.invalidate_value(target);
    ...
    IrCmd::StoreTvalue => {
      ...
      if target.kind() == IrOpKind::VmReg || target.kind() == IrOpKind::Inst {
        ...
        if target.kind() == IrOpKind::Inst {
          ...
          state.invalidate_table_store_location(target_inst, opt_op_c_ref(inst), tag);
        }
    ...
    IrCmd::StoreSplitTvalue => {
      ...
      if target.kind() == IrOpKind::VmReg {
        ...
        let tag = function.tag_op(op_b_ref(inst));
        state.save_tag(target, tag);

        let value = op_c_ref(inst);
        if value.kind() == IrOpKind::Constant {
          state.save_value(target, value);
        }
      } else if target.kind() == IrOpKind::Inst {
        let tag = function.tag_op(op_b_ref(inst));
        let offset = opt_op_d_ref(inst);
        let target_inst = function.instructions[target.index() as usize].clone();
    ...
    IrCmd::AddNum | IrCmd::SubNum => {
      let rhs = resolve_value(state, op_b_ref(inst));
    ...
    IrCmd::MulNum => {
      ...
      if let Some(k) = function.as_double_op(rhs) {
        ...
        if k == 1.0 {
          ...
          substitute_with(state, function, block, index, inst, IrCmd::UnmNum, &[lhs]);
        } else {
          state.substitute_or_record(inst, index);
        }
      } else {
    ...
    IrCmd::MulFloat => {
      ...
      if let Some(k) = function.as_double_op(rhs) {
        ...
        if kf == 1.0 {
          ...
          substitute_with(state, function, block, index, inst, IrCmd::UnmFloat, &[lhs]);
        } else {
          state.substitute_or_record(inst, index);
        }
      } else {
    ...
    IrCmd::UintToNum | IrCmd::UintToFloat => {
      ...
      if !src.is_null() {
        // SAFETY：src 非空且指向 function.instructions 存活指令，仅只读；
        // 借用（IrOp Copy）在 replace 前结束
        let src_inst = unsafe { &*src };
        if src_inst.cmd == IrCmd::TruncateUint {
    ...
    IrCmd::NumToInt => {
      ...
      if !src.is_null() {
        ...
        if src_clone.cmd == IrCmd::IntToNum {
          ...
          substitute_at(function, index, op_a(&mut src_clone));
          return;
        }
        ...
        if src_clone.cmd == IrCmd::UintToNum {
          ...
          if src_source.kind() != IrOpKind::Constant {
            let truncated = substitute_with_truncated_uint_at(function, block, index, src_source);

            if truncated && LUAU_CODEGEN_SUBSTITUTE_REPLACEMENTS.get() {
              state.substitute_or_record(inst, index);
            }
    ...
    IrCmd::NumToUint => {
      ...
      if !src.is_null() {
        ...
        if src_clone.cmd == IrCmd::UintToNum {
          let mut src_clone = src_clone;
          let truncated =
            substitute_with_truncated_uint_at(function, block, index, op_a(&mut src_clone));
    ...
    IrCmd::NumToFloat => {
      ...
      if !src.is_null() {
        // SAFETY：src 非空且指向 function.instructions 存活指令，仅只读
        let src_inst = unsafe { &*src };
        if src_inst.cmd == IrCmd::FloatToNum {
          substitute_at(function, index, op_a_ref(src_inst));
        } else if src_inst.cmd == IrCmd::UintToNum {
          let src_source = op_a_ref(src_inst);
          replace_ir_function_ir_block_u32_ir_inst(
            function,
            block,
    ...
    IrCmd::JumpIfTruthy => {
      ...
      if tag != K_UNKNOWN_TAG {
        if tag == LuaType::Nil as u8 {
          replace_with_jump(function, block, index, op_c_ref(inst));
        } else if tag != LuaType::Boolean as u8 {
          replace_with_jump(function, block, index, op_b_ref(inst));
        }
      }
    }
    IrCmd::JumpIfFalsy => {
    ...
    IrCmd::CmpSplitTvalue => {
      ...
      if tag_a == LuaType::String as u8
        ...
        && let Some((source, tag)) =
          type_name_tag_comparison(function, op_c_ref(inst), op_d_ref(inst))
      {
        let tag_op = build.const_tag(tag);
        let replacement = const_prop_make_inst(IrCmd::CmpTag, &[source, tag_op, op_e_ref(inst)]);
    ...
    IrCmd::JumpEqPointer => {
      if let Some((source, tag)) = type_name_tag_comparison(function, op_a(inst), op_b_ref(inst)) {
        let tag_op = build.const_tag(tag);
        let replacement = const_prop_make_inst(
          IrCmd::JumpEqTag,
          &[source, tag_op, op_c_ref(inst), op_d_ref(inst)],
        );
    ...
    IrCmd::JumpEqTag => {
      ...
      if tag_a != K_UNKNOWN_TAG && tag_b != K_UNKNOWN_TAG {
        ...
        replace_with_jump(function, block, index, target);
      } else if a == b {
        replace_with_jump(function, block, index, op_c_ref(inst));
      }
    }
    ...
    IrCmd::JumpCmpNum => {
      ...
      if let (Some(value_a), Some(value_b)) = (value_a, value_b) {
        let target = if compare_f64_f64_ir_condition(value_a, value_b, condition_op(op_c_ref(inst)))
        {
          op_d_ref(inst)
        } else {
          op_e_ref(inst)
    ...
    IrCmd::CheckTag => {
      let target = op_a(inst);
      let expected = function.tag_op(op_b_ref(inst));
    ...
    IrCmd::GetSlotNodeAddr => {
      ...
      let matched = state.get_slot_node_cache.iter().position(|entry| {
        ...
        op_a_ref(prev) == op_a(inst) && op_c_ref(prev) == op_c_ref(inst)
      });
    ...
    IrCmd::GetArrAddr => {
      for prev_idx in state.get_arr_addr_cache.iter().copied() {
        ...
        if op_a_ref(prev) == op_a(inst) && op_b_ref(prev) == op_b_ref(inst) {
          substitute_at(
            function,
            index,
            IrOp::ir_op_ir_op_kind_u32(IrOpKind::Inst, prev_idx),
        ...
        if op_a_ref(prev) == op_a(inst) && op_b_ref(prev) == op_b_ref(inst) {
          ...
          if el.known_to_not_be_nil {
            ...
            replace_ir_function_ir_block_u32_ir_inst(
              ...
              block,
              index,
              const_prop_make_inst(IrCmd::CheckNodeValue, &[op_a(inst), op_c_ref(inst)]),
            );
          }
    ...
    IrCmd::CheckBufferLen => {
      let buffer_offset_op = op_b_ref(inst);
      ...
      let min_offset = function.int_op(op_c_ref(inst));
      let max_offset = function.int_op(op_d_ref(inst));
      ...
      {
        replace_with_jump(function, block, index, op_f_ref(inst));
        return;
      }

      for prev_idx in state.check_buffer_len_cache.clone() {
        // SAFETY：prev_ptr 由 &mut function.instructions[prev_idx] 派生，界内且存活；
        // 裸指针绕过借用冲突（try_merge_* 需 &mut prev 且 function 另有借用），
        // 缓存快照迭代期间 instructions 不重分配
        ...
        if op_a(prev) == op_a(inst)
          && op_b_ref(prev) == op_b_ref(inst)
          && op_c_ref(prev) == op_c_ref(inst)
          && op_d_ref(prev) == op_d_ref(inst)
        {
          if DebugLuauAbortingChecks.get() {
            let replacement = build.undef();
        ...
        if op_a(prev) == op_a(inst)
          && op_b_ref(inst).kind() == IrOpKind::Constant
          && op_b_ref(prev).kind() == IrOpKind::Constant
        {
          let curr_bound = function.int_op(op_b_ref(inst));
          let prev_bound = function.int_op(op_b_ref(prev));
    ...
    | IrCmd::MaxVec => {
      ...
      // SAFETY：a_ptr 非空且指向 function.instructions 存活指令，仅只读；
      // 借用（IrOp Copy）在 replace 前结束
      if let Some(a_inst) = (!a_ptr.is_null()).then(|| unsafe { &*a_ptr })
        && a_inst.cmd == IrCmd::TagVector
      {
        let replacement = op_a_ref(a_inst);
      ...
      // SAFETY：同上，只读借用
      if let Some(b_inst) = (!b_ptr.is_null()).then(|| unsafe { &*b_ptr })
        && b_inst.cmd == IrCmd::TagVector
      {
        let replacement = op_a_ref(b_inst);
        replace_ir_function_ir_op_ir_op(function, &mut inst.ops[1], replacement);
      }
    ...
    IrCmd::SETLIST => {
      if LuauCodegenExtraTableOpts.get() {
        if let Some(load_idx) =
          state.get_previous_versioned_load_index(IrCmd::LoadPointer, op_b_ref(inst))
        {
    ...
    IrCmd::FallbackGetvarargs => {
      let first_reg = vm_reg_op(op_b_ref(inst));
      let count = function.int_op(op_c_ref(inst));
      state.invalidate_register_range(first_reg, count);
    }
    IrCmd::FASTCALL => {
      // bfid 校验后再转换：`as u8` 截断 + 非法判别值 transmute 是 UB；
      // 非法 id 归入 LBF_NONE（handle_builtin_effects 该臂为空操作），与 C++ switch default 一致
      let bfid = LuauBuiltinFunction::from_id(function.uint_op(op_a(inst)) as i32)
        .unwrap_or(LuauBuiltinFunction::LBF_NONE);
      let first_return_reg = vm_reg_op(op_b_ref(inst));
      let nresults = function.int_op(op_d_ref(inst));
    ...
    IrCmd::InvokeFastcall => {
      ...
      let bfid = LuauBuiltinFunction::from_id(function.uint_op(op_a(inst)) as i32)
        .unwrap_or(LuauBuiltinFunction::LBF_NONE);
      let first_return_reg = vm_reg_op(op_b_ref(inst)) as u32;
      let nresults = function.int_op(op_g_ref(inst));
    ...
    IrCmd::FallbackGetglobal => {
      state.invalidate_ir_op(op_b_ref(inst));
      state.invalidate_user_call();
    }
    ...
    IrCmd::CheckUserdataTag => {
      for prev_idx in state.useradata_tag_cache.iter().copied() {
        ...
        match prev.cmd {
          IrCmd::CheckUserdataTag => {
            if op_a_ref(prev) != op_a_ref(inst) || op_b_ref(prev) != op_b_ref(inst) {
              continue;
            }
          }
          IrCmd::NewUserdata
            // 对 NEW_USERDATA 结果做同 tag 检查是冗余的
            if op_a_ref(inst).kind() == IrOpKind::Inst
              && prev_idx == op_a_ref(inst).index()
              && op_b_ref(prev) == op_b_ref(inst) => {}
          _ => continue,
        }
