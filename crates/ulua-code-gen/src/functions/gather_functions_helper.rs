...
pub unsafe fn gather_functions_helper(
  ...
  // cpp CodeGenLower.h:54-55：LPF_NATIVE_FUNCTION = 1<<2，LPF_NATIVE_COLD = 1<<1，
  // 必须与 LuauProtoFlag 枚举一致（此前误用 1<<0，与 LPF_NATIVE_MODULE 冲突）
  let lpf_native_function = LuauProtoFlag::LPF_NATIVE_FUNCTION as u32;
  let lpf_native_cold = LuauProtoFlag::LPF_NATIVE_COLD as u32;
  ...
  // 子原型数组为 sizep 个连续指针（VM 不变式），按切片迭代避免逐索引取址；
  // sizep==0 时 p 允许为 null（cpp 空循环语义），零长度切片构造是 UB，短路
  if proto_ref.sizep != 0 {
    for &child_proto in unsafe { slice::from_raw_parts(proto_ref.p, proto_ref.sizep as usize) } {
