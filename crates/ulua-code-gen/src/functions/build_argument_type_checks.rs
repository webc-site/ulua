...
use crate::{
  ...
  functions::{check_tag_exit::check_tag_exit, has_typed_parameters::has_typed_parameters},
...
pub fn build_argument_type_checks(build: &mut IrBuilder, entry: IrOp) {
  ...
  for (i, et) in argument_types.iter().copied().enumerate() {
    let tag = et & !(LuauBytecodeType::LBC_TYPE_OPTIONAL_BIT.0 as u8);
    let optional = (et & (LuauBytecodeType::LBC_TYPE_OPTIONAL_BIT.0 as u8)) != 0;
    ...
    let luau_tag = match tag {
      x if x == LuauBytecodeType::LBC_TYPE_NIL.0 as u8 => LuaType::Nil as u8,
      x if x == LuauBytecodeType::LBC_TYPE_BOOLEAN.0 as u8 => LuaType::Boolean as u8,
      x if x == LuauBytecodeType::LBC_TYPE_NUMBER.0 as u8 => LuaType::Number as u8,
      x if x == LuauBytecodeType::LBC_TYPE_INTEGER.0 as u8 => LuaType::Integer as u8,
      x if x == LuauBytecodeType::LBC_TYPE_STRING.0 as u8 => LuaType::String as u8,
      x if x == LuauBytecodeType::LBC_TYPE_TABLE.0 as u8 => LuaType::Table as u8,
      x if x == LuauBytecodeType::LBC_TYPE_FUNCTION.0 as u8 => LuaType::Function as u8,
      x if x == LuauBytecodeType::LBC_TYPE_THREAD.0 as u8 => LuaType::Thread as u8,
      x if x == LuauBytecodeType::LBC_TYPE_USERDATA.0 as u8 => LuaType::UserData as u8,
      x if x == LuauBytecodeType::LBC_TYPE_VECTOR.0 as u8 => LuaType::Vector as u8,
      x if x == LuauBytecodeType::LBC_TYPE_BUFFER.0 as u8 => LuaType::Buffer as u8,
      x if x >= LuauBytecodeType::LBC_TYPE_TAGGED_USERDATA_BASE.0 as u8
        && x < LuauBytecodeType::LBC_TYPE_TAGGED_USERDATA_END.0 as u8 =>
      {
    ...
    check_tag_exit(build, load, luau_tag, K_VM_EXIT_ENTRY_GUARD_PC as i32);
  ...
  if argument_types
    .last()
    .is_some_and(|ty| (ty & (LuauBytecodeType::LBC_TYPE_OPTIONAL_BIT.0 as u8)) == 0)
  {
