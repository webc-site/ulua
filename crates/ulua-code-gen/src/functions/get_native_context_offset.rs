...
use crate::{
  constants::LBF_IR_MATH_LOG2, macros::codegen_assert::CODEGEN_ASSERT,
...
// CodeGen 内部虚拟 builtin id 统一取自 crate::constants（见 IrData.h:504）
...
pub fn get_native_context_offset(bfid: i32) -> u32 {
  match bfid {
    ...
    _ => {
      // cpp IrUtils.cpp 侧原文即 CODEGEN_ASSERT(!"Unsupported bfid")，不带值
      CODEGEN_ASSERT!(false, "Unsupported bfid");
      0
    }
  }
}
