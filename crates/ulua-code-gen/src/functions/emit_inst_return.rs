...
pub fn emit_inst_return(
  ...
  if function_variadic {
    ...
    build.mov(
      OperandX64::reg(res),
      mem(SizeX64::Qword, res, offset_of!(CallInfo, func) as i32),
    );
  ...
  if actual_results == 0 {
    ...
    build.vmovups(OperandX64::reg(RegisterX64::XMM0), luau_reg(ra));
    build.vmovups(
    ...
    // 逐寄存器取值写入返回槽：cpp `xmmword[res + r * sizeof(TValue)]`
    (0..actual_results).for_each(|r| {
      build.vmovups(OperandX64::reg(RegisterX64::XMM0), luau_reg(ra + r));
      build.vmovups(
    ...
    build.vmovups(
      OperandX64::reg(RegisterX64::XMM0),
      mem(SizeX64::Xmmword, vali, 0),
    );
    build.vmovups(
      mem(SizeX64::Xmmword, res, 0),
      OperandX64::reg(RegisterX64::XMM0),
    );
    build.add(
      OperandX64::reg(vali),
