...
pub unsafe fn compile_internal(
  ...
  unsafe {
    ...
    for pair in native_protos.windows(2) {
      let header = get_native_proto_exec_data_header_mut(pair[0].as_ptr());
      let begin = (*header).entry_offset_or_address as usize as u32;
      let end = (*get_native_proto_exec_data_header_mut(pair[1].as_ptr())).entry_offset_or_address
        as usize as u32;
    ...
    if let Some(last) = native_protos.last() {
      let header = get_native_proto_exec_data_header_mut(last.as_ptr());
      let begin = (*header).entry_offset_or_address as usize as u32;
