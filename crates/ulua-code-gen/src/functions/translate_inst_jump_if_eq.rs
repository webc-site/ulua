...
pub unsafe fn translate_inst_jump_if_eq(
  ...
  // 快路径：integer（两侧期望为 integer 或未知类型）
    ...
    && is_expected_or_unknown_bytecode_type(bc_types.b, LuauBytecodeType::LBC_TYPE_INTEGER)
  {
    ...
    let reg_ra = build.vm_reg(ra);
    let ta = build.inst_ir_cmd_ir_op(IrCmd::LoadTag, reg_ra);
    let integer_tag = build.const_tag(LuaType::Integer as u8);
    build.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::CheckTag, ta, integer_tag, fallback);

    let reg_rb = build.vm_reg(rb);
    let tb = build.inst_ir_cmd_ir_op(IrCmd::LoadTag, reg_rb);
    let integer_tag = build.const_tag(LuaType::Integer as u8);
    build.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::CheckTag, tb, integer_tag, fallback);
