...
use ulua_vm::{
  macros::{
    checkliveness::checkliveness, lu_tag_iterator::LU_TAG_ITERATOR, setobj::setobj,
    setpvalue::setpvalue, ttisnil::ttisnil,
  },
...
/// 取 `LuaNode` 的 value 字段地址（cpp nodeGVal）。
/// # Safety
/// `n` 必须有效且指向存活的 LuaNode 节点。
unsafe fn node_gval(n: *const LuaNode) -> *const TValue {
  unsafe { addr_of!((*(n as *const LocalLuaNode)).val) }
}

/// 把 node 的 key 复制到 `obj` 并做 liveness 检查（cpp getNodeKey）。
/// # Safety
/// `l`/`obj`/`node` 必须有效且指向存活对象，`obj.extra` 有足够容量。
unsafe fn get_node_key(l: *mut lua_State, obj: *mut TValue, node: *const LuaNode) {
  unsafe {
    let key = addr_of!((*(node as *const LocalLuaNode)).key);
    (*obj).value = (*key).value;
    copy_nonoverlapping(
      (*key).extra.as_ptr(),
