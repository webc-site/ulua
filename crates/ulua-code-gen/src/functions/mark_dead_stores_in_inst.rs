...
use crate::{
  ...
  records::{
    ir_block::IrBlock, ir_builder::IrBuilder, ir_data::K_UNKNOWN_TAG, ir_function::IrFunction,
  ...
  match inst.cmd {
    ...
    IrCmd::StorePointer => {
      if inst.ops[0].kind() == IrOpKind::VmReg {
        ...
        let maybe_gco = reg_info.known_tag == K_UNKNOWN_TAG || is_gco(reg_info.known_tag);
        ...
        if reg_info.known_tag != K_UNKNOWN_TAG {
    ...
    IrCmd::StoreTvalue => {
      if inst.ops[0].kind() == IrOpKind::VmReg {
        ...
        reg_info.known_tag = try_get_operand_tag(function, inst.ops[1]).unwrap_or(K_UNKNOWN_TAG);
