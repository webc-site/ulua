...
pub fn emit_interrupt(build: &mut AssemblyBuilderA64) {
  ...
  build.ldr(X2, mem(R_STATE, offset_of!(lua_State, global) as i32));
  build.ldr(
    X2,
    mem(
      X2,
  ...
  build.ldr(X1, mem(R_STATE, offset_of!(lua_State, ci) as i32));
  ...
  build.ldrb(W0, mem(R_STATE, offset_of!(lua_State, status) as i32));
