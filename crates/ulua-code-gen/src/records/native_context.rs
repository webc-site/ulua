...
pub struct NativeContext {
  /// JIT gate 入口，由 `init_header_functions` 注册（ABI 契约见 `native_fn::GateFn`）；
...
impl Default for NativeContext {
  fn default() -> Self {
    // Safety: 本结构为传给 JIT 生成机器码的 #[repr(C)] ABI 块，cpp 侧即
    // `memset(&context, 0, sizeof(context))`；字段全为裸指针/`Option<fn>`
    // (None 的 niche 为 0) 与函数指针表，全零位即合法的「未挂钩」状态。
    unsafe { zeroed() }
  }
}
