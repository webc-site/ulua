use crate::{
  ...
  functions::{
    reg_bitset::{reg_bit_set, reg_bit_test},
...
impl<'a> BlockVmRegLiveInComputation<'a> {
  ...
  pub fn def(&mut self, op: IrOp, offset: i32) {
    ...
    reg_bit_set(&mut self.def_rs.regs, reg);
  }

  pub fn r#use(&mut self, op: IrOp, offset: i32) {
    let reg = (vm_reg_op(op) + offset) as usize;
    if !reg_bit_test(&self.def_rs.regs, reg) {
      reg_bit_set(&mut self.in_rs.regs, reg);
    }
  }
  ...
  pub fn capture(&mut self, reg: i32) {
    let r = reg as usize;
    reg_bit_set(self.captured_regs, r);
  }
}
