use crate::{
  ...
  macros::{op_a_ref::op_a_ref, op_b_ref::op_b_ref, op_e_ref::op_e_ref},
  records::{
...
impl ConstPropState {
  pub fn try_merge_buffer_range_check(
    ...
    if op_a_ref(inst) != op_a_ref(prev) {
      return false;
    }

    let curr_index_op = op_b_ref(inst);
    ...
    if curr_index.cmd == IrCmd::NumToInt && prev_index.cmd == IrCmd::NumToInt {
      let curr_index_source = op_a_ref(curr_index);
      let prev_index_source = op_a_ref(prev_index);
        ...
        && offset_base_curr.op.kind() != IrOpKind::Constant
      {
        ...
        if op_e_ref(prev).kind() == IrOpKind::Undef {
          let replacement = op_a_ref(prev_index);
          unsafe {
