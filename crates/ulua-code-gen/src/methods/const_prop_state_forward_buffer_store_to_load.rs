use crate::{
  ...
  macros::{op_a::op_a, op_b_ref::op_b_ref, op_c_ref::op_c_ref, op_d_ref::op_d_ref},
  records::{
...
impl ConstPropState {
  pub fn forward_buffer_store_to_load(
    ...
    let tag = unsafe { &mut *self.function }.tag_op(op_d_ref(store_inst));
    ...
    let offset = unsafe { &mut *self.function }.int_op(op_b_ref(store_inst));
    ...
    if op_c_ref(store_inst).kind() == IrOpKind::Constant {
      if load_cmd == IrCmd::BufferReadi8 {
        let v_i32 = unsafe { &mut *self.function }.int_op(op_c_ref(store_inst));
        ...
        let v_f64 = unsafe { &mut *self.function }.double_op(op_c_ref(store_inst));
