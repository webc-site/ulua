...
use crate::{
  ...
  records::{
    ...
    ir_call_wrapper_x_64::IrCallWrapperX64, ir_const::IrConst, ir_data::K_INVALID_INST_IDX,
    ir_inst::IrInst, ir_lowering_x_64::IrLoweringX64, ir_op::IrOp, label::Label,
    ...
    unsafe {
      ...
      match inst.cmd {
        ...
        IrCmd::SelectVec => {
          ...
          let o_a = self.vec_op(*get_op_mut(inst, 0), &mut tmp1);
          let o_b = self.vec_op(*get_op_mut(inst, 1), &mut tmp2);
          (*self.build).vblendvps(inst.reg_x64, o_a, OperandX64::reg(o_b), inst.reg_x64);
        }
        ...
        IrCmd::JumpCmpInt64 => {
          ...
          (*self.build).jcc(cc, &mut *self.label_op(*get_op_mut(inst, 3)));
          let target_block = self.block_op(*get_op_mut(inst, 4));
          self.jump_or_fallthrough(&mut *target_block, next);
        }
        IrCmd::JumpEqPointer => {
        ...
        IrCmd::TryCallFastgettm => {
          {
            ...
            (*self.build).mov(
              ...
              OperandX64::mem(
                ...
                1,
                r_state(),
                (offset_of!(lua_State, global) as i32),
              ),
            );
          ...
          if (*get_op_mut(inst, 1_u32)).kind() == IrOpKind::Constant {
            ...
            (*self.build).mov(
              OperandX64::mem(
                ...
                1,
                r_state(),
                (offset_of!(lua_State, top) as i32),
              ),
              OperandX64::reg(tmp.reg),
            );
          } else if (*get_op_mut(inst, 1_u32)).kind() == IrOpKind::Inst {
        ...
        IrCmd::DoArith => {
          ...
          // SAFETY：tm 来自编译器写入 IR 的 TMS 常量（界内连续），与 C++ `TMS(tm)` 强转一致
        ...
        IrCmd::CloseUpvals => {
          {
            ...
            (*self.build).mov(
              ...
              OperandX64::mem(
                ...
                1,
                r_state(),
                (offset_of!(lua_State, openupval) as i32),
              ),
            );
          ...
          (*self.build).mov(
            ...
            OperandX64::mem(
              ...
              (self.uint_op(*get_op_mut(inst, 2)) as i32)
                .wrapping_mul(size_of::<*mut Proto>() as i32),
            ),
          );
