use crate::{
  ...
  macros::{op_a::op_a, op_b::op_b, opt_op_a::opt_op_a, opt_op_b_ref::opt_op_b_ref},
...
impl ConstPropState {
  pub fn get_offset_base(&mut self, value: IrOp) -> BufferAccessBase {
    ...
    loop {
      ...
      let rhs_num = unsafe { (*self.function).as_double_op(opt_op_b_ref(&inst)) };
      let lhs_int = unsafe { (*self.function).as_int_op(opt_op_a(inst.clone())) };
      let rhs_int = unsafe { (*self.function).as_int_op(opt_op_b_ref(&inst)) };
