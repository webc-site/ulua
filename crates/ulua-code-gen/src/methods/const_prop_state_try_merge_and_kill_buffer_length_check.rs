use crate::{
  ...
  macros::{op_c_ref::op_c_ref, op_d_ref::op_d_ref},
  records::{
...
impl ConstPropState {
  pub fn try_merge_and_kill_buffer_length_check(
    ...
    let prev_min_offset = unsafe { (*self.function).int_op(op_c_ref(prev_check)) };
    let prev_max_offset = unsafe { (*self.function).int_op(op_d_ref(prev_check)) };
    let curr_min_offset = unsafe { (*self.function).int_op(op_c_ref(curr_check)) } + extra_offset;
    let curr_max_offset = unsafe { (*self.function).int_op(op_d_ref(curr_check)) } + extra_offset;
