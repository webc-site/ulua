...
impl IrLoweringA64 {
  ...
  /// 按 IrOp 取块指针（cpp blockOp 的 unsafe 形态）。
  /// # Safety
  /// `op` 必须是函数内合法 block 下标。
    ...
    unsafe {
      ...
      match inst.cmd {
        IrCmd::LoadTag => {
          ...
          let addr = self.temp_addr(*get_op_mut(inst, 0), (offset_of!(TValue, tt) as i32));
        ...
        IrCmd::LoadInt => {
          inst.reg_a64 = self.regs.alloc_reg(KindA64::W, index);
          let addr = self.temp_addr(*get_op_mut(inst, 0), (offset_of!(TValue, value) as i32));
          (*self.build).ldr(inst.reg_a64, addr);
        }
        IrCmd::LoadInt64 => {
          inst.reg_a64 = self.regs.alloc_reg(KindA64::X, index);
        ...
        IrCmd::GetArrAddr => {
          {
            ...
            if (*get_op_mut(inst, 1)).kind() == IrOpKind::Inst {
              ...
              if self.int_op(*get_op_mut(inst, 1)) == 0 {
                ...
                let stride = (self.int_op(*get_op_mut(inst, 1)) as i64 as u64)
                  .wrapping_mul(size_of::<TValue>() as u64);
                if stride <= K_MAX_IMMEDIATE as u64 {
                  (*self.build).add(inst.reg_a64, inst.reg_a64, stride as u16);
                } else {
                  let temp = self.regs.alloc_temp(KindA64::X);
                  (*self.build).mov(temp, stride as u32 as i32);
        ...
        IrCmd::GetSlotNodeAddr => {
          {
            ...
            (*self.build).ldrb(
              ...
              mem(
                self.reg_op(*get_op_mut(inst, 0)),
                (offset_of!(LuaTable, nodemask8) as i32),
              ),
            );
            (*self.build)
        ...
        IrCmd::StoreExtra => {
          let addr = self.temp_addr(*get_op_mut(inst, 0), (offset_of!(TValue, extra) as i32));
          if self.int_op(*get_op_mut(inst, 1)) == 0 {
            (*self.build).str(WZR, addr);
          } else {
            let temp = self.regs.alloc_temp(KindA64::W);
        ...
        IrCmd::StoreVector => {
          ...
          if has_op_e(inst) {
            ...
            (*self.build).str(
              temp,
              self.temp_addr(*get_op_mut(inst, 0), (offset_of!(TValue, tt) as i32)),
            );
          }
        }
        IrCmd::StoreTvalue => {
        ...
        IrCmd::SignFloat => {
          ...
          (*self.build).fmov(temp0, 0.0f32);
          (*self.build).fmov(temp1, 1.0f32);
          (*self.build).fcsel(
            inst.reg_a64,
            temp1,
            temp0,
        ...
        IrCmd::CmpAny => {
          {
            ...
            if cond == IrCondition::Equal {
              ...
              (*self.build).ldr(
                tempb,
                self.temp_addr(*get_op_mut(inst, 1), (offset_of!(TValue, tt) as i32)),
              );
        ...
        IrCmd::CmpTag => {
          ...
          if (*get_op_mut(inst, 1)).kind() == IrOpKind::Inst {
            ...
            let addr = self.temp_addr(*get_op_mut(inst, 1), (offset_of!(TValue, tt) as i32));
        ...
        IrCmd::JumpCmpInt64 => {
          ...
          let mut lhs = *get_op_mut(inst, 0);
          let mut rhs = *get_op_mut(inst, 1);
          let swapped = lhs.kind() == IrOpKind::Constant;

          if swapped {
            lhs = *get_op_mut(inst, 1);
            rhs = *get_op_mut(inst, 0);
          }
          ...
          {
            (*self.build).cbz(self.reg_op(lhs), &mut *self.label_op(*get_op_mut(inst, 3)));
          } else if cond == IrCondition::NotEqual
            && rhs.kind() == IrOpKind::Constant
            && self.int_64_op(rhs) == 0
          {
            (*self.build).cbnz(self.reg_op(lhs), &mut *self.label_op(*get_op_mut(inst, 3)));
          } else {
            if rhs.kind() == IrOpKind::Constant
              && (self.int_64_op(rhs) as u64) <= K_MAX_IMMEDIATE as u64
            {
              (*self.build).cmp(self.reg_op(lhs), self.int_64_op(rhs) as u16);
            } else {
              let temp = self.temp_int64(rhs);
              (*self.build).cmp(self.reg_op(lhs), temp);
            }
        ...
        IrCmd::TryCallFastgettm => {
          {
            ...
            (*self.build).ldrb(temp2, mem(temp1, (offset_of!(LuaTable, tmcache) as i32)));
            ...
            (*self.build).ldr(
              ...
              mem(
                ...
                (offset_of!(global_State, tmname) as i32)
                  + self
                    .int_op(*get_op_mut(inst, 1))
                    .wrapping_mul(size_of::<*mut tstring>() as i32),
              ),
            );
            (*self.build).ldr(
              X3,
        ...
        IrCmd::AdjustStackToReg => {
          {
            ...
            if (*get_op_mut(inst, 1)).kind() == IrOpKind::Constant {
              ...
              (*self.build).str(temp, mem(R_STATE, (offset_of!(lua_State, top) as i32)));
        ...
        IrCmd::AdjustStackToTop => {
          let temp = self.regs.alloc_temp(KindA64::X);
          (*self.build).ldr(temp, mem(R_STATE, (offset_of!(lua_State, ci) as i32)));
          (*self.build).ldr(temp, mem(temp, (offset_of!(CallInfo, top) as i32)));
          (*self.build).str(temp, mem(R_STATE, (offset_of!(lua_State, top) as i32)));
        }
        IrCmd::FASTCALL => {
          self
        ...
        IrCmd::InvokeFastcall => {
          {
            ...
            if (*get_op_mut(inst, 4)).kind() != IrOpKind::Undef {
              ...
              (*self.build).ldr(X4, mem(R_STATE, (offset_of!(lua_State, top) as i32)));
            ...
            if self.int_op(*get_op_mut(inst, 5)) == LUA_MULTRET {
              // l->top - (ra + 1)
              (*self.build).ldr(X5, mem(R_STATE, (offset_of!(lua_State, top) as i32)));
        ...
        IrCmd::DoArith => {
          ...
          match self.int_op(*get_op_mut(inst, 3)) as u32 {
            x if x == TMS::TmAdd as u32 => (*self.build).ldr(
              ...
              mem(
                R_NATIVE_CONTEXT,
                (offset_of!(NativeContext, lua_v_doarithadd) as i32),
              ),
            ),
            x if x == TMS::TmSub as u32 => (*self.build).ldr(
              ...
              mem(
                R_NATIVE_CONTEXT,
                (offset_of!(NativeContext, lua_v_doarithsub) as i32),
              ),
            ),
            x if x == TMS::TmMul as u32 => (*self.build).ldr(
              ...
              mem(
                R_NATIVE_CONTEXT,
                (offset_of!(NativeContext, lua_v_doarithmul) as i32),
              ),
            ),
            x if x == TMS::TmDiv as u32 => (*self.build).ldr(
              ...
              mem(
                R_NATIVE_CONTEXT,
                (offset_of!(NativeContext, lua_v_doarithdiv) as i32),
              ),
            ),
            x if x == TMS::TmIDiv as u32 => (*self.build).ldr(
              ...
              mem(
                R_NATIVE_CONTEXT,
                (offset_of!(NativeContext, lua_v_doarithidiv) as i32),
              ),
            ),
            x if x == TMS::TmMod as u32 => (*self.build).ldr(
              ...
              mem(
                R_NATIVE_CONTEXT,
                (offset_of!(NativeContext, lua_v_doarithmod) as i32),
              ),
            ),
            x if x == TMS::TmPow as u32 => (*self.build).ldr(
              ...
              mem(
                R_NATIVE_CONTEXT,
                (offset_of!(NativeContext, lua_v_doarithpow) as i32),
              ),
            ),
            x if x == TMS::TmUnm as u32 => (*self.build).ldr(
              ...
              mem(
                R_NATIVE_CONTEXT,
                (offset_of!(NativeContext, lua_v_doarithunm) as i32),
              ),
            ),
        ...
        IrCmd::GetTable => {
          ...
          (*self.build).ldr(
            ...
            mem(
              R_NATIVE_CONTEXT,
              (offset_of!(NativeContext, lua_v_gettable) as i32),
            ),
          );
          (*self.build).blr(X4);
        ...
        IrCmd::GetUpvalue => {
          {
            ...
            let mut skip = Label::default();
            (*self.build).ldr(temp2, mem(temp1, (offset_of!(TValue, tt) as i32)));
            ...
            (*self.build).ldr(temp1, mem(temp1, (offset_of!(UpVal, v) as i32)));
        ...
        IrCmd::SetUpvalue => {
          {
            ...
            (*self.build).ldr(temp2, mem(temp1, (offset_of!(UpVal, v) as i32)));
              ...
              || is_gco(self.tag_op(*get_op_mut(inst, 2)))
            {
              ...
              (*self.build).ldr(
                ...
                mem(
                  R_NATIVE_CONTEXT,
                  (offset_of!(NativeContext, lua_c_barrierf) as i32),
                ),
              );
              (*self.build).blr(X3);
        ...
        IrCmd::CheckReadonly => {
          {
            ...
            (*self.build).ldrb(
              ...
              mem(
                self.reg_op(*get_op_mut(inst, 0)),
                (offset_of!(LuaTable, readonly) as i32),
              ),
            );
            let target =
        ...
        IrCmd::JumpSlotMatch | IrCmd::CheckSlotMatch => {
          {
            ...
            let addr = self.temp_addr(*get_op_mut(inst, 1), (offset_of!(TValue, value) as i32));
        ...
        IrCmd::CheckBufferLen => {
          {
            ...
            let reg_a = if (*get_op_mut(inst, 0)).kind() == IrOpKind::Inst {
              self.reg_op(*get_op_mut(inst, 0))
            } else {
              NOREG
            };
            let reg_b = if (*get_op_mut(inst, 1)).kind() == IrOpKind::Inst {
              self.reg_op(*get_op_mut(inst, 1))
            } else {
              NOREG
            };
            let reg_e = if (*get_op_mut(inst, 4)).kind() != IrOpKind::Undef {
            ...
            if (*get_op_mut(inst, 4)).kind() != IrOpKind::Undef {
              ...
              if ((*self.build).features & FEATURE_JSCVT) != 0 {
                ...
                (*self.build).fjcvtzs(temp, reg_e); // fjcvtzs sets PSTATE.Z (equal) iff conversion is exact
                (*self.build).b_condition_a_64_label(ConditionA64::NotEqual, &mut *target);
              } else {
                let temp = temp_d;

                (*self.build).scvtf(temp, reg_b);
                (*self.build).fcmp(reg_e, temp);
                (*self.build).b_condition_a_64_label(ConditionA64::NotEqual, &mut *target);
              }
            }

            let temp = temp_w1;
            (*self.build).ldr(temp, mem(reg_a, K_BUFFER_LEN_OFFSET));
            ...
            if (*get_op_mut(inst, 1)).kind() == IrOpKind::Inst {
              ...
              if access_size == 1 && min_offset == 0 {
                ...
                (*self.build).sub(tempx, tempx, reg_b); // implicit uxtw
              ...
              // note: this is a signed 64-bit comparison so that out of bounds offset fails
                ...
                if min_offset >= 0 {
                  (*self.build).add(cast_reg(KindA64::W, temp2), reg_b, (min_offset) as u16);
                } else {
                  (*self.build).sub(cast_reg(KindA64::W, temp2), reg_b, (-min_offset) as u16);
                }
        ...
        IrCmd::BarrierTableBack => {
          {
            ...
            (*self.build).mov(X1, reg);
            (*self.build).mov(X0, R_STATE);
            (*self.build).add(X2, X1, (offset_of!(LuaTable, gclist) as i32) as u16);
            (*self.build).ldr(
              X3,
              mem(
                R_NATIVE_CONTEXT,
        ...
        IrCmd::SetSavedpc => {
          ...
          emit_add_offset(
            ...
            R_CODE,
            (self.uint_op(*get_op_mut(inst, 0)) as i32)
              .wrapping_mul(size_of::<Instruction>() as i32),
          );
          (*self.build).ldr(temp2, mem(R_STATE, (offset_of!(lua_State, ci) as i32)));
          (*self.build).str(temp1, mem(temp2, (offset_of!(CallInfo, savedpc) as i32)));
        }
        IrCmd::CloseUpvals => {
          {
            let mut skip = Label::default();
            ...
            (*self.build).ldr(
              temp1,
              mem(R_STATE, (offset_of!(lua_State, openupval) as i32)),
            );
        ...
        IrCmd::CALL => {
          ...
          if self.int_op(*get_op_mut(inst, 1)) == LUA_MULTRET {
            (*self.build).ldr(X2, mem(R_STATE, (offset_of!(lua_State, top) as i32)));
          } else {
            (*self.build).add(
              X2,
              r_base(),
          ...
          (*self.build).ldr(
            ...
            mem(
              R_NATIVE_CONTEXT,
              (offset_of!(NativeContext, call_fallback) as i32),
            ),
          );
          (*self.build).blr(X4);
        ...
        IrCmd::RETURN => {
          ...
          if (*self.function).variadic {
            (*self.build).ldr(X1, mem(R_STATE, (offset_of!(lua_State, ci) as i32)));
            (*self.build).ldr(X1, mem(X1, (offset_of!(CallInfo, func) as i32)));
        ...
        IrCmd::FORGLOOP => {
          ...
          (*self.build).ldr(
            ...
            mem(
              R_NATIVE_CONTEXT,
              (offset_of!(NativeContext, forg_loop_table_iter) as i32),
            ),
          );
          (*self.build).blr(X4);
        ...
        IrCmd::ForgloopFallback => {
          ...
          if fflag::LuauYieldIter2.get() {
            ...
            (*self.build).ldr(
              ...
              mem(
                R_NATIVE_CONTEXT,
                (offset_of!(NativeContext, forg_loop_non_table_fallback_deprecated) as i32),
              ),
            );
            (*self.build).blr(X3);
            emit_update_base(&mut *self.build);
        ...
        IrCmd::FallbackGetglobal => {
          ...
          emit_fallback(
            &mut *self.build,
            (offset_of!(NativeContext, execute_getglobal) as i32),
            self.uint_op(*get_op_mut(inst, 0)),
          );
        }
        IrCmd::FallbackSetglobal => {
          ...
          emit_fallback(
            &mut *self.build,
            (offset_of!(NativeContext, execute_setglobal) as i32),
            self.uint_op(*get_op_mut(inst, 0)),
          );
        }
        IrCmd::FallbackGettableks => {
          ...
          emit_fallback(
            &mut *self.build,
            (offset_of!(NativeContext, execute_gettableks) as i32),
            self.uint_op(*get_op_mut(inst, 0)),
          );
        }
        IrCmd::FallbackSettableks => {
          ...
          emit_fallback(
            &mut *self.build,
            (offset_of!(NativeContext, execute_settableks) as i32),
            self.uint_op(*get_op_mut(inst, 0)),
          );
        }
        IrCmd::FallbackNamecall => {
          ...
          emit_fallback(
            &mut *self.build,
            (offset_of!(NativeContext, execute_namecall) as i32),
            self.uint_op(*get_op_mut(inst, 0)),
          );
        }
        IrCmd::FallbackPrepvarargs => {
          ...
          emit_fallback(
            &mut *self.build,
            (offset_of!(NativeContext, execute_prepvarargs) as i32),
            self.uint_op(*get_op_mut(inst, 0)),
          );
        }
        ...
        IrCmd::NEWCLOSURE => {
          {
            ...
            (*self.build).ldr(X3, mem(X3, (offset_of!(Proto, p) as i32)));
        ...
        IrCmd::FallbackDupclosure => {
          ...
          emit_fallback(
            &mut *self.build,
            (offset_of!(NativeContext, execute_dupclosure) as i32),
            self.uint_op(*get_op_mut(inst, 0)),
          );
        }
        IrCmd::FallbackForgprep => {
          ...
          emit_fallback(
            &mut *self.build,
            (offset_of!(NativeContext, execute_forgprep) as i32),
            self.uint_op(*get_op_mut(inst, 0)),
          );
          self.jump_or_fallthrough_op(*get_op_mut(inst, 2), next);
        ...
        IrCmd::BitlrotateUint => {
          {
              ...
              && (*get_op_mut(inst, 1)).kind() == IrOpKind::Constant
            {
              ...
              (*self.build).ror(
                ...
                (32u32.wrapping_sub(self.int_op(*get_op_mut(inst, 1)) as u32) & 31) as u8,
              );
            } else {
              inst.reg_a64 = self
                .regs
        ...
        IrCmd::GetTypeof => {
          ...
          (*self.build).ldr(
            ...
            mem(
              R_NATIVE_CONTEXT,
              (offset_of!(NativeContext, lua_t_objtypenamestr) as i32),
            ),
          );
          (*self.build).blr(X2);
        ...
        IrCmd::FINDUPVAL => {
          ...
          (*self.build).ldr(
            ...
            mem(
              R_NATIVE_CONTEXT,
              (offset_of!(NativeContext, lua_f_findupval) as i32),
            ),
          );
          (*self.build).blr(X2);
