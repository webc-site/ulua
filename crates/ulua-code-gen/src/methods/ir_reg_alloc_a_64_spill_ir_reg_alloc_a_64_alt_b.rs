...
use crate::{
  ...
  functions::{
    alloc_spill::alloc_spill, get_reload_offset::get_reload_offset,
    is_used_in_vm_exit_sync::is_used_in_vm_exit_sync, vm_reg_op::vm_reg_op,
  },
...
impl IrRegAllocA64 {
  pub fn spill_set_u32_u32(&mut self, set: &mut Set, index: u32, target_inst_idx: u32) {
    ...
    if unsafe { (*def).last_use } == index
      && !is_used_in_vm_exit_sync(unsafe { &mut *self.function }, index, target_inst_idx)
    {
      ...
      if self.is_extra_spill_slot(slot as u32) {
        ...
        unsafe {
          ...
          (*self.build).ldr(
            emergency_temp,
            mem(R_STATE, offset_of!(lua_State, global) as i32),
          );
          (*self.build).ldr(
            emergency_temp,
