...
mod new_basic_overload_selection {
...
mod new_basic_overload_selection1 {
...
mod new_metamethod_could_be_overloaded {
  ...
  fn new_metamethod_could_be_overloaded() {
    ...
    // __call 自身可以是重载的
    let tbl =
      fixture.table_with_call(fixture.meet_initializer_list_type_id(&[overload1, overload2]));
...
mod new_overload_group_could_include_metamethod {
...
mod new_overloads_with_different_arities {
...
mod new_overloads_with_different_arities1 {
...
mod new_separate_non_viable_overloads_by_arity_mismatch {
...
mod new_select {
  ...
  fn new_select() {
    ...
    let select_ty: TypeId = unsafe {
      let arg_types = (*fixture.arena).add_type_pack_vector_type_id_optional_type_pack_id(
...
mod new_pass_table_with_indexer {
  ...
  fn new_pass_table_with_indexer() {
    ...
    // {[any]: number}
    let any_number_table: TypeId = unsafe {
...
mod generic_higher_order_function_called_improperly {
  ...
  fn generic_higher_order_function_called_improperly() {
    ...
    let (generic_a, generic_bs, generic_cs): (TypeId, TypePackId, TypePackId) = unsafe {
      ...
      let generic_bs =
        (*fixture.arena).add_type_pack_t(GenericTypePack::new_name(String::from("B")));
      let generic_cs =
        (*fixture.arena).add_type_pack_t(GenericTypePack::new_name(String::from("C")));
