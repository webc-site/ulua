...
fn type_infer_anyerror_any_type_propagates() {
  ...
  assert_eq!("any", to_string_type_id(fixture.require_type_string("bar")));
}
...
fn type_infer_anyerror_assign_prop_to_table_by_calling_any_yields_any() {
  ...
  let table_ty = follow_type::follow(fixture.require_type_string("T"));
  let table = get_type::get::<TableType>(table_ty).expect("expected table");
...
fn type_infer_anyerror_call_to_any_yields_any() {
  ...
  assert_eq!("any", to_string_type_id(fixture.require_type_string("b")));
}
...
fn type_infer_anyerror_calling_error_type_yields_error() {
  use ulua_analysis::{
    functions::to_string_to_string::to_string_type_id, records::unknown_symbol::UnknownSymbol,
  };
...
fn type_infer_anyerror_can_get_length_of_any() {
  ...
  assert_eq!(0, result.errors.len(), "{:?}", result.errors);
  let bar_type = fixture.require_type_string("bar");
  assert_eq!(
