...
fn type_infer_definitions_class_definition_function_prop() {
  ...
  assert_eq!(
    "(number) -> string",
    to_string_type_id(fixture.require_type_string("prop"))
  );
}
...
fn type_infer_definitions_class_definition_indexer() {
  ...
  let etv = get_type::get::<ExternType>(fixture.require_type_string("x"))
...
fn type_infer_definitions_class_definition_overload_metamethods() {
  ...
  assert_eq!(
    "CFrame",
    to_string_type_id(fixture.require_type_string("shouldBeCFrame"))
  );
  assert_eq!(
    "Vector3",
    to_string_type_id(fixture.require_type_string("shouldBeVector"))
  );
}
...
fn type_infer_definitions_declaring_generic_functions() {
  ...
  assert_eq!(
    "boolean",
    to_string_type_id(fixture.require_type_string("w"))
  );
  assert_eq!(
    "number",
...
fn type_infer_definitions_definition_file_class_function_args() {
  ...
  assert_eq!(
    ...
    to_string_type_id_to_string_options(fixture.require_type_string("methodRef1"), &mut opts)
  );
  assert_eq!(
    "(self: Foo, x: number, y: string) -> number",
    to_string_type_id_to_string_options(fixture.require_type_string("methodRef2"), &mut opts)
  );
  assert_eq!(
    "(a: number, b: string) -> string",
    to_string_type_id_to_string_options(fixture.require_type_string("prop"), &mut opts)
  );
}
...
fn type_infer_definitions_definition_file_extern_types() {
  ...
  assert_eq!(
    "number",
    to_string_type_id(fixture.require_type_string("inheritedProp"))
  );
  assert_eq!(
    "number",
    to_string_type_id(fixture.require_type_string("method"))
  );
  assert_eq!(
    "string",
    to_string_type_id(fixture.require_type_string("method2"))
  );
  assert_eq!(
    "Bar",
    to_string_type_id(fixture.require_type_string("metamethod"))
  );
  assert_eq!(
    "number",
    to_string_type_id(fixture.require_type_string("inheritedMethod"))
  );
}
...
fn type_infer_definitions_definition_file_has_source_module_name_set() {
  ...
  let etv = get_type::get::<ExternType>(foo_ty.r#type()).expect("expected Foo extern type");
...
fn type_infer_definitions_definition_file_loading() {
  ...
  use ulua_analysis::functions::{
    get_global_binding::get_global_binding, to_string_to_string::to_string_type_id,
  };
  use ulua_unit_test::records::fixture::Fixture;
...
fn type_infer_definitions_definitions_documentation_symbols() {
  ...
  use ulua_analysis::{
    functions::{get_mutable_type, try_get_global_binding::try_get_global_binding},
  ...
  let bar_class =
    get_mutable_type::get_mutable::<ExternType>(bar_ty.r#type()).expect("expected Bar extern type");
  ...
  let y_table =
    get_mutable_type::get_mutable::<TableType>(y_binding.type_id).expect("expected y table type");
...
fn type_infer_definitions_definitions_symbols_are_generated_for_recursively_referenced_types() {
  ...
  let class = get_mutable_type::get_mutable::<ExternType>(my_class_ty.r#type())
  ...
  let function = get_mutable_type::get_mutable::<FunctionType>(method_ty)
    .expect("expected myMethod function type");
  let definition = function.definition().expect("myMethod function definition");

  assert_eq!(
    Some(&ModuleName::from("@test")),
...
fn type_infer_definitions_load_definition_file_errors_do_not_pollute_global_scope() {
  ...
  use ulua_analysis::functions::try_get_global_binding::try_get_global_binding;
  use ulua_unit_test::records::fixture::Fixture;
  ...
  let frontend = fixture.get_frontend();
  let target_scope = frontend.globals.global_scope();
  let parse_fail_result = frontend.load_definition_file(
    |frontend| &mut frontend.globals,
    target_scope,
    r#"
        declare foo
  ...
  let foo_ty = try_get_global_binding(&mut frontend.globals, "foo");
  assert!(foo_ty.is_none());

  let target_scope = frontend.globals.global_scope();
  let check_fail_result = frontend.load_definition_file(
    |frontend| &mut frontend.globals,
    target_scope,
    r#"
  ...
  let bar_ty = try_get_global_binding(&mut frontend.globals, "bar");
