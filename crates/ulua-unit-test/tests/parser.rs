...
mod parser_export_value_rfc {
  ...
  fn parser_export_value_rfc() {
    ...
    for node in unsafe { from_raw_parts((*abc).vars.data, (*abc).vars.size as usize) } {
      assert!(unsafe { (**node).is_exported });
      assert!(!unsafe { (**node).is_const });
    }
    ...
    for node in unsafe {
      from_raw_parts(
        (*forward_decls).vars.data,
        (*forward_decls).vars.size as usize,
      )
    } {
      assert!(unsafe { (**node).is_exported });
      assert!(!unsafe { (**node).is_const });
    ...
    for node in unsafe { from_raw_parts((*xyz).vars.data, (*xyz).vars.size as usize) } {
      assert!(unsafe { (**node).is_exported });
      assert!(!unsafe { (**node).is_const });
    }
...
mod parser_parse_if_else_expression {
  ...
  fn parser_parse_if_else_expression() {
...

    {
      ...
      assert!(!stat.is_null());
      let block = unsafe {
        rtti::ast_node_as::<AstStatBlock>(addr_of_mut!((*stat).base) as *mut ast_node::AstNode)
      };
      assert!(!block.is_null());
      assert!((unsafe { (*block).body.size }) > 0);
      let return_stat = unsafe { (*block).body.data.add(0).read() };
      let return_stat =
        unsafe { rtti::ast_node_as::<AstStatReturn>(addr_of_mut!((*return_stat).base)) };
      assert!(!return_stat.is_null());
      assert!((unsafe { (*return_stat).list.size }) == 1);
      let expr = unsafe { (*return_stat).list.data.add(0).read() };
      let if_else_expr = unsafe { rtti::ast_node_as::<AstExprIfElse>(addr_of_mut!((*expr).base)) };
...

    {
      ...
      assert!((unsafe { (*return_stat).list.size }) == 1);
      let expr = unsafe { (*return_stat).list.data.add(0).read() };
      let if_else_expr1 = unsafe { rtti::ast_node_as::<AstExprIfElse>(addr_of_mut!((*expr).base)) };
      assert!(!if_else_expr1.is_null());
      let false_expr = unsafe { (*if_else_expr1).false_expr };
      let if_else_expr2 =
        unsafe { rtti::ast_node_as::<AstExprIfElse>(addr_of_mut!((*false_expr).base)) };
...

    {
      ...
      let nested_if_else_expr =
        unsafe { rtti::ast_node_as::<AstExprIfElse>(addr_of_mut!((*condition).base)) };
