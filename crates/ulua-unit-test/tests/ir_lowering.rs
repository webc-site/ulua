...
/// cpp `IrLowering.test.cpp` 缺省参样板收口：夹具逐用例独占，`(source, false, 1, 2, false)`
/// （include_ir_types=false、debug=1、opt=2、clip_to_first_return=false）为最多的恒等传参
/// 形态，语义与原调用逐位一致。
#[inline]
fn codegen(source: &str) -> String {
  LoweringFixture::default().get_codegen_assembly(source, false, 1, 2, false)
}

/// 同 [`codegen`]，仅开放随用例变化的 `include_ir_types` / `clip_to_first_return`
/// 两参（debug=1、opt=2 恒为 1、2）。
#[inline]
fn codegen_flags(source: &str, include_ir_types: bool, clip_to_first_return: bool) -> String {
  LoweringFixture::default().get_codegen_assembly(
    source,
    include_ir_types,
    1,
    2,
...
fn ir_lowering_vector_custom_namecall_chain_2() {
  use ulua_common::{fflag, fflag::LuauCodegenPropagateFallbackTags};
  use ulua_unit_test::type_aliases::scoped_fast_flag::ScopedFastFlag;
...
fn ir_lowering_integer_compare() {
  ...
  let _luau_codegen_integer_3 = ScopedFastFlag::new(&fflag::LuauCodegenInteger3, true);
  let _luau_integer_type = ScopedFastFlag::new(&fflag::LuauIntegerType2, true);
  let _luau_codegen_integer_compare = ScopedFastFlag::new(&fflag::LuauCodegenIntegerCompare, true);
  let assembly = codegen_flags(
    r#"
...
// 回归测试（opt-r84）：A64 lowering 的 BitarshiftInt64 臂此前缺失
// `set_label_label(negative)`，negative 标签只被 `b(Less, negative)` 分支引用却
// 永不绑定，finalize 时命中「未分配标签」断言（SIGTRAP/SIGABRT）。
// 触发形态：integer 库整数位移被内联成 BitarshiftInt64 IR 的采样（R83 A/B 差分样本）。
#[test]
fn ir_lowering_a64_bit_arshift_int64_binds_negative_label() {
  use ulua_common::fflag;
  use ulua_unit_test::type_aliases::scoped_fast_flag::ScopedFastFlag;

  let _f1 = ScopedFastFlag::new(&fflag::LuauIntegerFastcalls, true);
  let _f2 = ScopedFastFlag::new(&fflag::LuauCodegenInteger3, true);
  let _f3 = ScopedFastFlag::new(&fflag::LuauIntegerType2, true);
  let assembly = codegen(
    r#"
local function f(a: integer, b: integer)
    local r1 = integer.lshift(a, b)
    local r2 = integer.lshift(a, 5)
    local r3 = integer.rshift(a, 5)
    local r4 = integer.lshift(a, 63)
    local r5 = integer.rshift(a, 63)
    local r6 = integer.arshift(a, b)
    local r7 = integer.arshift(a, 5)
    local r8 = integer.lrotate(a, 7i)
    local r9 = integer.rrotate(a, b)
