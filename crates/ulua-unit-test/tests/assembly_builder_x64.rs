...
mod assembly_builder_x_64_constant_caching {
  ...
  fn assembly_builder_x_64_constant_caching() {
    ...
    // 强制 data relocation：填充 4096 个常量
    (0..4096).for_each(|i| {
      build.f64(i as f64);
    });
...
mod assembly_builder_x_64_constant_storage {
  ...
  fn assembly_builder_x_64_constant_storage() {
    ...
    // 每 4 字节一个小端 i32，倒序存 (3000 - i)
    for (i, chunk) in build.data.as_chunks::<4>().0.iter().enumerate() {
      let delta = 3000 - i as i32;
      assert_eq!(chunk[0], (delta & 0xff) as u8);
      assert_eq!(chunk[1], (delta >> 8) as u8);
      assert_eq!(chunk[2], 0x00);
      assert_eq!(chunk[3], 0x00);
    }
  }
}
