...
fn type_infer_union_types_indexing_into_a_cyclic_union_doesnt_crash() {
  ...
  use ulua_analysis::{
    ...
    records::{
      scope::Scope, table_indexer::TableIndexer, table_type::TableType, type_fun::TypeFun,
  ...
  // (a) 类 `as *mut Frontend`/`as *mut BuiltinTypes` 绕道已消除：直借
  // `&mut Frontend`，内建单例走 `builtin_types_ref` chokepoint（借用不绑定
  // frontend）。global_scope_ptr 为 (b) 类句柄：被测 API 形参即 `*mut Scope`，
  // Arc::as_ptr 仅取址不解引用，底层块由本帧 `global_scope` 强引用保活。
  let mut fixture = Fixture::fixture_bool(false);
  {
    ...
    let bad_cyclic_union_ty =
      arena.fresh_type_not_null_builtin_types_scope(builtins, global_scope_ptr);
    ...
    // Safety: bad_cyclic_union_ty 是 arena 刚产出的存活 TypeId；as_mutable_type_id
    // 仅去 const（cpp const_cast 同义），此处按 cpp `badCyclicUnion->ty = Union{…}`
    // 同款回写变体，本帧单线程、无第二可变借用。
    unsafe {
    ...
    // Safety: global_scope_ptr 为 Arc<Scope>（本帧强引用保活）堆块地址，非空存活；
    // 对应 cpp exportedTypeBindings 写入，insert 借用随语句结束。
    unsafe {
...
fn type_infer_union_types_return_types_can_be_disjoint() {
  ...
  assert_eq!(0, result.errors.len(), "{:?}", result.errors);
  let utv =
    get_type::get::<FunctionType>(fixture.require_type_string("most_of_the_natural_numbers"));
  assert!(utv.is_some());
}
...
// Ported from `tests/TypeInfer.unionTypes.test.cpp` ("oss_2134").
// 泛型高阶函数实例化：addIndex 对 filter/map 的两次实例化不得报错。
#[test]
fn type_infer_union_types_oss_2134() {
  use ulua_unit_test::records::fixture::Fixture;
...
// Ported from `tests/TypeInfer.unionTypes.test.cpp` ("oss_2393").
// 可选类型 T? 在泛型类型实例化中的协变/逆变统一不得报错。
#[test]
fn type_infer_union_types_oss_2393() {
  use ulua_unit_test::records::fixture::Fixture;
