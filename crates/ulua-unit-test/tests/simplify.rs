...
fn simplify_any_and_indeterminate_types() {
  use ulua_analysis::{functions::get_type, records::union_type::UnionType};
  ...
  let any_rhs_blocked = get_type::get::<UnionType>(actual).expect("expected union");
  ...
  let any_lhs_blocked = get_type::get::<UnionType>(actual).expect("expected union");
  ...
  let any_rhs_pending = get_type::get::<UnionType>(actual).expect("expected union");
  ...
  let any_lhs_pending = get_type::get::<UnionType>(actual).expect("expected union");
...
fn simplify_cyclic_never_union_and_string() {
  ...
  use ulua_analysis::{functions::get_mutable_type, records::union_type::UnionType};
  use ulua_unit_test::records::simplify_fixture::SimplifyFixture;
  ...
  let left_union =
    get_mutable_type::get_mutable::<UnionType>(left_type).expect("expected mutable union");
...
fn simplify_intersect_parts_empty_table_non_empty() {
  ...
  use ulua_analysis::{
    ...
    records::{
      arena_handle::Handle, property_type::Property, table_type::TableType,
      to_string_options::ToStringOptions, type_arena::TypeArena, type_ids::TypeIds,
...
fn simplify_simplify_stops_at_cycles() {
  ...
  let tt = get_mutable_type::get_mutable::<TableType>(t).expect("expected table");

  let t2 = fixture.mk_table(&[]);
  let t2t = get_mutable_type::get_mutable::<TableType>(t2).expect("expected table");
...
fn simplify_union_where_lhs_elements_are_a_subset_of_the_rhs() {
  use ulua_analysis::functions::to_string_to_string::to_string_type_id_to_string_options;
  use ulua_unit_test::records::simplify_fixture::SimplifyFixture;
...
mod simplify_unknown_and_concrete_simplify_test_case_2 {
  //! Source: `tests/Simplify.test.cpp`
