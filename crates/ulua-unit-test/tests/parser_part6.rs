...
mod parser_string_literal_call {
  ...
  fn parser_string_literal_call() {
    ...
    let arg =
      as_node_at::<AstExprConstantString, _>(&ec.args, 0).expect("args[0] 应为字符串字面量");
...
mod parser_string_literals_escape {
  ...
  fn parser_string_literals_escape() {
    ...
    for (index, want) in expected.into_iter().enumerate() {
      let str_ = as_node_at::<AstExprConstantString, _>(&ret.list, index)
        .expect("list 元素应为字符串字面量");
...
mod parser_type_assertion_expression_binds_tightly {
  ...
  fn parser_type_assertion_expression_binds_tightly() {
    ...
    let bin =
      as_node_at::<AstExprBinary, _>(&local.values, 0).expect("values[0] 应为 AstExprBinary");
    assert!(
