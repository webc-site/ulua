...
use ulua_code_gen::{
  ...
  functions::{
    ...
    compute_iterated_dominance_frontier_for_defs::compute_iterated_dominance_frontier_for_defs,
    ...
    to_dot::to_dot, to_dot_cfg::to_dot_cfg, to_dot_dj_graph::to_dot_dj_graph,
    to_string_ir_dump::to_string, update_use_counts::update_use_counts,
  },
  records::{idf_context::IdfContext, ir_inst::IrInst, ir_op::IrOp},
};
use ulua_common::{enums::luau_builtin_function::LuauBuiltinFunction, fflag};
use ulua_unit_test::{
  records::ir_builder_fixture::IrBuilderFixture, type_aliases::scoped_fast_flag::ScopedFastFlag,
};
use ulua_vm::enums::lua_type::LuaType;

// 各测试函数公用的 Lua tag 常量（cpp IrBuilder.test.cpp 顶部 `LUA_T*` 抄写），
// 由 136 个测试函数体内的重复声明统一上提到文件顶层一次。
pub(crate) const TNIL: u8 = LuaType::Nil as u8;
pub(crate) const TBOOLEAN: u8 = LuaType::Boolean as u8;
pub(crate) const TNUMBER: u8 = LuaType::Number as u8;
pub(crate) const TINTEGER: u8 = LuaType::Integer as u8;
pub(crate) const TVECTOR: u8 = LuaType::Vector as u8;
pub(crate) const TSTRING: u8 = LuaType::String as u8;
pub(crate) const TTABLE: u8 = LuaType::Table as u8;
pub(crate) const TFUNCTION: u8 = LuaType::Function as u8;
pub(crate) const TUSERDATA: u8 = LuaType::UserData as u8;
...
fn ir_builder_control_flow_cmp_int() {
  ...
  fn compare_fold(
    ...
    fix.with_two_blocks(|b, a, bb| {
      let c = b.cond(cond);
      inst_op = b.inst_ir_cmd_ir_op_ir_op_ir_op_ir_op_ir_op(IrCmd::JumpCmpInt, lhs, rhs, c, a, bb);
      expected_target = if result { a } else { bb };
    });
...
// Source: `tests/IrBuilder.test.cpp`
// "DeadPredecessorDoesNotPreventTagPropagation" (line 4333).
// The C++ `LuauCodegenSkipDeadPredecessorTags` flag now lives in
// `ulua_common::fflag` (registered for `--fflags=`/batch enable parity).
#[test]
fn ir_builder_dead_predecessor_does_not_prevent_tag_propagation() {
  use ulua_common::fflag::LuauCodegenSkipDeadPredecessorTags;

  let _luau_codegen_skip_dead_predecessor_tags =
    ScopedFastFlag::new(&LuauCodegenSkipDeadPredecessorTags, true);

  let mut fix = IrBuilderFixture::new();
  {
    let b = &mut fix.build;
    let entry = b.block(IrBlockKind::Internal);
    let dead_block = b.block(IrBlockKind::Internal);
    let live_block = b.block(IrBlockKind::Internal);
    let join_block = b.block(IrBlockKind::Internal);
    let other_block = b.block(IrBlockKind::Internal);
    ...
    let tstring = b.const_tag(LuaType::String as u8);
    b.inst_ir_cmd_ir_op_ir_op_ir_op_ir_op(IrCmd::JumpEqTag, tag1, tstring, dead_block, live_block);

    // Dead block: never reached, but stores conflicting info to R0 tag
    b.begin_block(dead_block);
    let r0 = b.vm_reg(0);
    let tstring = b.const_tag(LuaType::String as u8);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreTag, r0, tstring);
    b.inst_ir_cmd_ir_op(IrCmd::JUMP, join_block);

    // Live block: stores same tag as entry to R0
    b.begin_block(live_block);
    let r0 = b.vm_reg(0);
    let tnumber = b.const_tag(LuaType::Number as u8);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreTag, r0, tnumber);
    let r2 = b.vm_reg(2);
    ...
    let tstring = b.const_tag(LuaType::String as u8);
    b.inst_ir_cmd_ir_op_ir_op_ir_op_ir_op(IrCmd::JumpEqTag, tag2, tstring, join_block, other_block);

    // Join block: there's only one live predecessor, so tag remaains a number and check can be removed
    b.begin_block(join_block);
    let r0 = b.vm_reg(0);
    let tag = b.inst_ir_cmd_ir_op(IrCmd::LoadTag, r0);
    let tnumber = b.const_tag(LuaType::Number as u8);
    let exit = b.vm_exit(0);
    b.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::CheckTag, tag, tnumber, exit);
    let r0 = b.vm_reg(0);
    let one = b.const_int(1);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::RETURN, r0, one);

    b.begin_block(other_block);
    let r0 = b.vm_reg(0);
    let one = b.const_int(1);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::RETURN, r0, one);
  }
...
mod ir_builder_dominance_verification_4_ir_builder_test_case_2 {
  //! Source: `tests/IrBuilder.test.cpp`
  use super::*;
...
// Source: `tests/IrBuilder.test.cpp` "DoubleContractionDeduplication" (line 3583).
#[test]
fn ir_builder_double_contraction_deduplication() {
  use ulua_common::fflag::LuauCodegenSubstituteReplacements;

  let _luau_codegen_substitute_replacements =
    ScopedFastFlag::new(&LuauCodegenSubstituteReplacements, true);
  ...
  let mut fix = IrBuilderFixture::new();
  {
    ...
    let two = b.const_double(2.0);
    let mul_num = b.inst_ir_cmd_ir_op_ir_op(IrCmd::MulNum, value, two);
    let r1 = b.vm_reg(1);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r1, mul_num);
    let two = b.const_double(2.0);
    let mul_num = b.inst_ir_cmd_ir_op_ir_op(IrCmd::MulNum, value, two);
    let r2 = b.vm_reg(2);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r2, mul_num);

    let neg_one = b.const_double(-1.0);
    let mul_num = b.inst_ir_cmd_ir_op_ir_op(IrCmd::MulNum, value, neg_one);
    let r3 = b.vm_reg(3);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r3, mul_num);
    let neg_one = b.const_double(-1.0);
    let mul_num = b.inst_ir_cmd_ir_op_ir_op(IrCmd::MulNum, value, neg_one);
    let r4 = b.vm_reg(4);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r4, mul_num);
    let neg_one = b.const_double(-1.0);
    let div_num = b.inst_ir_cmd_ir_op_ir_op(IrCmd::DivNum, value, neg_one);
    let r5 = b.vm_reg(5);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r5, div_num);

    let sixteen = b.const_double(16.0);
    let div_num = b.inst_ir_cmd_ir_op_ir_op(IrCmd::DivNum, value, sixteen);
    let r6 = b.vm_reg(6);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r6, div_num);
    let sixteen = b.const_double(16.0);
    let div_num = b.inst_ir_cmd_ir_op_ir_op(IrCmd::DivNum, value, sixteen);
    let r7 = b.vm_reg(7);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r7, div_num);

    let r1 = b.vm_reg(1);
    let seven = b.const_int(7);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::RETURN, r1, seven);
  }
...
// Source: `tests/IrBuilder.test.cpp`
// "DseVmExitSyncMultiUseChainedDependency1" (line 8622).
#[test]
fn ir_builder_dse_vm_exit_sync_multi_use_chained_dependency_1() {
  ...
  let mut fix = IrBuilderFixture::new();
  {
    ...
    let load = b.inst_ir_cmd_ir_op(IrCmd::LoadDouble, r0);
    let one = b.const_double(1.0);
    let x = b.inst_ir_cmd_ir_op_ir_op(IrCmd::AddNum, load, one);
    let two = b.const_double(2.0);
    let a = b.inst_ir_cmd_ir_op_ir_op(IrCmd::MulNum, x, two);

    let r1 = b.vm_reg(1);
    let tnumber = b.const_tag(LuaType::Number as u8);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreTag, r1, tnumber);
    let r1 = b.vm_reg(1);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r1, a);

    let r2 = b.vm_reg(2);
    let tnumber = b.const_tag(LuaType::Number as u8);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreTag, r2, tnumber);
    let r2 = b.vm_reg(2);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r2, x);
...
// Source: `tests/IrBuilder.test.cpp`
// "DseVmExitSyncMultiUseChainedDependency2" (line 8671).
#[test]
fn ir_builder_dse_vm_exit_sync_multi_use_chained_dependency_2() {
  let _luau_codegen_vm_exit_sync_multi_use =
    ScopedFastFlag::new(&fflag::LuauCodegenVmExitSyncMultiUse, true);
  ...
  let mut fix = IrBuilderFixture::new();
  {
    ...
    let x = b.inst_ir_cmd_ir_op_ir_op(IrCmd::AddNum, load, one);
    let three = b.const_double(3.0);
    let y = b.inst_ir_cmd_ir_op_ir_op(IrCmd::AddNum, x, three);
    let five = b.const_double(5.0);
    let z = b.inst_ir_cmd_ir_op_ir_op(IrCmd::AddNum, y, five);

    let r1 = b.vm_reg(1);
    let tnumber = b.const_tag(LuaType::Number as u8);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreTag, r1, tnumber);
    let r1 = b.vm_reg(1);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r1, z);
    let r2 = b.vm_reg(2);
    let tnumber = b.const_tag(LuaType::Number as u8);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreTag, r2, tnumber);
    let r2 = b.vm_reg(2);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r2, y);
    let r3 = b.vm_reg(3);
    let tnumber = b.const_tag(LuaType::Number as u8);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreTag, r3, tnumber);
    let r3 = b.vm_reg(3);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r3, x);
...
// Source: `tests/IrBuilder.test.cpp` "DseVmExitSyncMultiUseSink" (line 8570).
#[test]
fn ir_builder_dse_vm_exit_sync_multi_use_sink() {
  let _luau_codegen_vm_exit_sync_multi_use =
    ScopedFastFlag::new(&fflag::LuauCodegenVmExitSyncMultiUse, true);
  ...
  let mut fix = IrBuilderFixture::new();
  {
    ...
    let tag = b.inst_ir_cmd_ir_op(IrCmd::LoadTag, r2);
    let tnumber = b.const_tag(LuaType::Number as u8);
    let exit1 = b.vm_exit(1);
    b.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::CheckTag, tag, tnumber, exit1);
    let r3 = b.vm_reg(3);
    let tag = b.inst_ir_cmd_ir_op(IrCmd::LoadTag, r3);
    let tnumber = b.const_tag(LuaType::Number as u8);
    let exit2 = b.vm_exit(2);
    b.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::CheckTag, tag, tnumber, exit2);
...
// Source: `tests/IrBuilder.test.cpp`
// "EntryBlockBackEdgeTagPropagation" (line 4909).
// 入口块有隐式的函数入口边，该时刻尚无任何 tag 信息，不得从回边前驱（bb_5）传播 tag；
// 否则 createLinearBlocks 为入口块派生的线性块会把 CHECK_TAG 错误折叠
// （守卫出处：cpp CodeGen/src/IrUtils.cpp:1884）。
#[test]
fn ir_builder_entry_block_back_edge_tag_propagation() {
  use ulua_common::fflag::LuauCodegenPropagateFallbackTags;

  let _luau_codegen_propagate_fallback_tags =
    ScopedFastFlag::new(&LuauCodegenPropagateFallbackTags, true);
  ...
  let mut fix = IrBuilderFixture::new();
  {
    ...
    let target = b.block(IrBlockKind::Internal);
    let fallback = b.fallback_block(0);
    let cont1 = b.block(IrBlockKind::Internal);
    let cont2 = b.block(IrBlockKind::Internal);
    let cont3 = b.block(IrBlockKind::Internal);

    b.begin_block(entry);
    let r2 = b.vm_reg(2);
    let tnumber = b.const_tag(TNUMBER);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreTag, r2, tnumber);
    b.inst_ir_cmd_ir_op(IrCmd::JUMP, target);

    b.begin_block(target);
    let r0 = b.vm_reg(0);
    let tag0 = b.inst_ir_cmd_ir_op(IrCmd::LoadTag, r0);
    let ttable = b.const_tag(TTABLE);
    let exit = b.vm_exit(0);
    b.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::CheckTag, tag0, ttable, exit);
    let r1 = b.vm_reg(1);
    let tag1 = b.inst_ir_cmd_ir_op(IrCmd::LoadTag, r1);
    let tnumber = b.const_tag(TNUMBER);
    // Fallback edge to trigger linear block creation
    b.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::CheckTag, tag1, tnumber, fallback);
    let r0 = b.vm_reg(0);
    let ptr = b.inst_ir_cmd_ir_op(IrCmd::LoadPointer, r0);
    let r3 = b.vm_reg(3);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StorePointer, r3, ptr);
    b.inst_ir_cmd_ir_op(IrCmd::JUMP, cont1);
    ...
    let r4 = b.vm_reg(4);
    let tnumber = b.const_tag(TNUMBER);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreTag, r4, tnumber);
    b.inst_ir_cmd_ir_op(IrCmd::JUMP, cont2);

    b.begin_block(cont2);
    let r5 = b.vm_reg(5);
    let two = b.const_double(2.0);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r5, two);
    let r5 = b.vm_reg(5);
    let tnumber = b.const_tag(TNUMBER);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreTag, r5, tnumber);
    b.inst_ir_cmd_ir_op(IrCmd::JUMP, cont3);

    b.begin_block(cont3);
    let r6 = b.vm_reg(6);
    let three = b.const_double(3.0);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r6, three);
    let r6 = b.vm_reg(6);
    let tnumber = b.const_tag(TNUMBER);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreTag, r6, tnumber);
    // Jump to entry
...
// Source: `tests/IrBuilder.test.cpp`
// "FallbackClobberDoesNotPropagateOverwrittenTag" (line 4157).
// C++ gates on `LuauCodegenPropagateFallbackTags`, absent from the Rust
// `FFlag` set (behavior is unconditional there).
#[test]
fn ir_builder_fallback_clobber_does_not_propagate_overwritten_tag() {
  let mut fix = IrBuilderFixture::new();
  {
    let b = &mut fix.build;
    let entry = b.block(IrBlockKind::Internal);
    let fallback = b.fallback_block(0);
    let merge = b.block(IrBlockKind::Internal);
    ...
    b.inst_ir_cmd_ir_op(IrCmd::JUMP, merge);
...
// Source: `tests/IrBuilder.test.cpp`
// "FallbackMultipleBranchPointsFromSameBlockIntersect" (line 4211).
// C++ gates on `LuauCodegenPropagateFallbackTags`, absent from the Rust
// `FFlag` set (behavior is unconditional there).
#[test]
fn ir_builder_fallback_multiple_branch_points_from_same_block_intersect() {
  let mut fix = IrBuilderFixture::new();
  {
    let b = &mut fix.build;
    let entry = b.block(IrBlockKind::Internal);
    ...
    let r2 = b.vm_reg(2);
    let three = b.const_double(3.0);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r2, three);
    b.inst_ir_cmd_ir_op(IrCmd::JUMP, merge);
    ...
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreTag, r2, tnumber);
    let r2 = b.vm_reg(2);
    let seven = b.const_double(7.0);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r2, seven);
    b.inst_ir_cmd_ir_op(IrCmd::JUMP, merge);
...
// Source: `tests/IrBuilder.test.cpp`
// "FallbackReachedByFoldedUnconditionalJump" (line 4275).
#[test]
fn ir_builder_fallback_reached_by_folded_unconditional_jump() {
  use ulua_common::fflag::LuauCodegenPropagateFallbackTags;
  ...
  let mut fix = IrBuilderFixture::new();
  {
    ...
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreTag, r2, tnumber);
    let r2 = b.vm_reg(2);
    let answer = b.const_double(42.0);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r2, answer);
    b.inst_ir_cmd_ir_op(IrCmd::JUMP, merge);
  ...
  compute_cfg_block_edges(&mut fix.build.function); // Refresh CFG info after optimizations
...
// Source: `tests/IrBuilder.test.cpp` "FloatContractionDeduplication" (line 3625).
#[test]
fn ir_builder_float_contraction_deduplication() {
  use ulua_common::fflag::LuauCodegenSubstituteReplacements;
  ...
  let mut fix = IrBuilderFixture::new();
  {
    ...
    let value = b.inst_ir_cmd_ir_op(IrCmd::NumToFloat, dbl);

    let two = b.const_double(2.0);
    let mul_float = b.inst_ir_cmd_ir_op_ir_op(IrCmd::MulFloat, value, two);
    let float_to_num = b.inst_ir_cmd_ir_op(IrCmd::FloatToNum, mul_float);
    let r1 = b.vm_reg(1);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r1, float_to_num);
    let two = b.const_double(2.0);
    let mul_float = b.inst_ir_cmd_ir_op_ir_op(IrCmd::MulFloat, value, two);
    let float_to_num = b.inst_ir_cmd_ir_op(IrCmd::FloatToNum, mul_float);
    let r2 = b.vm_reg(2);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r2, float_to_num);

    let neg_one = b.const_double(-1.0);
    let mul_float = b.inst_ir_cmd_ir_op_ir_op(IrCmd::MulFloat, value, neg_one);
    let float_to_num = b.inst_ir_cmd_ir_op(IrCmd::FloatToNum, mul_float);
    let r3 = b.vm_reg(3);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r3, float_to_num);
    let neg_one = b.const_double(-1.0);
    let div_float = b.inst_ir_cmd_ir_op_ir_op(IrCmd::DivFloat, value, neg_one);
    let float_to_num = b.inst_ir_cmd_ir_op(IrCmd::FloatToNum, div_float);
    let r4 = b.vm_reg(4);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r4, float_to_num);

    let sixteen = b.const_double(16.0);
    let div_float = b.inst_ir_cmd_ir_op_ir_op(IrCmd::DivFloat, value, sixteen);
    let float_to_num = b.inst_ir_cmd_ir_op(IrCmd::FloatToNum, div_float);
    let r5 = b.vm_reg(5);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r5, float_to_num);
    let sixteen = b.const_double(16.0);
    let div_float = b.inst_ir_cmd_ir_op_ir_op(IrCmd::DivFloat, value, sixteen);
    let float_to_num = b.inst_ir_cmd_ir_op(IrCmd::FloatToNum, div_float);
    let r6 = b.vm_reg(6);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r6, float_to_num);

    let r1 = b.vm_reg(1);
    let six = b.const_int(6);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::RETURN, r1, six);
  }
...
// Source: `tests/IrBuilder.test.cpp` "FloatSimplifications" (line 3527).
#[test]
fn ir_builder_float_simplifications() {
  use ulua_common::fflag::LuauCodegenSubstituteReplacements;
  ...
  let mut fix = IrBuilderFixture::new();
  {
    ...
    let zero = b.const_double(0.0);
    let sub_float = b.inst_ir_cmd_ir_op_ir_op(IrCmd::SubFloat, value, zero);
    let float_to_num = b.inst_ir_cmd_ir_op(IrCmd::FloatToNum, sub_float);
    let r1 = b.vm_reg(1);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r1, float_to_num);
    let neg_zero = b.const_double(-0.0);
    let add_float = b.inst_ir_cmd_ir_op_ir_op(IrCmd::AddFloat, value, neg_zero);
    let float_to_num = b.inst_ir_cmd_ir_op(IrCmd::FloatToNum, add_float);
    let r2 = b.vm_reg(2);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r2, float_to_num);

    let one = b.const_double(1.0);
    let mul_float = b.inst_ir_cmd_ir_op_ir_op(IrCmd::MulFloat, value, one);
    let float_to_num = b.inst_ir_cmd_ir_op(IrCmd::FloatToNum, mul_float);
    let r3 = b.vm_reg(3);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r3, float_to_num);
    let two = b.const_double(2.0);
    ...
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r5, float_to_num);
    let three = b.const_double(3.0);
    let mul_float = b.inst_ir_cmd_ir_op_ir_op(IrCmd::MulFloat, value, three);
    let float_to_num = b.inst_ir_cmd_ir_op(IrCmd::FloatToNum, mul_float);
    let r6 = b.vm_reg(6);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r6, float_to_num);

    let one = b.const_double(1.0);
    let div_float = b.inst_ir_cmd_ir_op_ir_op(IrCmd::DivFloat, value, one);
    let float_to_num = b.inst_ir_cmd_ir_op(IrCmd::FloatToNum, div_float);
    let r7 = b.vm_reg(7);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r7, float_to_num);
    let neg_one = b.const_double(-1.0);
    let div_float = b.inst_ir_cmd_ir_op_ir_op(IrCmd::DivFloat, value, neg_one);
    let float_to_num = b.inst_ir_cmd_ir_op(IrCmd::FloatToNum, div_float);
    let r8 = b.vm_reg(8);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r8, float_to_num);
    let thirty_two = b.const_double(32.0);
    let div_float = b.inst_ir_cmd_ir_op_ir_op(IrCmd::DivFloat, value, thirty_two);
    let float_to_num = b.inst_ir_cmd_ir_op(IrCmd::FloatToNum, div_float);
    let r9 = b.vm_reg(9);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r9, float_to_num);
    let six = b.const_double(6.0);
    let div_float = b.inst_ir_cmd_ir_op_ir_op(IrCmd::DivFloat, value, six);
    let float_to_num = b.inst_ir_cmd_ir_op(IrCmd::FloatToNum, div_float);
    let r10 = b.vm_reg(10);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r10, float_to_num);
...
// Source: `tests/IrBuilder.test.cpp` "IntegerConversionDeduplication" (line 3670).
#[test]
fn ir_builder_integer_conversion_deduplication() {
  use ulua_common::fflag::LuauCodegenSubstituteReplacements;
  ...
  let mut fix = IrBuilderFixture::new();
  {
    ...
    let r0 = b.vm_reg(0);
    let dbl = b.inst_ir_cmd_ir_op(IrCmd::LoadDouble, r0);
    let uint_inst = b.inst_ir_cmd_ir_op(IrCmd::NumToUint, dbl); // produces dirty high register bits
    let roundtripped = b.inst_ir_cmd_ir_op(IrCmd::UintToNum, uint_inst);

    let num_to_int = b.inst_ir_cmd_ir_op(IrCmd::NumToInt, roundtripped);
    let int_to_num = b.inst_ir_cmd_ir_op(IrCmd::IntToNum, num_to_int);
    let r1 = b.vm_reg(1);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r1, int_to_num);
    let num_to_int = b.inst_ir_cmd_ir_op(IrCmd::NumToInt, roundtripped);
    let int_to_num = b.inst_ir_cmd_ir_op(IrCmd::IntToNum, num_to_int);
    let r2 = b.vm_reg(2);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r2, int_to_num);
    let num_to_uint = b.inst_ir_cmd_ir_op(IrCmd::NumToUint, roundtripped);
    let r3 = b.vm_reg(3);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreInt, r3, num_to_uint);

    let r4 = b.vm_reg(4);
    let a = b.inst_ir_cmd_ir_op(IrCmd::LoadInt, r4);
    let r5 = b.vm_reg(5);
    let b_value = b.inst_ir_cmd_ir_op(IrCmd::LoadInt, r5);
    let uint_to_num_a = b.inst_ir_cmd_ir_op(IrCmd::UintToNum, a);
    let uint_to_num_b = b.inst_ir_cmd_ir_op(IrCmd::UintToNum, b_value);
    let sum = b.inst_ir_cmd_ir_op_ir_op(IrCmd::AddNum, uint_to_num_a, uint_to_num_b);
    let num_to_uint = b.inst_ir_cmd_ir_op(IrCmd::NumToUint, sum);
    let r6 = b.vm_reg(6);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreInt, r6, num_to_uint);
    let num_to_uint = b.inst_ir_cmd_ir_op(IrCmd::NumToUint, sum);
    let r7 = b.vm_reg(7);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreInt, r7, num_to_uint);

    let ten = b.const_double(10.0);
    let uint_to_num_a = b.inst_ir_cmd_ir_op(IrCmd::UintToNum, a);
    let diff = b.inst_ir_cmd_ir_op_ir_op(IrCmd::SubNum, uint_to_num_a, ten);
    let num_to_uint = b.inst_ir_cmd_ir_op(IrCmd::NumToUint, diff);
    let r8 = b.vm_reg(8);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreInt, r8, num_to_uint);
    let num_to_uint = b.inst_ir_cmd_ir_op(IrCmd::NumToUint, diff);
    let r9 = b.vm_reg(9);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreInt, r9, num_to_uint);

    let r10 = b.vm_reg(10);
    let c = b.inst_ir_cmd_ir_op(IrCmd::LoadInt, r10);
    let nc = b.inst_ir_cmd_ir_op(IrCmd::UintToNum, c);
    let num_to_float = b.inst_ir_cmd_ir_op(IrCmd::NumToFloat, nc);
    let float_to_num = b.inst_ir_cmd_ir_op(IrCmd::FloatToNum, num_to_float);
    let r11 = b.vm_reg(11);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r11, float_to_num);
    let num_to_float = b.inst_ir_cmd_ir_op(IrCmd::NumToFloat, nc);
    let float_to_num = b.inst_ir_cmd_ir_op(IrCmd::FloatToNum, num_to_float);
    let r12 = b.vm_reg(12);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r12, float_to_num);

    let r1 = b.vm_reg(1);
    let twelve = b.const_int(12);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::RETURN, r1, twelve);
  }
...
// Source: `tests/IrBuilder.test.cpp`
// "LoadOriginNoRedirectAfterCapturedMutation" (line 8786).
#[test]
fn ir_builder_load_origin_no_redirect_after_captured_mutation() {
  let mut fix = IrBuilderFixture::new();
  {
    let b = &mut fix.build;
    let entry = b.block(IrBlockKind::Internal);
    ...
    let r1 = b.vm_reg(1);
    let one = b.const_uint(1);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::CAPTURE, r1, one);

    let r1 = b.vm_reg(1);
    let val1 = b.inst_ir_cmd_ir_op(IrCmd::LoadTvalue, r1);
    let r7 = b.vm_reg(7);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreTvalue, r7, val1);
    ...
    let r7 = b.vm_reg(7);
    let val7 = b.inst_ir_cmd_ir_op(IrCmd::LoadTvalue, r7);
    let r6 = b.vm_reg(6);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreTvalue, r6, val7);

    let r6 = b.vm_reg(6);
    let result = b.inst_ir_cmd_ir_op(IrCmd::LoadDouble, r6);

    let r8 = b.vm_reg(8);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r8, result);
    let r8 = b.vm_reg(8);
    let tnumber = b.const_tag(LuaType::Number as u8);
    ...
    let r8 = b.vm_reg(8);
    let one = b.const_int(1);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::RETURN, r8, one);
  }
...
// Source: `tests/IrBuilder.test.cpp` "NoLinearExtractionForBlockWithCall" (line 4588).
#[test]
fn ir_builder_no_linear_extraction_for_block_with_call() {
  let mut fix = IrBuilderFixture::new();
  {
    let b = &mut fix.build;
    let block1 = b.block(IrBlockKind::Internal);
    ...
    let block5 = b.block(IrBlockKind::Internal);
    ...
    let one_params = b.const_int(1);
    let one_results = b.const_int(1);
    b.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::CALL, r0, one_params, one_results);
    b.inst_ir_cmd_ir_op(IrCmd::JUMP, block3);

    b.begin_block(block3);
    let r2 = b.vm_reg(2);
    let tag3 = b.inst_ir_cmd_ir_op(IrCmd::LoadTag, r2);
    let tnumber = b.const_tag(LuaType::Number as u8);
    b.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::CheckTag, tag3, tnumber, fallback2);
    ...
    b.inst_ir_cmd_ir_op(IrCmd::JUMP, block5);

    b.begin_block(block5);
    let r0 = b.vm_reg(0);
    let zero = b.const_int(0);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::RETURN, r0, zero);
  }
...
// Source: `tests/IrBuilder.test.cpp` "NoLinearExtractionForChainWithCall" (line 4664).
#[test]
fn ir_builder_no_linear_extraction_for_chain_with_call() {
  let mut fix = IrBuilderFixture::new();
  {
    let b = &mut fix.build;
    let block1 = b.block(IrBlockKind::Internal);
...
// Source: `tests/IrBuilder.test.cpp`
// "NoLinearExtractionForChainWithCallLiveOut" (line 4818).
#[test]
fn ir_builder_no_linear_extraction_for_chain_with_call_live_out() {
  let mut fix = IrBuilderFixture::new();
  {
    let b = &mut fix.build;
    let block_start = b.block(IrBlockKind::Internal);
    let fallback_start = b.fallback_block(0);
    let target1 = b.block(IrBlockKind::Internal);
    let fallback1 = b.fallback_block(0);
    let target2 = b.block(IrBlockKind::Internal);
    let fallback2 = b.fallback_block(0);
    let target3 = b.block(IrBlockKind::Internal);
    let call_block = b.block(IrBlockKind::Internal);
    let exit_block = b.block(IrBlockKind::Internal);

    b.begin_block(block_start);
    let r2 = b.vm_reg(2);
    let tag0 = b.inst_ir_cmd_ir_op(IrCmd::LoadTag, r2);
    let tnumber = b.const_tag(LuaType::Number as u8);
    b.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::CheckTag, tag0, tnumber, fallback_start);
    b.inst_ir_cmd_ir_op(IrCmd::JUMP, target1);

    b.begin_block(fallback_start);
    let r1 = b.vm_reg(1);
    let r2 = b.vm_reg(2);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::DoLen, r1, r2);
    b.inst_ir_cmd_ir_op(IrCmd::JUMP, target1);

    b.begin_block(target1);
    let r2 = b.vm_reg(2);
    let tag1 = b.inst_ir_cmd_ir_op(IrCmd::LoadTag, r2);
    let tnumber = b.const_tag(LuaType::Number as u8);
    b.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::CheckTag, tag1, tnumber, fallback1);
    b.inst_ir_cmd_ir_op(IrCmd::JUMP, target2);
    ...
    b.begin_block(target2);
    let r2 = b.vm_reg(2);
    let tag2 = b.inst_ir_cmd_ir_op(IrCmd::LoadTag, r2);
    let tnumber = b.const_tag(LuaType::Number as u8);
    b.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::CheckTag, tag2, tnumber, fallback2);
    b.inst_ir_cmd_ir_op(IrCmd::JUMP, target3);
    ...
    b.begin_block(target3);
    let r4 = b.vm_reg(4);
    let val = b.inst_ir_cmd_ir_op(IrCmd::LoadDouble, r4);
    let r5 = b.vm_reg(5);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r5, val);
    b.inst_ir_cmd_ir_op(IrCmd::JUMP, call_block);

    b.begin_block(call_block);
    let r6 = b.vm_reg(6);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r6, val);
    let r0 = b.vm_reg(0);
    let one_params = b.const_int(1);
    let one_results = b.const_int(1);
    b.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::CALL, r0, one_params, one_results);
    b.inst_ir_cmd_ir_op(IrCmd::JUMP, exit_block);

    b.begin_block(exit_block);
    let r0 = b.vm_reg(0);
    let zero = b.const_int(0);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::RETURN, r0, zero);
  }
...
fn ir_builder_recursive_scc_use_removal_2() {
  let mut fix = IrBuilderFixture::new();
  {
    ...
    let one = b.const_int(1);
    let equal = b.cond(IrCondition::Equal);
    b.inst_ir_cmd_ir_op_ir_op_ir_op_ir_op_ir_op(IrCmd::JumpCmpInt, zero, one, equal, block, exit1);
...
fn ir_builder_select_if_truthy() {
  let mut fix = IrBuilderFixture::new();
  {
    ...
    let op =
      b.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::SelectIfTruthy, unknown_tv1, unknown_tv2, unknown_tv2);
    let r = b.vm_reg(0);
...
fn ir_builder_tag_eq_removal() {
  let mut fix = IrBuilderFixture::new();
  {
    ...
    let tnumber = b.const_tag(TNUMBER);
    b.inst_ir_cmd_ir_op_ir_op_ir_op_ir_op(IrCmd::JumpEqTag, tag, tnumber, true_block, false_block);
...
// Source: `tests/IrBuilder.test.cpp` "TagsAreJoinedFromFallbackPredecessor" (line 4040).
#[test]
fn ir_builder_tags_are_joined_from_fallback_predecessor() {
  use ulua_common::fflag::LuauCodegenPropagateFallbackTags;
  ...
  let mut fix = IrBuilderFixture::new();
  {
    ...
    let tag = b.inst_ir_cmd_ir_op(IrCmd::LoadTag, r1);
    let tnumber = b.const_tag(LuaType::Number as u8);
    let exit_op = b.vm_exit(1);
    b.inst_ir_cmd_ir_op_ir_op_ir_op(IrCmd::CheckTag, tag, tnumber, exit_op);
    let r1 = b.vm_reg(1);
    let one = b.const_int(1);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::RETURN, r1, one);
  }
...
// Source: `tests/IrBuilder.test.cpp`
// "TagsEstablishedBeforeFallbackBranchPropagateThroughMerge" (line 4095).
#[test]
fn ir_builder_tags_established_before_fallback_branch_propagate_through_merge() {
  use ulua_common::fflag::LuauCodegenPropagateFallbackTags;
  ...
  let mut fix = IrBuilderFixture::new();
  {
    ...
    let r3 = b.vm_reg(3);
    let answer = b.const_double(42.0);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r3, answer);
    b.inst_ir_cmd_ir_op(IrCmd::JUMP, merge);
    ...
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreTag, r3, tnumber);
    let r3 = b.vm_reg(3);
    let ninety_nine = b.const_double(99.0);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::StoreDouble, r3, ninety_nine);
    b.inst_ir_cmd_ir_op(IrCmd::JUMP, merge);
    ...
    let r3 = b.vm_reg(3);
    let one = b.const_int(1);
    b.inst_ir_cmd_ir_op_ir_op(IrCmd::RETURN, r3, one);
  }
...
fn ir_builder_userdata_buffer_store_forwarding_invalidation() {
  let mut fix = IrBuilderFixture::new();
  {
    ...
    let tuserdata = b.const_tag(TUSERDATA);
    b.inst_ir_cmd_ir_op_ir_op_ir_op_ir_op(IrCmd::BufferWritei32, ud, four, ninety_nine, tuserdata);
