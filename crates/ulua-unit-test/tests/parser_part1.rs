...
mod parser_class_declaration {
  ...
  fn parser_class_declaration() {
    use ulua_ast::records::{
      ...
      ast_expr_local::AstExprLocal, ast_stat_class::AstStatClass, ast_stat_expr::AstStatExpr,
    ...
    // cpp 比对实参名与类名（本移植把类名绑定为局部，见 parseClassStat::push_local）。
    let local = as_node_at::<AstExprLocal, _>(&call.args, 0).expect("args[0] 应为 AstExprLocal");
    assert_eq!(
      local
        .local
        .as_ref_opt()
        .expect("实参 AstLocal 必须存在")
        .name,
      first_name.name
    );
  }
...
mod parser_complex_union_in_generic_ty {
  ...
  fn parser_complex_union_in_generic_ty() {
    ...
    for (i, &expected) in expected_types.iter().enumerate() {
      let ty_ref =
        as_node_at::<AstTypeReference, _>(&union_ty.types, i).expect("types[i] 应为引用");
