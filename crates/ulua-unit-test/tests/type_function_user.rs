...
// 405 处 test fn 体内逐例重复的 use 统一上提至此（借 tst-r16 pretty_printer/fragment_autocomplete 上提先例；源头 cpp 侧即为整文件共享的 using 声明）。
...
local y: keyof<typeof(x)>
  ...
  assert_eq!(
    ...
    to_string_type_id_to_string_options(fixture.base.require_type_string("x"), &mut x_opts)
  );

  let mut y_opts = ToStringOptions::new(true);
  assert_eq!(
    r#""a" | "b""#,
    to_string_type_id_to_string_options(fixture.base.require_type_string("y"), &mut y_opts)
  );
}
...
local x: foo<{a: foo<string>, b: foo<number>}> = nil
  ...
  assert_eq!(
    "{ a: string?, b: number? }?",
    to_string_type_id_to_string_options(fixture.base.require_type_string("x"), &mut opts)
  );
}
...
return {}
  ...
  assert_eq!(
    "\"firstsecond\"",
    to_string_type_id(fixture.base.require_type_module_name_string("game/A", "a"))
  );
...
fn type_function_user_metatable_serialization() {
  ...
  assert_eq!(
    "Expected this to be 'number', but got 'setmetatable<{ a: number }, { ma: boolean }>'",
...
return {}
  ...
  assert_eq!(
    "number",
    to_string_type_id(fixture.base.require_type_module_name_string("game/A", "x"))
  );
...
,
  ...
  match &result.errors[0].data {
    TypeErrorData::TypeMismatch(tm) => {
      assert_eq!(
        "setmetatable<{  }, { [number]: boolean, string: number }>",
        to_string_type_id(tm.given_type)
      );
    }
    other => panic!("expected TypeMismatch, got {other:?}"),
...
type bar = identity<foo>
  ...
  let bar_ty = fixture.base.require_type_alias("bar");
  let ftv = get_type::get::<FunctionType>(bar_ty).expect("expected FunctionType");
...
// Source: `tests/TypeFunction.user.test.cpp:3464`
#[test]
fn type_function_user_issubtypeof_top_and_bottom() {
  if fflag::DebugLuauForceOldSolver.get() {
    return;
  }
  let _sff = ScopedFastFlag::new(&fflag::LuauUdtfTypeIsSubtypeOf, true);
...
// Source: `tests/TypeFunction.user.test.cpp:3497`
#[test]
fn type_function_user_issubtypeof_any() {
  if fflag::DebugLuauForceOldSolver.get() {
    return;
  }
  let _sff = ScopedFastFlag::new(&fflag::LuauUdtfTypeIsSubtypeOf, true);
  ...
  assert_eq!(0, result.errors.len(), "{:?}", result.errors);
  for (name, expected) in [("a", "false"), ("b", "true"), ("c", "true"), ("d", "true")] {
    assert_eq!(
      expected,
      to_string_type_id(fixture.base.require_type_string(name)),
      "issub<{name}> 期望 {expected}"
...
// Source: `tests/TypeFunction.user.test.cpp:3521`
#[test]
fn type_function_user_issubtypeof_table_structural() {
  if fflag::DebugLuauForceOldSolver.get() {
    return;
  }
  let _sff = ScopedFastFlag::new(&fflag::LuauUdtfTypeIsSubtypeOf, true);
...
// Source: `tests/TypeFunction.user.test.cpp:3555`
#[test]
fn type_function_user_issubtypeof_function() {
  if fflag::DebugLuauForceOldSolver.get() {
    return;
  }
  let _sff = ScopedFastFlag::new(&fflag::LuauUdtfTypeIsSubtypeOf, true);
...
// Source: `tests/TypeFunction.user.test.cpp:3585`
#[test]
fn type_function_user_issubtypeof_union() {
  if fflag::DebugLuauForceOldSolver.get() {
    return;
  }
  let _sff = ScopedFastFlag::new(&fflag::LuauUdtfTypeIsSubtypeOf, true);
  ...
  assert_eq!(0, result.errors.len(), "{:?}", result.errors);
  for (name, expected) in [("a", "true"), ("b", "true"), ("c", "false"), ("d", "true")] {
    assert_eq!(
      expected,
      to_string_type_id(fixture.base.require_type_string(name)),
      "issub<{name}> 期望 {expected}"
...
// Source: `tests/TypeFunction.user.test.cpp:3608`
#[test]
fn type_function_user_issubtypeof_intersection() {
  if fflag::DebugLuauForceOldSolver.get() {
    return;
  }
  let _sff = ScopedFastFlag::new(&fflag::LuauUdtfTypeIsSubtypeOf, true);
  ...
  assert_eq!(0, result.errors.len(), "{:?}", result.errors);
  for (name, expected) in [("a", "true"), ("b", "true"), ("c", "false")] {
    assert_eq!(
      expected,
      to_string_type_id(fixture.base.require_type_string(name)),
      "issub<{name}> 期望 {expected}"
...
//   `LuauNewTypePathErrorMessages` 新报错渲染（同 type_path.rs 阻清单）。
// - explicit_export_zero_arg（:1678）、generic_type_function_property_access_
//   across_modules（:1704）、generic_type_function_property_access_across_
//   modules_2（:1752）——依赖 FFlag `LuauCloneTypeFunctionFromForeignArena`/
//   `LuauClonePublicInterfaceRetainTypeFunctionSolvedStatus`（均未同步）。
// - error_handling_pcall（:3726）、cross_type_function_type_check（:3753）——
//   依赖 FFlag `LuauUdtfErrorHandling`/`LuauUdtfPopulateEnv`（均未同步）。
// - error_when_serializing_environment_but_not_arguements（:3790）——依赖
//   FFlag `LuauTypeFunctionsReturnAfterAllSerialized`（未同步）。
// 注：旧账 TypeInfer.TypeFunction=4 严重低估；TypeFunction.user.test.cpp 组
//   实测 cpp:176 / rust 移植前:153，真缺+阻 24 例。
