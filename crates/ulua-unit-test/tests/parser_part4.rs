...
fn parser_parse_class_declarations_unaffected_by_global_flag() {
  ...
  let declared =
    as_node_at::<AstStatDeclareExternType, _>(&stat.body, 0).expect("body[0] 应为 extern 类型声明");
...
fn parser_parse_const() {
  ...
  let value =
    as_node_at::<AstExprConstantNumber, _>(&stat_local.values, 0).expect("values[0] 应为数字常量");
...
fn parser_parse_error_confusing_function_call() {
  ...
  use crate::AMBIGUOUS_ARGUMENT_LIST;
...
fn parser_parse_error_with_too_many_nested_ifelse_expressions1() {
  ...
  use crate::EXPR_RECURSION_MESSAGE;
...
fn parser_parse_error_with_too_many_nested_type_group() {
  ...
  use crate::TYPE_RECURSION_MESSAGE;
...
fn parser_parse_extern_type_declarations() {
  ...
  let subclass =
    as_node_at::<AstStatDeclareExternType, _>(&stat.body, 1).expect("body[1] 应为 extern 类型声明");
...
fn parser_parse_interpolated_string_double_brace_begin() {
  ...
  use crate::DOUBLE_BRACE_MESSAGE;
...
fn parser_parse_interpolated_string_mid_without_end_brace_in_table() {
  ...
  use crate::{MISSING_END_BRACE_MESSAGE, UNCLOSED_TABLE_MESSAGE};
