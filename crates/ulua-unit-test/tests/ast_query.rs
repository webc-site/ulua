...
mod ast_query_luau_nested_query {
  ...
  fn ast_query_luau_nested_query() {
    ...
    let if_ = query::<AstStatIf>(block as *mut _ as *mut AstNode, vec![nth_t::<AstStatIf>(1)]);
    assert!(!if_.is_null());
...
mod ast_query_luau_nested_query_but_first_query_failed {
  ...
  fn ast_query_luau_nested_query_but_first_query_failed() {
    ...
    let if_ = query::<AstStatIf>(block as *mut _ as *mut AstNode, vec![nth_t::<AstStatIf>(2)]);
    assert!(if_.is_null());
