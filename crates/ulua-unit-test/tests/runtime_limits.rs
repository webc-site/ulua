...
fn runtime_limits_native_stack_guard_prevents_stack_overflows() {
  ...
  let _sff_luau_use_native_stack_guard = ScopedFastFlag::new(&fflag::LuauUseNativeStackGuard, true);
...
// Ported from `tests/RuntimeLimits.test.cpp`.
// cpp:293-336 Signal_exerpt 是 doctest::timeout 回归闸：cpp 对结果零断言
// （cpp:335 `(void)result;`），此处 `let _ =` 是等形镜像而非弱断言——
// 任何在此编造 errors 断言都会超出 cpp 承诺。用例能跑到终点即验证了不挂死。
#[test]
...
// Ported from `tests/RuntimeLimits.test.cpp`.
// cpp:368-487 同 Signal_exerpt：超时回归闸，cpp:486 `std::ignore = check(src);`
// 不附断言，下面的 `let _ =` 即其等形，勿按弱断言强化。
#[test]
...
fn runtime_limits_test_generic_pruning_recursion_limit() {
  ...
  let _old_solver = ScopedFastFlag::new(&fflag::DebugLuauForceOldSolver, false);
  // 灭失申报：cpp:494 还有 {LuauIterativeTypeSearcher, false} 的截断旗标，
  // 该 flag 本端口 HEAD 尚未 sync（同 generalization.rs:791 的申报），无法置位。
  ...
  assert!(result.errors.is_empty(), "{:?}", result.errors);
  // cpp:506 锁 "<T>(…)"；本端口 typevar 渲染从 <a> 起名，正文其余部分全等。
  assert_eq!(
    "<a>({ read Do: { read Re: { read Mi: a } } }) -> ()",
    to_string_type_id(fixture.base.require_type_string("get"))
  );
}
...
fn runtime_limits_unification_runs_a_limited_number_of_iterations_before_stopping_subtyping() {
  ...
  let _old_solver = ScopedFastFlag::new(&fflag::DebugLuauForceOldSolver, false);
  // cpp RuntimeLimits.test.cpp:513-516 同窗钉住两个迭代上限，缺一即与 cpp 场景不齐
  let _limit = ScopedFastInt::new(&fint::LuauSubtypingIterationLimit, 100);
  let _iter_limit = ScopedFastInt::new(&fint::LuauTypeInferIterationLimit, 100);
