...
fn subtyping_a_a_number_number_number() {
  ...
  let generic_as_to_number_ty = fixture.generic_pack_fn(vec![generic_as], generic_as, number_pack);
...
fn subtyping_number_string_number_string_string() {
  ...
  let number_to_two_strings_ty = fixture.fn_item_initializer_list_type_id_initializer_list_type_id(
    vec![number_ty],
...
fn subtyping_weird_cyclic_instantiation() {
  ...
  use ulua_analysis::{
    ...
    functions::{
      get_mutable_type, instantiate_2_instantiation_2::instantiate_2,
      to_string_to_string::to_string_type_id,
    },
    records::{
      arena_handle::Handle, free_type::FreeType, function_type::FunctionType,
      generic_type::GenericType, scope::Scope, type_arena::TypeArena,
    },
  ...
  let mut generic_substitutions: DenseHashMap<TypeId, TypeId> = DenseHashMap::default();
  let generic_pack_substitutions: DenseHashMap<TypePackId, TypePackId> = DenseHashMap::default();
...
// 回归测试：Normalizer::normalize 失败语义（try_normalize 在归一化过复杂时返回 None）。
// 对应 cpp `NormalizationTooComplex` / try_semantic_subtyping 的不可判定分支。
#[test]
fn subtyping_normalizer_try_normalize_returns_none_when_fuel_exhausted() {
  use alloc::vec;

  use ulua_common::fint;
  use ulua_unit_test::{
    records::subtype_fixture::SubtypeFixture, type_aliases::scoped_fast_int::ScopedFastInt,
  };

  let _fuel = ScopedFastInt::new(&fint::LuauNormalizerInitialFuel, 2);
  let mut fixture = SubtypeFixture::default();

  let string_ty = fixture.builtin_types.string_type;
  let number_ty = fixture.builtin_types.number_type;

  // 大函数类型联合：归一化需要多次 consume_fuel，2 单位燃料必耗尽
  let mut big_union = fixture
    .fn_item_initializer_list_type_id_initializer_list_type_id(vec![string_ty], vec![number_ty]);
  for _ in 0..8 {
    let next = fixture
      .fn_item_initializer_list_type_id_initializer_list_type_id(vec![string_ty], vec![number_ty]);
    big_union = fixture.join(big_union, next);
  }

  assert!(
    fixture.normalizer.try_normalize(big_union).is_none(),
    "normalization of a large union should report too-complex (None)"
  );
}

// 回归测试：燃料充足时 try_normalize 正常返回 Some（对照组）。
#[test]
fn subtyping_normalizer_try_normalize_succeeds_within_fuel() {
  use ulua_unit_test::records::subtype_fixture::SubtypeFixture;
  ...
  let _norm = fixture
    .normalizer
    .try_normalize(number_ty)
    .expect("number should normalize without hitting limits");

  let string_ty = fixture.builtin_types.string_type;
  assert!(fixture.normalizer.try_normalize(string_ty).is_some());
}

// 回归测试：语义子类型检查在 try_normalize 失败时返回
// `{ is_subtype: false, normalization_too_complex: true }`（cpp NormalizationTooComplex）。
#[test]
fn subtyping_semantic_subtyping_reports_normalization_too_complex() {
  use alloc::vec;
  ...
  // 宽表交叉：归一化需要多次 intersect（consume_fuel），2 单位燃料必耗尽
  let mut big_intersection = fixture.tbl(SubtypeFixture::props(vec![(
    "f0",
    Property::rw_type_id(string_ty),
  )]));
  for i in 1..9 {
    let part = fixture.tbl(SubtypeFixture::props(vec![(
      alloc::format!("f{i}").as_str(),
      Property::rw_type_id(number_ty),
    )]));
    big_intersection = fixture.meet(big_intersection, part);
  }

  let result = fixture.is_subtype_type_id_type_id(big_intersection, string_ty);

  assert!(!result.is_subtype());
  assert!(!result.is_error_suppressing());
  let debug = alloc::format!("{:?}", result);
  assert!(
    debug.contains("normalization_too_complex: true"),
    "expected normalization_too_complex, got {debug}"
  );
}

// 回归测试：归一化失败（燃料耗尽）时 `try_normalize` 返回 None，
// `isInhabited` 路径按 C++ `isInhabited(nullptr, seen)` 的 `!norm` 检查
// 返回 HitLimits（而非把空 norm 误判为 uninhabited/False）。
#[test]
fn subtyping_normalize_failure_reports_hit_limits() {
  use alloc::vec;

  use ulua_analysis::enums::normalization_result::NormalizationResult;
  use ulua_common::fint;
  use ulua_unit_test::{
    records::subtype_fixture::SubtypeFixture, type_aliases::scoped_fast_int::ScopedFastInt,
  };
  ...
  assert!(
    fixture.normalizer.try_normalize(big_union).is_none(),
    "the union should fail try_normalize under exhausted fuel"
  );

  // isInhabited 走 normalize 失败分支须报 HitLimits（C++ nullptr 分支）
  assert_eq!(
    fixture.normalizer.is_inhabited_type_id(big_union),
    NormalizationResult::HitLimits,
    "normalize failure inside isInhabited must surface as HitLimits, not False"
  );
}
