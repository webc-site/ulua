...
mod subtyping_normalize_falls_back_to_empty_normalized_type {
  //! 回归测试：Normalizer::normalize（非 try）在归一化失败时回退到空
  //! NormalizedType（unknown tops）而不是 panic。

  #[test]
  fn subtyping_normalize_falls_back_to_empty_normalized_type() {
    use alloc::vec;
    ...
    // normalize（非 try 版）在失败时回退到空 NormalizedType（全 never 集合），
    // 不再 panic —— 这正是 Normalizer::normalize 失败语义修复的回归点。
    let norm = fixture.normalizer.normalize(big_union);
    assert!(
      !norm.is_unknown(),
      "fallback of a failed normalization is an empty (never) set, not unknown"
    );

    // 对照：同一类型确实无法在当前燃料下归一化
    assert!(
      fixture.normalizer.try_normalize(big_union).is_none(),
      "the same union should still fail try_normalize"
    );
  }
}
