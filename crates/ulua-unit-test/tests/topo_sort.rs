...
mod topo_sort_break_comes_last {
  ...
  fn topo_sort_break_comes_last() {
    use ulua_ast::records::{
      ast_node::AstNode, ast_stat_repeat::AstStatRepeat, parse_options::ParseOptions,
    };
    use ulua_unit_test::{
      functions::{node_as::node_as, toposort::toposort},
...
until true
    ...
    let repeat = node_as::<AstStatRepeat>(program.body.as_slice()[0] as *mut AstNode).unwrap();

    // SAFETY: repeat 指向 arena 存活的 AstStatRepeat，body 为 arena 分配的
    // AstStatBlock 指针（repeat.body 为裸指针字段，无安全 API 可用）；
    // 借用随即传入 toposort，生命周期互不重叠。
