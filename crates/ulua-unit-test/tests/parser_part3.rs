...
fn parser_moved_out_allocator_can_still_be_used() {
  ...
  let _inner = Allocator::move_from(&mut outer);
...
fn parser_grouped_function_type() {
  ...
  let union_ty = param_ty
    .as_type()
    .and_then(|t| t.as_node::<AstTypeUnion>())
...
fn parser_lex_broken_unicode() {
  ...
  let mut lexer = Lexer::new(test_input, &mut names, Position::default());
...
";
  ...
  assert_eq!(1, func1.attributes.len());
...
";
  ...
  assert_eq!(1, func2.attributes.len());
...
";
  ...
  assert_eq!(1, root_block.body.len());
