...
// 缺口（未移植，对照 `tests/TypeInfer.modules.test.cpp`，共 7 例）：
// - cli_194463_modify_bounds_of_visited_generic_regression（:982）——Rust 求解器
//   对 `function Container.Set<K, V>(self: Container<K, V>, ...)`(Container 为
//   `{[K]: V} & typeof(Container)` 交叉别名)报 CannotAssignToExport,与上游新
//   solver 的 visited-generic bounds 行为存在分歧,faithful 断言(0 错误)不可达。
// - exported_module_initializer_type_packs（:1057）/
//   exported_module_initializer_type_packs_multi（:1095）——依赖 FFlag
//   `LuauExportTypecheckTypepacks`，本移植未定义。
// - exported_module_annotation_uses_binding_type（:1417）/
//   exported_module_annotation_preferred_over_initializer（:1459）/
//   exported_module_binding_is_readonly（:1491）/
//   exported_module_annotation_mismatch_errors（:1519）——依赖 FFlag
//   `LuauExportAnnotationBinding`，本移植未定义。
