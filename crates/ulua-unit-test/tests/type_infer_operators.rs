...
fn type_infer_operators_compound_assign_mismatch_result() {
  ...
  let tm0 = type_error_data_ref::<TypeMismatch>(&result.errors[0]).expect("expected TypeMismatch");
...
fn type_infer_operators_disallow_string_and_types_without_metatables_from_arithmetic_binary_ops() {
  ...
  if !fflag::DebugLuauForceOldSolver.get() {
    ...
    assert_eq!(fixture.base.require_type_string("foo"), tm2.given_type);
  }
}
...
fn type_infer_operators_luau_polyfill_array_startswith() {
  ...
  let result = fixture.base.check_string_optional_frontend_options(
          ...
          r#"
--!strict
local function starts_with(value: string, substring: string, position: number?): boolean
...
fn type_infer_operators_primitive_arith_no_metatable() {
  ...
  let function_type = fixture.base.require_type_string("add");
  let function_type = get_type::get::<FunctionType>(follow_type::follow(function_type))
...
fn type_infer_operators_primitive_arith_no_metatable_with_follows() {
  ...
  assert_eq!(0, result.errors.len(), "{:?}", result.errors);
  let number_type = fixture.get_builtins().number_type;
  let solar_mass_type = follow_type::follow(fixture.require_type_string("SOLAR_MASS"));
...
fn type_infer_operators_primitive_arith_possible_metatable() {
  ...
  assert_eq!(0, result.errors.len(), "{:?}", result.errors);
  assert_eq!("any", to_string_type_id(fixture.require_type_string("t")));
}
...
local w = c and 1
  ...
  if !fflag::DebugLuauForceOldSolver.get() {
    assert_eq!(
      "false | number",
      to_string_type_id(fixture.base.require_type_string("z"))
    );
    assert_eq!(
      "number?",
...
local f1 = f or 'f'
  ...
  if !fflag::DebugLuauForceOldSolver.get() {
    assert_eq!(
      "string | true",
      to_string_type_id(fixture.base.require_type_string("c1"))
    );
    assert_eq!(
      "string | true",
      to_string_type_id(fixture.base.require_type_string("d1"))
    );
  } else {
    assert_eq!(
      "boolean | string",
  ...
  assert_eq!(
    "string",
    to_string_type_id(fixture.base.require_type_string("e1"))
  );
  assert_eq!(
    "string",
    to_string_type_id(fixture.base.require_type_string("f1"))
  );
}
