...
"#,
    ...
    assert!(!rhs.is_null());
    assert_eq!("x", unsafe {
      (*(*lhs).local).name.as_c_str().unwrap().to_str().unwrap()
    });
    assert_eq!("y", unsafe {
      (*(*rhs).local).name.as_c_str().unwrap().to_str().unwrap()
    });
  }
}
...
"#,
    ...
    assert_eq!("y", unsafe {
      (*rhs).name.as_c_str().unwrap().to_str().unwrap()
    });
  }
}
...
"#,
    ...
    assert!(
      !result
        .local_map
        .iter()
        .any(|(name, _)| { name.as_c_str().unwrap().to_bytes() == b"self" })
    );
  }
}
...
"#,
    ...
    assert_eq!(
      "self",
      unsafe { (*last).name.as_c_str().unwrap() }
        .to_str()
        .unwrap()
    );
  }
...
"#,
    ...
    assert_eq!(
      "q",
      unsafe { (*var).name.as_c_str().unwrap() }.to_str().unwrap()
    );
  }
}
