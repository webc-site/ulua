...
"#,
  ...
  assert_eq!(1, result.errors.len(), "{:?}", result.errors);
  type_error_data_ref::<OccursCheckFailed>(&result.errors[0]).expect("expected OccursCheckFailed");
}
...
fn type_infer_aliases_corecursive_function_types() {
  ...
  assert_eq!(
    ...
    to_string_type_id(fixture.require_type_alias("A"))
  );
  assert_eq!(
    "t1 where t1 = () -> (string, () -> (number, t1))",
    to_string_type_id(fixture.require_type_alias("B"))
  );
}
...
fn type_infer_aliases_dont_allow_redefining_builtin_types() {
  ...
  let _disallow_redefining = ScopedFastFlag::new(&fflag::LuauDisallowRedefiningBuiltinTypes, true);
  let mut fixture = BuiltinsFixture::default();
  fixture.get_frontend();
  let result = fixture.base.check_string_optional_frontend_options(
    r#"
...
fn type_infer_aliases_dont_lose_track_of_pending_expansion_types_after_substitution() {
  ...
  let result = fixture
    ...
    .check_module_name_optional_frontend_options(
      &ModuleName::from("game/React/React/ReactHooks"),
      None,
    );
  assert_eq!(0, result.errors.len(), "{:?}", result.errors);
}
...
fn type_infer_aliases_forward_declared_alias_is_not_clobbered_by_prior_unification_with_any() {
  ...
  assert_eq!(
    "{ foo: number }",
    to_string_type_id_to_string_options(fixture.require_type_string("d"), &mut opts)
  );
  assert_eq!(1, result.errors.len(), "{:?}", result.errors);
}
...
fn type_infer_aliases_general_require_multi_assign() {
  ...
  let result = fixture
    .get_frontend()
    .check_module_name_optional_frontend_options(&ModuleName::from("workspace/C"), None);
  assert_eq!(0, result.errors.len(), "{:?}", result.errors);

  let a_type_id = fixture
    .base
    .require_type_module_name_string("workspace/C", "a");
  let a_type =
    get_type::get::<TableType>(follow_type::follow(a_type_id)).expect("expected table type for a");
  assert_eq!(2, a_type.props.len());

  let b_type_id = fixture
    .base
    .require_type_module_name_string("workspace/C", "b");
  let b_type =
    get_type::get::<TableType>(follow_type::follow(b_type_id)).expect("expected table type for b");
...
fn type_infer_aliases_names_are_ascribed() {
  ...
  assert_eq!(0, result.errors.len(), "{:?}", result.errors);
  assert_eq!("T", to_string_type_id(fixture.require_type_string("x")));
}
...
fn type_infer_aliases_non_recursive_aliases_that_reuse_a_generic_name() {
  ...
  assert_eq!(
    "{number | string}",
    to_string_type_id_to_string_options(fixture.require_type_string("p"), &mut opts)
  );
}
...
"#,
  ...
  type_error_data_ref::<OccursCheckFailed>(&result.errors[1]).expect("expected OccursCheckFailed");
...
fn type_infer_aliases_report_shadowed_aliases() {
  ...
  let t1 = fixture.lookup_type("MyString").expect("expected MyString");
  assert_eq!(Some(PrimitiveType::STRING), fixture.get_primitive_type(t1));

  let t2 = fixture.lookup_type("string").expect("expected string");
...
fn type_infer_aliases_table_types_record_the_property_locations() {
  ...
  let ttv = get_type::get::<TableType>(follow_type::follow(ty)).expect("expected TableType");
...
fn type_infer_aliases_type_alias_import_mutation() {
  use ulua_analysis::{
    functions::{
      get_global_binding::get_global_binding, get_type, to_string_to_string::to_string_type_id,
    },
    records::table_type::TableType,
  };
  use ulua_unit_test::records::builtins_fixture::BuiltinsFixture;
  ...
  let result = fixture
    .base
    .check_string_optional_frontend_options("type t10<x> = typeof(table)", None);
  assert_eq!(0, result.errors.len(), "{:?}", result.errors);
  ...
  let ttv = get_type::get::<TableType>(ty).expect("expected table type");
...
fn type_infer_aliases_type_alias_local_mutation() {
  ...
  let ty = fixture.require_type_string("c");
  assert_eq!("Cool", to_string_type_id(ty));

  let ty = follow_type::follow(ty);
  let ttv = get_type::get::<TableType>(ty).expect("expected TableType");
  assert!(ttv.instantiated_type_params.is_empty());
}
...
"#,
  ...
  assert_eq!("Cool", to_string_type_id(fixture.require_type_string("c")));
  assert_eq!(
    "NotCool",
...
type X = Import.X
  ...
  let ty2 = fixture
    ...
    .lookup_type("X")
    .expect("expected local type X");
  assert_eq!(follow_type::follow(ty1), follow_type::follow(ty2));
}
...
// Source: `tests/TypeInfer.aliases.test.cpp:1358`
#[test]
fn type_infer_aliases_only_report_single_error_for_missing_generics_1() {
  use ulua_analysis::{
  ...
  assert_eq!(1, result.errors.len(), "{:?}", result.errors);
  assert!(get_type_error::<IncorrectGenericParameterCount>(&result.errors[0]).is_some());
}

// Source: `tests/TypeInfer.aliases.test.cpp:1370`
#[test]
fn type_infer_aliases_only_report_single_error_for_missing_generics_2() {
  use ulua_analysis::{
    functions::get_error::get_type_error,
    records::incorrect_generic_parameter_count::IncorrectGenericParameterCount,
  };
...
// 缺口（未移植，对照 `tests/TypeInfer.aliases.test.cpp`，共 5 例）：
// - unused_type_arguments（:1421）、type_argument_duplicate_pending_expansions
//   （:1431）——依赖 FFlag `LuauInstantiationCheckArguments`(/Dedup)，本移植
//   未定义该 flag 与实例化实参检查。
// - blocked_type_alias_do_not_leak_generic_arguments（:1485）——依赖 FFlag
//   `LuauBlockingTypeAliasExpansion`，未定义。
// - cyclic_type_alias_through_generic_does_not_assert（:1382）——新求解器
//   bind_constraint 对该输入触发内部 LUAU_ASSERT（上游修复未 sync）。
// - unpack_doesnt_emplace_typeof_type（:1403）——try_dispatch 对该输入触发
//   can_mutate_type_id 内部 LUAU_ASSERT（上游 emplace 行为修复未 sync）。
