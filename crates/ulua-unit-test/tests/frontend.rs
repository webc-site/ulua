...
mod frontend_clear_modules_cleans_up_reverse_dependency_edges {
  //! Ported from `tests/Frontend.test.cpp`.
  //! Source: `tests/Frontend.test.cpp`
  ...
  fn frontend_clear_modules_cleans_up_reverse_dependency_edges() {
    ...
    assert!(
      fixture
        .get_frontend()
        .source_nodes
        .get(&String::from("game/Gui/Modules/A"))
        .expect("expected source node A")
        .dependents
        .contains(&String::from("game/Gui/Modules/B"))
    );

    fixture
      .get_frontend()
      .clear_modules(&[String::from("game/Gui/Modules/B")]);
...
mod frontend_clear_modules_erases_module_and_marks_dependents_dirty {
  //! Ported from `tests/Frontend.test.cpp`.
  //! Source: `tests/Frontend.test.cpp`
  ...
  fn frontend_clear_modules_erases_module_and_marks_dependents_dirty() {
    ...
    assert!(
      !fixture
        .get_frontend()
        .is_dirty(&String::from("game/Gui/Modules/A"), false)
    );
    assert!(
      !fixture
        .get_frontend()
        .is_dirty(&String::from("game/Gui/Modules/B"), false)
    );
    assert!(
      !fixture
        .get_frontend()
        .is_dirty(&String::from("game/Gui/Modules/C"), false)
    );

    fixture
      .get_frontend()
      .clear_modules(&[String::from("game/Gui/Modules/A")]);
    ...
    assert!(
      fixture
        .get_frontend()
        .get_source_module_mut(&String::from("game/Gui/Modules/A"))
        .is_null()
    );
    assert!(
      fixture
        .get_frontend()
        .module_resolver
        .try_get_module(&String::from("game/Gui/Modules/A"))
        .is_none()
    );
...
mod frontend_clear_modules_multiple_with_shared_dependents {
  //! Ported from `tests/Frontend.test.cpp`.
  //! Source: `tests/Frontend.test.cpp`
...
mod frontend_clear_modules_nonexistent_module_is_noop {
  //! Ported from `tests/Frontend.test.cpp`.
  //! Source: `tests/Frontend.test.cpp`
  ...
  fn frontend_clear_modules_nonexistent_module_is_noop() {
    ...
    fixture
      .get_frontend()
      .clear_modules(&[String::from("game/Gui/Modules/DoesNotExist")]);
...
mod frontend_deleted_source_is_evicted_on_recheck {
  //! Ported from `tests/Frontend.test.cpp`.
  //! Source: `tests/Frontend.test.cpp`
  ...
  fn frontend_deleted_source_is_evicted_on_recheck() {
    ...
    assert!(
      !fixture
        .get_frontend()
        .source_nodes
        .contains_key(&String::from("game/B"))
    );
    assert!(
      fixture
        .get_frontend()
        .module_resolver
        .try_get_module(&String::from("game/B"))
        .is_none()
    );
    ...
    assert!(
      fixture
        .get_frontend()
        .module_resolver
        .try_get_module(&String::from("game/A"))
        .is_some()
    );
  }
}
...
mod frontend_exported_tables_have_position_metadata {
  //! Ported from `tests/Frontend.test.cpp`.
  //! Source: `tests/Frontend.test.cpp`
  ...
  fn frontend_exported_tables_have_position_metadata() {
    ...
    let table_type = get_type_id::<TableType>(ret_head[0]).expect("expected a table type");
...
mod frontend_module_scope_check {
  //! Ported from `tests/Frontend.test.cpp`.
  //! Source: `tests/Frontend.test.cpp`
  ...
  fn frontend_module_scope_check() {
    ...
    use ulua_analysis::{
      ...
      records::{binding::Binding, scope::Scope, symbol::Symbol},
    ...
    fixture.base.get_frontend().prepare_module_scope = Some(Rc::new(
      move |_name: &ModuleName, scope: &ScopePtr, _for_autocomplete| {
        let scope_ptr: *mut Scope = Arc::as_ptr(scope).cast_mut();
        // SAFETY: `scope` is the checker's `Arc<Scope>` for the module being
        // checked; it outlives this callback and no other borrow is live.
        unsafe {
          (*scope_ptr).bindings.insert(
            Symbol::from_global(AstName::from_c_str(c"x")),
            Binding {
              type_id: number_type,
...
mod frontend_test_traverse_dependents {
  ...
  fn frontend_test_traverse_dependents() {
    ...
    fixture
      .get_frontend()
      .traverse_dependents(&String::from("game/Gui/Modules/B"), move |node| {
        unsafe {
...
mod frontend_test_traverse_dependents_early_exit {
  ...
  fn frontend_test_traverse_dependents_early_exit() {
    ...
    fixture
      .get_frontend()
      .traverse_dependents(&String::from("game/Gui/Modules/A"), move |node| {
        unsafe {
          (*visited_ptr).push(node.name.clone());
        }
