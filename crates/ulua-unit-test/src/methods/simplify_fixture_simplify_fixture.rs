...
use crate::{
  functions::{create_some_extern_types::create_some_extern_types, raw_handle::raw_handle},
...
impl SimplifyFixture {
  pub fn new() -> Self {
    ...
    let builtins = unsafe {
      // Safety: builtins_ptr 是 fixture 早前 get_frontend 刷新缓存的 Frontend.builtin_types.as_ptr()（指向 Box 内 builtin_types_ 稳定非空地址）；&mut 物化借用仅用于取 any_type_pack 构造 scope 与 arena 登记（cpp BuiltinTypes* 同形），单线程该借用期内无人改写该字段内容。
      &mut *builtins_ptr
    };
    let scope = Arc::new(Scope::scope_type_pack_id(builtins.any_type_pack));
    let scope_ptr = raw_handle(&scope);
