...
pub struct OverloadResolverFixture {
  ...
  /// `resolver.limits` 借用上面 `limits` Box 的堆内容：与相邻 `arena`/`empty_set`
  /// 裸指针字段同一契约——Box 堆地址不随 fixture 移动，指针指向对象比 fixture
  /// 存活期长（对应 C++ fixture 成员自引用，OverloadResolver.test.cpp）。
  pub resolver: OverloadResolver<'static>,
