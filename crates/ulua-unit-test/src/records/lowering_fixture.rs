...
//! Node: `cxx:Record:Luau.UnitTest:tests/IrLowering.test.cpp:93:lowering_fixture`
//! Source: `tests/IrLowering.test.cpp`
//! Graph edges:
//! - declared_by: source_file tests/IrLowering.test.cpp
//! - incoming:
//!   - declares <- source_file tests/IrLowering.test.cpp
//!   - type_ref <- method LoweringFixture::LoweringFixture (tests/IrLowering.test.cpp)
//!   - type_ref <- method LoweringFixture::getCodegenAssembly (tests/IrLowering.test.cpp)
//!   - type_ref <- method LoweringFixture::getCodegenAssemblyUsingCApi (tests/IrLowering.test.cpp)
//!   - type_ref <- method LoweringFixture::getCodegenHeader (tests/IrLowering.test.cpp)
//! - outgoing:
...
//!   - translates_to -> rust_item LoweringFixture
//!
...
use ulua_code_gen::{
  ...
  functions::{
    get_assembly::get_assembly, luau_codegen_create::luau_codegen_create,
    luau_codegen_supported::luau_codegen_supported, set_userdata_remapper::set_userdata_remapper,
  },
  records::{
...
fn luau_library_constant_lookup(library: &str, member: &str, constant: *mut CompileConstant) {
  ...
  if library == "test" {
    match member {
      "some_nil" => luau_set_compile_constant_nil(const_ptr),
      "some_boolean" => luau_set_compile_constant_boolean(const_ptr, true),
      "some_number" => unsafe { luau_set_compile_constant_number(constant, 4.75) },
