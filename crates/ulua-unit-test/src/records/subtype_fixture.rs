...
use ulua_analysis::{
  ...
  records::{
    arena_handle::Handle, builtin_type_functions::BuiltinTypeFunctions,
    builtin_types::BuiltinTypes, global_types::GlobalTypes,
...
impl Default for SubtypeFixture {
  fn default() -> Self {
    ...
    let global_types = Box::new(GlobalTypes::new(&mut builtin_types, SolverMode::New));
    ...
    let mut normalizer = Box::new(Normalizer::new(
      ...
      Handle::from_mut(&mut *builtin_types),
...
impl Drop for SubtypeFixture {
  fn drop(&mut self) {
    ...
    // 干扰其他测试线程读该 flag，且两个 fixture 的 save/restore 交叉会把
