#[path = "common/mod_ir.rs"]
mod common;

extern crate alloc;

// Port of `cpp/tests/IrLowering.test.cpp`.
// IrLowering conformance tests.
...
"#,
  ...
  let actual = format!(
    "\n{}",
    fixture.codegen_assembly(source.as_c_str(), true, 1, 2, false)
  );
  let expected = r#"
...
"#,
  ...
  let actual = format!(
    "\n{}",
    fixture.codegen_assembly(source.as_c_str(), false, 1, 2, false)
  );
  let expected = r#"
...
"#,
  ...
  let actual = format!(
    "\n{}",
    fixture.codegen_assembly(source.as_c_str(), false, 1, 2, true)
  );
  let expected = r#"
; function foo($arg0) line 2
bb_0:
...
fn ir_lowering_buffer_vm_exit_sync_multi_use_sink() {
  ...
  // cpp IrLowering.test.cpp:5953 仍以 LuauCodegenVmExitSyncMultiUse 门控多用户沉入路径
  let _multi_use = ScopedFastFlag::new(&fflag::LuauCodegenVmExitSyncMultiUse, true);
...
"#,
  ...
  let actual = format!(
    "\n{}",
    fixture.codegen_assembly(source.as_c_str(), true, 2, 2, false)
  );
  let expected = r#"
; function foo(a) line 4
; R0: table [argument 'a']
...
"#,
  ...
  let actual = format!(
    "\n{}",
    fixture.codegen_assembly(source.as_c_str(), false, 1, 1, false)
  );
  let expected = r#"
; function f() line 2
bb_bytecode_0:
...
"#,
  ...
  let actual = fixture.codegen_assembly(source.as_c_str(), false, 1, 2, false);
...
fn ir_lowering_fuzz_test_14() {
  ...
  let _dse_ptr_store_tag_check = ScopedFastFlag::new(&fflag::LuauCodegenDsePtrStoreTagCheck, true);
...
"#,
  ...
  let actual = fixture.codegen_assembly(source.as_c_str(), false, 1, 1, false);
  assert!(!actual.is_empty());
}
...
"#,
  ...
  let assembly = fixture.codegen_assembly(source.as_c_str(), false, 1, 2, false);
...
#[test]
fn ir_lowering_library_field_types_and_constants_alt_b() {
  use std::ffi::CString;
...
"#,
  ...
  let actual = format!(
    "\n{}",
    fixture.codegen_assembly(source.as_c_str(), true, 1, 2, true)
  );
  let expected = r#"
; function test($arg0, $arg1) line 2
; R0: table [argument]
...
fn ir_lowering_buffer_write_checks_extra_args() {
  // 与 C++ 的差异：Rust 移植从栈副本寄存器 R4/R5/R6 加载，
  // C++ 直接从 R0/R1/R2 加载并带 CHECK_TAG；核心语义（CHECK_BUFFER_LEN + ADJUST_STACK_TO_REG）一致。
  use std::ffi::CString;
...
fn ir_lowering_fuzz_test_26() {
  ...
  let _integer_fastcalls = ScopedFastFlag::new(&fflag::LuauIntegerFastcalls, true);
  let _luau_codegen_integer_3 = ScopedFastFlag::new(&fflag::LuauCodegenInteger3, true);
...
#[test]
// Port of `FuzzTest27` from `cpp/tests/IrLowering.test.cpp`.
// cpp 版第一段（`for _ in 1,10 ... math.acos ...`）在 Rust 移植 code-gen 中
// 触发 const_prop_state_get_combined_array_load_offset_op 乘法溢出 panic，待修复后补。
