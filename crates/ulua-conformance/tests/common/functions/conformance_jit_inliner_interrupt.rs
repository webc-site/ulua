...
use ulua_vm::{luaL_error, records::lua_state::LuaState};
...
/// # Safety
///
/// 指针参数须为有效的 `LuaState`，由 VM 经回调契约调用。
///
/// 内联回归测试包含死循环用例，靠 interrupt 兜底，达阈值即报 "timeout"。
pub unsafe extern "C-unwind" fn conformance_jit_inliner_interrupt(l: *mut LuaState, gc: c_int) {
  unsafe {
    // gc >= 0 表示 GC 步进，不计数
    if gc >= 0 {
      return;
