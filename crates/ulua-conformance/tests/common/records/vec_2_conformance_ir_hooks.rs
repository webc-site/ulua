...
impl Vec2 {
  /// tag 取自 `userdata_tags`（对应 cpp `ConformanceIrHooks.h:15` 的 `kTagVec2`），
  /// 与 C API 路径共用同一来源，避免上游调整时两条路径静默漂移。
  pub const TAG: i32 = K_TAG_VEC2 as i32;
  pub const USERDATA_INDEX: u8 = UserdataKind::VEC2;
  pub const SIZE: usize = size_of::<Self>();
  pub const OFFSET_X: usize = offset_of!(Self, x);
  pub const OFFSET_Y: usize = offset_of!(Self, y);
