...
/// # Safety
/// `l` 必须指向存活的 `LuaState`。
pub(crate) unsafe fn push_c_str(l: *mut LuaState, s: &[u8]) {
  let s = c_str_prefix(s);
  // Safety: l 由本函数 /// # Safety 契约保证指向存活的 LuaState；s.as_ptr()/s.len() 由 &[u8] 切片导出，非空、对齐且长度匹配；lua_pushlstring 将字节拷入 VM 字符串存储后不再引用该切片，故短生命周期借用跨调用即安全。
...
/// # Safety
/// `l` 必须指向存活的 `LuaState`。
pub(crate) unsafe fn push_lowered_c_str(l: *mut LuaState, s: &[u8]) {
  // Safety: l 为契约保证存活的 LuaState；两分支传给 lua_pushlstring 的指针/长度分别来自本帧局部 Vec（lowered）或切片 s（均存活、长度匹配），被调方拷贝字节，无悬挂也无别名写。
  unsafe {
    let s = c_str_prefix(s);
