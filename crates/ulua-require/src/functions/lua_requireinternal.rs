...
pub(crate) unsafe fn lua_requireinternal(l: *mut lua_State, requirer_chunkname: &CStr) -> i32 {
  unsafe {
    lua_settop(l, 1);
...

    {
      ...
      if resolved_require.status == Status::ErrorReported {
        let error = c_cstring(&resolved_require.error);
        lua_pushstring(l, error.as_ptr());
        resolve_error = true;
      } else {
        let cache_key = c_cstring(&resolved_require.cache_key);
        let chunkname = c_cstring(&resolved_require.chunkname);
        let loadname = c_cstring(&resolved_require.loadname);
