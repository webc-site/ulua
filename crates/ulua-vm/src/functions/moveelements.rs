...
/// 数组区间拷贝：目标区间与源重叠且靠前时逆序，避免覆盖未读元素（与 cpp 一致）
#[inline]
unsafe fn copy_array_range(
  l: *mut lua_State,
  srcarray: *mut TValue,
  dstarray: *mut TValue,
  f: i32,
  ...
  unsafe {
    if reverse {
      for i in (0..n).rev() {
        setobj2t!(
          l,
          dstarray.offset((t + i - 1) as isize),
          srcarray.offset((f + i - 1) as isize)
        );
      }
    } else {
      for i in 0..n {
...
/// 栈槽区间搬移：经 lua_rawgeti/lua_rawseti 逐元素搬运，方向语义同 copy_array_range
#[inline]
  ...
  unsafe {
      ...
      && t_index.wrapping_add(n_unsigned) <= (*dst).sizearray as u32
    {
      ...
      if t > e || t <= f || (dstt != srct && dst != src) {
        copy_array_range(l, srcarray, dstarray, f, t, n, false);
      } else {
        copy_array_range(l, srcarray, dstarray, f, t, n, true);
      }
