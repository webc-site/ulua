...
/// cpp `rehash_DEPRECATED`（`VM/src/ltable.cpp:714`）：按数组/哈希两部分的键分布重算
/// 两部分容量并 `resize`。
///
/// §11 pass B（表键簇）：额外键 `ek` 的 `ttisnumber! + nvalue!` 区间计数判据经
/// [`ValueView`]（Number 变体即数值键，payload 即 `nvalue!` 的 f64），与 `numusehash`
/// 的键轴读法同形。
///
/// # Safety
