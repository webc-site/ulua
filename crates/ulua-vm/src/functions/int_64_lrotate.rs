...
#[cfg_attr(feature = "capi", unsafe(export_name = "ulua_int64_lrotate"))]
pub(crate) unsafe extern "C-unwind" fn int64_lrotate(l: *mut LuaState) -> c_int {
  unsafe {
    let n = lua_l_checkinteger_64(l, 1) as u64;
    let s = (lua_l_checkinteger_64(l, 2) as u64 % 64) as u32;

    let result = if s != 0 { n.rotate_left(s) } else { n };
