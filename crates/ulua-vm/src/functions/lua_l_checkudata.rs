...
pub unsafe fn lua_l_checkudata(l: *mut LuaState, ud: i32, tname: &str) -> *mut c_void {
  unsafe {
    if let Some(p) = lua_touserdata(l, ud)
      && lua_getmetatable(l, ud) != 0
    {
      ...
      // 超长串的堆缓冲；常规串为空 Vec，不发生分配
      let heap_buf: Vec<u8> = if bytes.len() < STACK_TNAME_CAP {
        Vec::new()
      } else {
      ...
      let c_tname: *const c_char = if heap_buf.is_empty() {
        ...
        heap_buf.as_ptr() as *const c_char
      };
