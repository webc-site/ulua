use crate::{
  functions::{sort_heap::sort_heap, sort_less::sort_less, sort_swap::sort_swap},
...
/// # Safety
///
/// `l`/`t`/`pred` 须满足 `sort_less`/`sort_swap` 的入约（存活调用帧与表、`0 <= lo <= u <
/// sizearray`、谓词不改表）；`limit` 为剩余快排递归预算，耗尽即退化到 `sort_heap` 保
/// nlogn。谓词违约（划分越出 `[lo, u]`）时经 `lua_l_error_l` 抛 "invalid order function
/// for sorting"，不返回。cpp ltablib.cpp:480 `sort_rec`。
pub(crate) unsafe fn sort_rec(
  ...
  // Safety: 契约保证区间界内；每层划分只在 [lo..u] 内 less/swap，越界即谓词违约
  unsafe {
