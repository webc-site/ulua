      ...
      'dispatch: loop {
        ...
        match transmute::<u8, LuauOpcode>(op) {
              ...
              if !ttisfunction!(ra as *const TValue) {
                ...
                if fn_tm.is_null() && ttisobject!(ra as *const TValue) {
                  ...
                  if ttisnil!(fn_tm) {
                    ...
                    luaG_typeerrorL(l, ra as *const TValue, ERR_ITERATE_OVER);
                  }
                }
                ...
                if !fn_tm.is_null() {
                  ...
                  if ttisnil!(ra as *const TValue) {
                    (*(*l).ci).savedpc = pc; // vm_protect_pc(): next call always errors
                    luaG_typeerrorL(l, ra as *const TValue, "call");
                  }
          ...
          LuauOpcode::LOP_FORGLOOP => {
            // lvmexecute.cpp:2807
            VM_INTERRUPT!(l, pc, base);
            // cpp lvmexecute.cpp:2709-2711（FFlag::LuauBackedgeHeapCheck 开启时在回边
            // 额外做 VM_CHECK_GC）。该旗标尚未接线（默认关，行为与 cpp 默认一致），
            // 接线需在 ulua-common 注册 FFlag 后再补此分支。
            let insn = *pc;
            pc = pc.add(1);
            let mut ra = VM_REG!(LUAU_INSN_A!(insn), l, base) as *mut TValue;
            let aux: u32 = *pc;
