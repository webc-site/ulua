use crate::{
  enums::lua_type::LuaType,
  functions::{lua_tonumberx::lua_tonumberx, tag_error::tag_error},
  records::lua_state::LuaState,
};

/// # Safety
/// `l` 必须是正在执行的 C 函数帧，`narg` 为其中合法栈索引；非数值时经 `tag_error` 抛 Lua 错误、
/// 不返回。索引越界会读到错误槽位甚至悬垂 TValue。cpp laux.cpp:196。
pub unsafe fn lua_l_checknumber(l: *mut LuaState, narg: i32) -> f64 {
  // Safety: 契约保证 `l` 为存活调用帧且 narg 栈槽可读；非数值路径经 typeerror 抛错、不返回
  unsafe {
    match lua_tonumberx(l, narg) {
      Some(d) => d,
      None => tag_error(l, narg, LuaType::Number as i32),
    }
  }
}

// lualib.h name
