...
// 重复字面量提取为常量，逻辑不变。
const EQ: c_char = b'=' as c_char;
const AT: c_char = b'@' as c_char;
...
pub(crate) unsafe fn lua_o_chunkid(
  ...
  // Safety: 契约保证 `source` 串数据可读、`output` 至少有 32 字节可写空间，块内截断/格式化写入不越出 output
  unsafe {
    ...
    if *source == EQ {
      ...
      *buf.add(buflen - 1) = NUL;
      ...
      // memcpy(buf, "...", 3);
      copy_nonoverlapping(DOTS.as_ptr(), buf as *mut u8, DOTS.len());
      ...
      *buf.add(buflen - 1) = NUL;
      ...
      if *source.add(len) != 0 {
        ...
        copy_nonoverlapping(DOTS.as_ptr(), buf.add(end) as *mut u8, DOTS.len());
        end += DOTS.len();
      }
      // strcat(buf, "\"]")
      let suffix = b"\"]";
