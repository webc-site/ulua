...
#[cfg_attr(feature = "capi", unsafe(export_name = "ulua_os_time"))]
pub(crate) unsafe extern "C-unwind" fn os_time(l: *mut lua_State) -> c_int {
  unsafe {
    let t: i64 = if lua_isnoneornil!(l, 1) {
      ...
      ts.tm_sec = getfield(l, c"sec", 0);
      ts.tm_min = getfield(l, c"min", 0);
      ts.tm_hour = getfield(l, c"hour", 12);
      ts.tm_mday = getfield(l, c"day", -1);
      ...
      ts.tm_mon = getfield(l, c"month", -1).wrapping_sub(1);
      ts.tm_year = getfield(l, c"year", -1).wrapping_sub(1900);
      ts.tm_isdst = getboolfield(l, c"isdst");
