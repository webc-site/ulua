...
use crate::{
  ...
  records::{
    g_cheader::GCheader, global_state::global_State, lua_page::lua_Page,
...
pub(crate) unsafe fn newgcoblock(l: *mut lua_State, size_class: c_int) -> *mut u8 {
  unsafe {
    ...
    let freegcopages = addr_of_mut!((*g).freegcopages) as *mut *mut lua_Page;
    ...
    if (*page).free_next >= 0 {
      let data = addr_of_mut!((*page).data) as *mut c_char;
      ...
      ASAN_UNPOISON_MEMORY_REGION!(
        (block as *mut c_char).add(size_of::<GCheader>()) as *mut u8,
