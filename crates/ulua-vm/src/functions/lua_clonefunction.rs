...
use crate::{
  ...
  functions::{
    c_slice, c_slice_mut, ensure_stack::ensure_stack, index_2_addr::index2addr,
    lua_concat::lua_c_threadbarrier_lapi, lua_f_new_lclosure::luaF_newLclosure,
  },
  macros::{
...
pub unsafe fn lua_clonefunction(l: *mut lua_State, idx: c_int) {
  unsafe {
    ...
    let cl = addr_of_mut!((*(*p).value.gc).cl) as *mut Closure;
    api_check!(
      l,
