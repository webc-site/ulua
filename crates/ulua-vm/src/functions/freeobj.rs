  ...
  unsafe {
    match (*o).gch.tt as i32 {
      x if x == LuaType::Proto as i32 => {
        luaF_freeproto(l, addr_of_mut!((*o).p) as *mut _, page);
      }
      x if x == LuaType::Function as i32 => {
        lua_f_freeclosure(l, addr_of_mut!((*o).cl) as *mut _, page);
      }
      x if x == LuaType::Upval as i32 => {
        lua_f_freeupval(l, addr_of_mut!((*o).uv) as *mut _, page);
      }
      x if x == LuaType::Table as i32 => {
        lua_h_free(l, addr_of_mut!((*o).h) as *mut _, page);
      }
      x if x == LuaType::Thread as i32 => {
        let th = addr_of_mut!((*o).th) as *mut lua_State;
        LUAU_ASSERT!(th != l && th != (*(*l).global).mainthread);
        luaE_freethread(l, th, page);
      }
      x if x == LuaType::String as i32 => {
        luaS_free(l, addr_of_mut!((*o).ts) as *mut _, page);
      }
      x if x == LuaType::UserData as i32 => {
        lua_u_freeudata(l, addr_of_mut!((*o).u) as *mut _, page);
      }
      x if x == LuaType::Buffer as i32 => {
        lua_b_freebuffer(l, addr_of_mut!((*o).buf) as *mut _, page);
      }
      x if x == LuaType::Class as i32 => {
        lua_r_freeclass(l, addr_of_mut!((*o).lclass) as *mut _, page);
      }
      x if x == LuaType::Object as i32 => {
        lua_r_freeobject(l, addr_of_mut!((*o).lobject) as *mut _, page);
      }
      _ => {
        LUAU_ASSERT!(false);
      }
