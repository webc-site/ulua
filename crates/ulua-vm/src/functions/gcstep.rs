...
use ulua_common::{clock_shim::monotonic_seconds, macros::luau_assert::LUAU_ASSERT};

use crate::{
  functions::{
    atomic::atomic, lua_m_getnextpage::lua_m_getnextpage, markroot::markroot,
...
pub(crate) unsafe fn gcstep(l: *mut LuaState, limit: usize) -> usize {
  unsafe {
    let mut cost = 0usize;
    let g = (*l).global;
    ...
    match (*g).gcstate as i32 {
      ...
      GCSATOMIC => {
        (*g).gcstats.atomicstarttimestamp = monotonic_seconds();
      ...
      GCSSWEEP => {
        while !(*g).sweepgcopage.is_null() && cost < limit {
          let next = lua_m_getnextpage(&*(*g).sweepgcopage);
