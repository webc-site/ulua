...
use crate::{
  ...
  macros::{
    lua_errerrmsg::LUA_ERRERRMSG, lua_memerrmsg::LUA_MEMERRMSG, lua_s_newliteral::lua_s_newliteral,
    setobj_2_s::setobj_2_s, setsvalue::setsvalue,
  },
...
#[cfg_attr(feature = "capi", unsafe(export_name = "ulua_luaD_seterrorobj"))]
pub unsafe fn lua_d_seterrorobj(l: *mut lua_State, errcode: c_int, oldtop: StkId) {
  unsafe {
    if errcode == LuaStatus::ErrMem as c_int {
      setsvalue!(l, oldtop, lua_s_newliteral(l, LUA_MEMERRMSG.as_ptr()));
    } else if errcode == LuaStatus::ErrErr as c_int {
      setsvalue!(l, oldtop, lua_s_newliteral(l, LUA_ERRERRMSG.as_ptr()));
