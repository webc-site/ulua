...
pub unsafe fn lua_c_step(l: *mut lua_State, assist: bool) -> usize {
  ...
  unsafe {
    ...
    // cpp lgc.cpp:1299-1306：`LuauBackedgeHeapCheck`（或双浮点向量构建）下 assist 步长补偿
    // `need = debt * gcstepmul / 100; if need > lim { lim = need }`。该旗标尚未接线
    // （cpp 默认关且本移植为单精度向量，行为与 cpp 默认一致），接线后需补此分支。
