use core::{
  ffi::{CStr, c_int},
  mem::size_of,
  ptr::{addr_of_mut, copy_nonoverlapping, null_mut, write_bytes},
};
...
pub(crate) unsafe extern "C-unwind" fn lua_r_constructobject(l: *mut lua_State) -> c_int {
  unsafe {
    ...
    // 批量拷贝参数（setobj2s 为纯结构体拷贝，与 cpp 逐元素 setobj2s 语义一致；
    // base..base+numargs 与 top 之后不重叠，边界由上方 luaD_checkstack 保证）
    let numargs = numargs as usize;
    copy_nonoverlapping((*l).base, (*l).top, numargs);
    (*l).top = (*l).top.add(numargs);
...
pub(crate) unsafe fn lua_r_setupconstructor(
  ...
  unsafe {
    ...
    let constructor_c = addr_of_mut!((*constructor).inner.c);
    ...
    let default_ctor_c = addr_of_mut!((*default_ctor).inner.c);
...
pub(crate) unsafe fn lua_r_newclass(
  ...
  unsafe {
    ...
    for member in c_slice_mut((*classobject).staticmembers, numberofstaticmembers as usize) {
      setnilvalue!(member);
    }
