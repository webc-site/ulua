...
pub(crate) fn parse_format_spec(spec: &[u8]) -> FormatSpec {
  ...
  let (width, i) = parse_usize(spec, i);
  fs.width = width;
  if i < spec.len() && spec[i] == b'.' {
    let (p, _) = parse_usize(spec, i + 1);
    fs.precision = Some(p);
  }
  fs
}

/// 从 `spec[i]` 起解析十进制数字串，返回 (值, 新下标)。
fn parse_usize(spec: &[u8], mut i: usize) -> (usize, usize) {
  let mut v = 0usize;
  while i < spec.len() && spec[i].is_ascii_digit() {
    v = v * 10 + (spec[i] - b'0') as usize;
    i += 1;
  }
  (v, i)
}
...
fn push_exponent(out: &mut Vec<u8>, e_char: u8, exp: i32) {
  ...
  out.extend_from_slice(itoa::Buffer::new().format(mag).as_bytes());
}
...
fn general_body(a: f64, precision: usize, alt: bool, e_char: u8) -> Vec<u8> {
  ...
  if exp < -4 || exp >= p as i32 {
    ...
    let mut out = format!("{:.*}", (p as i32 - 1 - exp) as usize, a).into_bytes();
    if !alt {
      strip_g(&mut out);
    } else if !out.contains(&b'.') {
      out.push(b'.');
...
pub fn format_g(v: f64) -> String {
  ...
  String::from_utf8(format_float(&spec, b'g', v)).unwrap_or_default()
}
