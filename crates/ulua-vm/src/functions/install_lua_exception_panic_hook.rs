...
//!
//! Luau's `lua_d_throw` uses C++ exceptions for control flow; the faithful Rust
//! port (`lua_d_throw`) emulates this with [`std::panic::panic_any`] carrying a
//! [`lua_exception`], caught at the [`lua_d_rawrunprotected`] boundary
...
pub fn install_lua_exception_panic_hook() {
  INSTALL.call_once(|| {
    ...
    set_hook(Box::new(move |info| {
      ...
      // `lua_d_rawrunprotected`; do not print anything for it.
