...
use crate::{
  ...
  functions::{
    ...
    report_error_compile::report_parse_error, report_error_compile_alt_b::report_compile_error,
  },
...
pub fn compile_file(
  ...
  let result = catch_unwind(AssertUnwindSafe(|| {
    ...
    let parse_options = ParseOptions {
      store_cst_data: unsafe { with(|opts| opts.parse_cst) },
    ...
    if unsafe { with(|opts| opts.only_parse) } {
      return true;
    }
