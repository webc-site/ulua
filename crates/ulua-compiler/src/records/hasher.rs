...
impl DenseHasher<(Node<AstExprTable>, AstName)> for Hasher {
  fn hash(&self, key: &(Node<AstExprTable>, AstName)) -> usize {
    // 键为两个指针（table 指针 + AstName 内部字符串指针），按小端序列进栈上数组，一次成型。
    const PTR_BYTES: usize = size_of::<usize>();
    let mut bytes = [0u8; PTR_BYTES * 2];
    bytes[..PTR_BYTES].copy_from_slice(&key.0.as_ptr().addr().to_le_bytes());
