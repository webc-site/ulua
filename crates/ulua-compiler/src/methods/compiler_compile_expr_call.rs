  ...
  pub unsafe fn compile_expr_call(
    ...
    unsafe {
      ...
      if self.options.optimization_level >= 2 && !expr_ref.self_ {
        ...
        if !func.is_null() && !can_inline && (*self.bytecode).needs_debug_remarks() {
          let func_ref = &*func;
          if func_ref.vararg {
            (*self.bytecode)
        ...
        && self.is_constant(*expr_ref.args.data.add(2))
      {
        ...
        if fi >= 0 && wi > 0 && fi as i64 + wi as i64 <= 32 {
          ...
          if cid < 0 {
            let location = expr_ref.base.base.location;
      ...
      if expr_ref.self_ {
        ...
        if cid < 0 {
          let location = (*fi).base.base.location;
          CompileError::raise(&location, format_args!("{ERR_EXCEEDED_CONSTANT_LIMIT}"));
        }
        ...
        if !(*self.bytecode).patch_skip_c(fastcall_label, call_label) {
          let location = (*expr_ref.func).base.location;
          CompileError::raise(
            &location,
