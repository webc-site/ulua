...
use ulua_common::{
  enums::{
    luau_bytecode_type::LBC_TYPE_ANY, luau_opcode::LuauOpcode, luau_proto_flag::LuauProtoFlag,
  },
  fflag,
  ...
  pub unsafe fn compile_function(
    ...
    unsafe {
      ...
      if !func_ref.self_.is_null() {
        self.push_local(func_ref.self_, args, !0u32);
      }
      for (i, &arg_local) in func_ref.args.iter().enumerate() {
        self.push_local(arg_local, args + self_ as u8 + i as u8, !0u32);
      }
      ...
      // 否则 24 位偏移静默截断产生坏字节码
      let has_long_jump_error = (*self.bytecode).expand_jumps();
      ...
      if (*self.bytecode).get_instruction_count() > K_MAX_INSTRUCTION_COUNT {
        CompileError::raise(
          &func_ref.base.base.location,
          format_args!(
      ...
      if func_ref.function_depth == 0 && !self.has_loops {
        protoflags |= LuauProtoFlag::LPF_NATIVE_COLD as u8;
      }
      if func_ref.has_native_attribute() {
        protoflags |= LuauProtoFlag::LPF_NATIVE_FUNCTION as u8;
      }

      // C++: `isInlinable = !hasMultiRet && !getfenvUsed && !setfenvUsed;`
      // 本移植无 hasMultiRet 跟踪，以 `!vararg` 近似（更保守）。
      ...
      if fflag::LuauEmitCallFeedback.get() && is_inlinable && self.upvals.is_empty() {
        protoflags |= LuauProtoFlag::LPF_INLINABLE as u8;
