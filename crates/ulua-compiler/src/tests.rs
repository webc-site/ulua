...
use crate::{
  functions::{
    ...
    fold_binary::fold_binary, fold_unary::fold_unary, parallel_add_sat::parallel_add_sat,
...
fn record_constant_logs_overwrite_for_undo() {
  ...
  let mut constants: DenseHashMap<*mut AstExpr, Constant> = DenseHashMap::new(null_mut());
  let mut variables: DenseHashMap<*mut AstLocal, Variable> = DenseHashMap::new(null_mut());
  let mut locals: DenseHashMap<*mut AstLocal, Constant> = DenseHashMap::new(null_mut());
  let table_constant_locals: DenseHashMap<*mut AstLocal, TableConstantKind> =
    DenseHashMap::new(null_mut());

  // 仅作哈希键，记录/回滚路径不解引用
  let key = NonNull::<AstExpr>::dangling().as_ptr();
  ...
  *constants.get_or_insert(key) = cnum(42.0);
  ...
  let mut visitor = ConstantVisitor::new(ConstantVisitorArgs {
    ...
    expr_change_log: &mut expr_log,
    local_change_log: &mut local_log,
  });
