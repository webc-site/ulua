...
pub(crate) fn get_builtin_info(bfid: i32) -> BuiltinInfo {
  // `as u8` 截断 + transmute 非法判别值是 UB：先校验范围，
  // 非法 id（-1/损坏数据）归入 LBF_NONE（params/results = -1），与 C++ switch 语义一致
  let bf = LuauBuiltinFunction::from_id(bfid).unwrap_or(LuauBuiltinFunction::LBF_NONE);
  match bf {
