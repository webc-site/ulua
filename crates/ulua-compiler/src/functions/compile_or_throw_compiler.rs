...
pub fn compile_or_throw_bytecode_builder_parse_result_ast_name_table_compile_options(
  ...
  let has_fenv =
    !names.get_str("getfenv").value.is_null() || !names.get_str("setfenv").value.is_null();
  ...
  for expr in functions {
    let (_, protoflags) = unsafe { compiler.compile_function(expr, 0) };
  ...
  let mainid = unsafe { compiler.compile_function(&mut main, main_flags).0 };
