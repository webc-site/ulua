    ...
    match data.r#type {
      ...
      Type::String => {
        ...
        if printable_string_constant(bytes) {
          ...
          if str.length < 32 {
            formatAppend(result, format_args!("'{:.*}'", str.length, s));
          } else {
            formatAppend(result, format_args!("'{:.*}'...", 32, s));
          }
        } else {
      ...
      Type::Import => {
        ...
        if count > 0 {
          ...
          formatAppend(result, format_args!("{}", seg0.to_string_lossy()));

          if count > 1 {
            let seg1 = self.import_segment_str(id1);
            formatAppend(result, format_args!(".{}", seg1.to_string_lossy()));
          }

          if count > 2 {
            let seg2 = self.import_segment_str(id2);
            formatAppend(result, format_args!(".{}", seg2.to_string_lossy()));
          }
        }
      }
      Type::Table => {
