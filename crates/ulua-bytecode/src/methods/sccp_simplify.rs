...
impl<I: VmConstOps + ?Sized> Sccp<'_, '_, I> {
  ...
  pub(crate) fn simplify_phis(&mut self) {
    ...
    for block_op in visited_blocks {
      ...
      let mut phi_idx = 0;
      while phi_idx < self.func.block(block_op).operator_deref().phis.len() {
        let phi_op = self.func.block(block_op).operator_deref().phis[phi_idx];
        ...
        let unique = phi_ops.as_slice()[0];
        let all_same = phi_ops.as_slice().iter().all(|&op| op == unique);
        if !all_same {
          phi_idx += 1;
          continue;
        }
        ...
        for use_op in self.state.uses_of(phi_op).to_vec() {
          match use_op.kind {
            BcOpKind::Inst => {
              let ops = &mut self.func.inst_op(use_op).ops;
            ...
            BcOpKind::Phi => {
              let ops = &mut self.func.phi_op(use_op).ops;
              for op in ops.as_mut_slice() {
                if *op == phi_op {
                  *op = unique;
                }
