...
impl BytecodeBuilder {
  pub fn write_function(&mut self, ss: &mut String, id: u32, flags: u8) {
    ...
    // cpp: `cpp/Bytecode/src/BytecodeBuilder.cpp:1041-1046` —
    // LPF_INLINABLE 时写 cost。Rust 编译器尚未把 costModel 接入
    // endFunction（cpp Compiler.cpp:621），此处恒写 0。
    if (FFlag::LuauBytecodeCostModel.get()
      || FFlag::LuauCompileEmitVectorDouble.get()
      || FFlag::LuauCompileFastpcall.get()
      || FFlag::DebugLuauUserDefinedClasses.get())
      && (flags & LuauProtoFlag::LPF_INLINABLE as u8) != 0
    {
      write_var_int(ss, 0);
    }
  }
