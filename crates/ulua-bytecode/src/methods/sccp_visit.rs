...
impl<I: VmConstOps + ?Sized> Sccp<'_, '_, I> {
  ...
  pub fn visit_phi(&mut self, phi_op: BcOp) {
    ...
    let mut fold = ConstnessLattice::UNDETERMINED;
    for op in phi_ops {
  ...
  pub fn visit_inst(&mut self, inst_op: BcOp) {
    ...
    if opcode == LuauOpcode::LOP_CAPTURE && inst_ops.len() >= 2 {
      ...
      if is_ref {
        ...
        if prev.kind != Constness::NotAConstant
          && matches!(src_op.kind, BcOpKind::Inst | BcOpKind::Phi)
        {
          self
            .state
            .op_constness
            .insert(src_op, ConstnessLattice::NOT_A_CONSTANT);
    ...
    while !self.state.flow_worklist.is_empty() || !self.state.ssa_worklist.is_empty() {
      while let Some(block_op) = self.state.flow_worklist.pop_front() {
        ...
        let (phis, ops, successors) = {
          let block = self.func.block(block_op);
          (
            block.operator_deref().phis.clone(),
            block.operator_deref().ops.clone(),
            block.operator_deref().successors.clone(),
          )
        };
        ...
        for succ_edge in successors.as_slice() {
