...
impl<'a> CallInliner<'a> {
  pub fn inline_target(&mut self, target_proto_id: u32) -> bool {
    ...
    if self.target.is_vararg {
      ...
      new_max_stack_size = new_max_stack_size
        .wrapping_add(cmp::max(self.call_params.len() as u8, self.target.numparams) as u32);
    }
    ...
    // 原地剔除 prev→call 块的 fallthrough 前驱边
    *insn_preds = insn_preds
    ...
    upsert_fallthrough(
      &mut prev_block.operator_deref_mut().successors,
    ...
    for (caller_block_op, target_return_op) in mem::take(&mut self.return_sites) {
      if !self.replace_return(&mut next_block, caller_block_op, target_return_op) {
        return false;
      }
    }
    ...
    for ret_op in &self.return_ops {
      if ret_op.kind == BcOpKind::Phi {
        next_block.operator_deref_mut().phis.push(*ret_op);
      }
    }
