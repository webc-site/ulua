...
impl<'a> BytecodeGraphSerializer<'a> {
  pub fn emit_instruction(&mut self, insn_op: BcOp) {
    // 克隆出独立副本，避免 &mut self.func 与 &mut self.bcb 借用冲突
    ...
    match insn.op {
      ...
      LuauOpcode::LOP_NEWCLASS => {
        ...
        let has_super = insn.ops.as_slice()[0].kind != BcOpKind::None;
        let b = if has_super {
          self.get_reg_input(insn, 0)
        } else {
          0xff
        };
        let count = self.get_imm_import(insn, 1);
        self
          .bcb
          .emit_abc(LuauOpcode::LOP_NEWCLASS, out, b, count as u8);
        let const_idx = self.get_vm_const_input_aux(insn, 2);
      ...
      LuauOpcode::LOP_LOADN => {
        ...
        let imm_int = self.get_imm_int_as_signed_d(insn, 0);
        let out = self.get_register(insn_op);
      ...
      LuauOpcode::LOP_GETIMPORT => {
        ...
        LUAU_ASSERT!(insn.ops.len() as u32 - 2 == components_count);
      ...
      LuauOpcode::LOP_SETUDATAKS | LuauOpcode::LOP_SETTABLEKS => {
        ...
        self.emit_ks_aux(insn, 3, 4);
      }
      LuauOpcode::LOP_GETTABLEN => {
        let reg_input_0 = self.get_reg_input(insn, 0);
        // cpp 419：getImmIntAsUnsignedABC(bias=-1) 超出 u8 时置 error
        let imm_int = self.get_imm_int_as_unsigned_abc(insn, 1, -1);
        let out = self.get_register(insn_op);
        self
          .bcb
      ...
      LuauOpcode::LOP_SETTABLEN => {
        ...
        let imm_int = self.get_imm_int_as_unsigned_abc(insn, 2, -1);
        self
          .bcb
      ...
      LuauOpcode::LOP_RETURN => {
        ...
        let reg_input = self.get_reg_input_for_range(insn, 1, imm_int_0);
        self
          .bcb
      ...
      LuauOpcode::LOP_SETLIST => {
        ...
        let start_reg = if imm_int_1 != 0 {
          ...
          self.get_reg_input(insn, 3)
        } else {
          self.get_reg_input(insn, 2)
        };
        let reg_input_2 = self.get_reg_input(insn, 2);
        let imm_int_0 = self.get_imm_int(insn, 0);
        self.bcb.emit_abc(
      ...
      LuauOpcode::LOP_GETVARARGS => {
        ...
        let reg_input = self.get_reg_input_for_range(insn, 0, imm_int_1);
        self.bcb.emit_abc(
          LuauOpcode::LOP_GETVARARGS,
          reg_input,
          (imm_int_1 + 1) as u8,
