  ...
  pub fn emit_instruction(&mut self, insn_op: BcOp) {
    ...
    match insn.op {
      ...
      LuauOpcode::LOP_GETUDATAKS | LuauOpcode::LOP_GETTABLEKS => {
        ...
        if insn.op == LuauOpcode::LOP_GETUDATAKS {
          let aux16 = self.get_vm_const_input_aux16(insn, 2);
          let flags = self.get_imm_int(insn, 3);
      ...
      LuauOpcode::LOP_SETUDATAKS | LuauOpcode::LOP_SETTABLEKS => {
        ...
        if insn.op == LuauOpcode::LOP_SETUDATAKS {
          let aux16 = self.get_vm_const_input_aux16(insn, 3);
          let flags = self.get_imm_int(insn, 4);
          self.bcb.emit_aux(aux16 | (flags as u32) << 16);
        } else {
