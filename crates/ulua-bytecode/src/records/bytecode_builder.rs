...
// cpp 中 BytecodeBuilder 按值传递即浅拷贝（encoder 是裸指针）。Rust 端 encoder
// 为 Box<dyn BytecodeEncoder> 不可 Clone；全仓无 clone/格式化使用点，故不派生。
...
pub struct BytecodeBuilder {
  ...
  // cpp 里是 `BytecodeEncoder*` 裸指针。BytecodeBuilder 被整条编译器链路以非泛型
  // 形式共享 (~100 处引用, Compiler 记录亦存 *mut BytecodeBuilder), 在结构体上引入
  // 泛型参数会波及全部链路, 故按"确需存储异构编码器"的例外用 Box<dyn> 做类型擦除;
  // 仅在 end_function 里回调一次, 单次编译至多一次堆分配。
  pub(crate) encoder: Option<Box<dyn BytecodeEncoder>>,
