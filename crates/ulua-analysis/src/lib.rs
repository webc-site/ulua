...
//!    C 字符串已按「非 C ABI 层一律 Rust 化」收口：全 crate 无 `c"..."`/`CString`
//!    字面量，VM 契约需要的 NUL 结尾静态名字集中在 `functions::lua_names`
//!    （`&'static [u8]` 常量），抛错走 `functions::throw_type_error`，Rust 字符串压栈
//!    走 `functions::push_string`，读 VM 返回的 `*const c_char` 只经 `ulua-common`
//!    的 `cstr_cow` / `cstr_bytes` 门面。
