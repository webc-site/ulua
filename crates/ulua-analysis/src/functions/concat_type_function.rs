...
pub unsafe fn concat_type_function(
  ...
  let (Some(norm_lhs_ty_ref), Some(norm_rhs_ty_ref)) = (
    unsafe { (*ctx_ref.normalizer.as_ptr()).try_normalize(lhs_ty) },
    unsafe { (*ctx_ref.normalizer.as_ptr()).try_normalize(rhs_ty) },
  ) else {
