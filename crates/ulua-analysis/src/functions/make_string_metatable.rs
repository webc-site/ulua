...
use crate::{
  ...
  functions::{
    ...
    attach_magic_function::attach_magic_function, get_mutable_type,
    make_function_builtin_definitions::make_function_type_arena_optional_type_id_initializer_list_type_id_initializer_list_type_pack_id_initializer_list_type_id_initializer_list_type_id_bool as make_function_poly,
  },
  methods::{
  ...
  records::{
    ...
    type_arena::TypeArena, type_pack::TypePack, union_type::UnionType,
    variadic_type_pack::VariadicTypePack,
  },
  type_aliases::{props_type::Props, type_id::TypeId, type_pack_id::TypePackId},
};

// B 型（arena 结点布局契约）：下方各 `TableType` 字面量的 `scope` 置空指针，实义
// 为「内建类型无量化作用域」（cpp Type.h:511 字段默认 `Scope* scope = nullptr`，
// BuiltinTypes 构造期即如此）；消费方判空后跳过 scope 相关分支，接口导出时
// clone_public_interface 亦将其清回无作用域态。
...
pub fn make_string_metatable(builtin_types: NonNull<BuiltinTypes>, mode: SolverMode) -> TypeId {
  ...
  // Safety: NonNull 句柄指向上面的活 BuiltinTypes 单例；共享引用仅用于读取
  // nil/number/boolean/string/any/unknown 等 Copy TypeId 字段，BuiltinTypes
  // 在构造期字段不被并发改写（本函数自身只写 arena 堆块，不写结构体）。
  ...
  let mut format_ftv = FunctionType::function_type_new(
    arena.add_type_pack_t(TypePack::new(vec![string_type], Some(variadic_tail_pack))),
    one_string_pack,
    None,
    false,
  );
  ...
  let mut find_func_ty = FunctionType::function_type_new(
    ...
    arena.add_type_pack_t(TypePack::new(
      vec![optional_number, optional_number],
      Some(string_variadic_list),
    )),
    None,
    false,
  );
  ...
  let mut string_dot_unpack = FunctionType::function_type_new(
    arena.add_type_pack_t(TypePack::from_vec(vec![
      string_type,
      string_type,
      optional_number,
    ])),
  ...
  if let Some(ttv) = get_mutable_type::get_mutable::<TableType>(table_type) {
