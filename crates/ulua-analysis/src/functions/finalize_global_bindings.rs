...
use crate::{
  functions::{get_mutable_type, persist_type::persist, to_string_symbol::to_string_symbol},
...
pub fn finalize_global_bindings(scope: ScopePtr) {
  for (symbol, binding) in scope.bindings.iter() {
    ...
    if let Some(ttv) = get_mutable_type::get_mutable::<TableType>(binding.type_id)
      && ttv.name.is_none()
    {
