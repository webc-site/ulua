use crate::{
  functions::{flatten_type_pack::flatten_type_pack_id, follow_type_pack, get_type_pack},
...
pub fn try_get_type_pack_type_at(tp: TypePackId, index: usize) -> Option<TypeId> {
  ...
  let follow_tp = follow_type_pack::follow(tp_tail_id);
  get_type_pack::get::<VariadicTypePack>(follow_tp).map(|vtp| vtp.ty)
}
