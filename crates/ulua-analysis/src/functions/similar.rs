  ...
  unsafe {
    match lhs_ref.base.class_index {
      <AstExprGroup as AstNodeClass>::CLASS_INDEX => {
        ...
        // expr 已句柄化恒非空；similar 递归为既有裸指针 API，经 as_ptr 桥接。
        similar(le.expr.as_ptr(), re.expr.as_ptr())
      }
      ...
      <AstExprIndexName as AstNodeClass>::CLASS_INDEX => {
        ...
        // expr 已句柄化恒非空；similar 递归为既有裸指针 API，经 as_ptr 桥接。
        le.index == re.index && similar(le.expr.as_ptr(), re.expr.as_ptr())
      }
      <AstExprIndexExpr as AstNodeClass>::CLASS_INDEX => {
        let le = &*lhs.cast::<AstExprIndexExpr>();
        let re = &*rhs.cast::<AstExprIndexExpr>();
        // expr/index 已句柄化恒非空；similar 递归为既有裸指针 API，经 as_ptr 桥接。
        similar(le.expr.as_ptr(), re.expr.as_ptr()) && similar(le.index.as_ptr(), re.index.as_ptr())
      }
      ...
      <AstExprUnary as AstNodeClass>::CLASS_INDEX => {
        ...
        // expr 已句柄化恒非空；similar 递归为既有裸指针 API，经 as_ptr 桥接。
        le.op == re.op && similar(le.expr.as_ptr(), re.expr.as_ptr())
      }
      <AstExprBinary as AstNodeClass>::CLASS_INDEX => {
        let le = &*lhs.cast::<AstExprBinary>();
        let re = &*rhs.cast::<AstExprBinary>();
        // left/right 已句柄化恒非空；similar 递归为既有裸指针 API，经 as_ptr 桥接。
        le.op == re.op
          && similar(le.left.as_ptr(), re.left.as_ptr())
          && similar(le.right.as_ptr(), re.right.as_ptr())
      }
      ...
      <AstExprIfElse as AstNodeClass>::CLASS_INDEX => {
        ...
        // 三子节点已句柄化恒非空；similar 递归为既有裸指针 API，经 as_ptr 桥接。
        similar(le.condition.as_ptr(), re.condition.as_ptr())
          && similar(le.true_expr.as_ptr(), re.true_expr.as_ptr())
          && similar(le.false_expr.as_ptr(), re.false_expr.as_ptr())
      }
