...
pub fn snapshot_scope(scope: &Scope, opts: &mut ToStringOptions) -> ScopeSnapshot {
  ...
  for (symbol, binding) in &scope.bindings {
    ...
    let key = unsafe {
      // Safety: `symbol.local` 为 `*mut AstLocal`（Symbol 对全局名置 null），`is_null()` 分支
      // 保证仅在其非空时才解引用读取 `name`（`AstName` Copy 字段）；`as_str_or_empty` 亦容忍空名。
      // 全程单线程只读，无别名冲突。
      let name = if !symbol.local.is_null() {
        (*symbol.local).name
      } else {
        symbol.global
      };
