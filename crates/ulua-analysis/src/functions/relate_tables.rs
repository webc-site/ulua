...
pub fn relate_tables(
  ...
  if right_table.props.len() == 1 && right_table.indexer.is_none() {
    let (name, prop) = right_table
      .props
      .iter()
      .next()
      .expect("len()==1 蕴含存在首元素");
  ...
  let left_indexer = left_table
    .indexer
    .as_ref()
    .expect("上方 is_none 分支已 return，left 必为 Some");
  let right_indexer = right_table
    .indexer
    .as_ref()
    .expect("上方两侧 indexer 有无一致性判据蕴含同为 Some");
