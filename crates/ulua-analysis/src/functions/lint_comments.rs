...
pub fn lint_comments(context: &mut LintContext, hotcomments: &[HotComment]) {
  ...
  for hc in hotcomments {
    ...
    if !hc.header {
      ...
      // content[pos..] 中首个非空白字符的绝对偏移；None 表示无参数或全空白
      let first_nonspace = |pos: usize| {
        hc.content[pos..]
          .find(|c: char| c != ' ' && c != '\t')
          .map(|p| pos + p)
      };
      ...
      if first == "nolint" {
        let notspace_pos = space_pos.and_then(first_nonspace);
      ...
      } else if matches!(first, "nocheck" | "nonstrict" | "strict") {
