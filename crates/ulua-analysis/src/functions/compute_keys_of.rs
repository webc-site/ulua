...
use crate::{
  functions::{
    find_metatable_entry::find_metatable_entry, follow_type, get_type, is_string::is_string,
  },
  records::{
    arena_handle::Handle, extern_type::ExternType, metatable_type::MetatableType,
    primitive_type::PrimitiveType, table_type::TableType,
...
pub fn compute_keys_of(
  ...
  if let Some(table_ty) = get_type::get::<TableType>(ty) {
  ...
  if let Some(metatable_ty) = get_type::get::<MetatableType>(ty) {
    let mut res = true;
  ...
  if let Some(extern_ty) = get_type::get::<ExternType>(ty) {
