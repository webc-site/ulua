...
use ulua_vm::{
  ...
  functions::{
    lua_l_checkboolean::lua_l_checkboolean, lua_l_error_l::lua_l_error_l,
    lua_l_typename::lua_l_typename, lua_type::lua_type, lua_typename::lua_typename,
  },
  macros::{
...
pub unsafe fn create_singleton(l: *mut LuaState) -> c_int {
  ...
  if FFlag::LuauUdtfCreateSingletonFixErrorMessage.get() {
    ...
    let type_name = unsafe { CStr::from_ptr(lua_l_typename(vm_l, 1)) }.to_string_lossy();
    unsafe {
      lua_l_error_l(
        vm_l,
        c"%s".as_ptr(),
