...
pub fn autocomplete_type_names(
  ...
  if unsafe { ast_node_is::<AstStatLocal>(&*parent) } {
    ...
    if !node.is_null() {
      for (i, &var) in unsafe { (*node).vars.as_slice() }.iter().enumerate() {
        if var.is_null() {
          continue;
        }
    ...
    for (i, &arg) in unsafe { (*node).args.as_slice() }.iter().enumerate() {
      if arg.is_null() {
        continue;
      }
    ...
    if !explicit.is_null() {
      for (i, &ret) in unsafe { (*explicit).type_list.types.as_slice() }
        .iter()
        .enumerate()
      {
        if ret.is_null() {
