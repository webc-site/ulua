...
use crate::{
  functions::{get_mutable_type, queue_type_pack::queue_type_pack},
  records::{
    ...
    metatable_type::MetatableType, table_type::TableType, r#type::Type, type_arena::TypeArena,
...
pub fn try_unify_with_any(
  ...
  while !queue.is_empty() {
    // while 头 `!queue.is_empty()` 蕴含 last() 命中 Some。
    let ty = state
      .log
      .follow_type_id(*queue.last().expect("while 头 !queue.is_empty() 蕴含非空"));
    queue.pop();

    // SAFETY: ty 指向 TypeArena 内 Type；仅读取 owning_arena 做归属比较。
    if unsafe { (*ty).owning_arena } != type_arena.get().arena_id {
      continue;
    }
    ...
    if get_mutable_type::get_mutable::<FreeType>(ty).is_some() {
      state
        .log
        .replace_type_id_t(ty, Type::new(TypeVariant::Bound(any_type)));
    } else if let Some(fun) = get_mutable_type::get_mutable::<FunctionType>(ty) {
    ...
    } else if let Some(table) = get_mutable_type::get_mutable::<TableType>(ty) {
    ...
    } else if let Some(mt) = get_mutable_type::get_mutable::<MetatableType>(ty) {
      queue.push(mt.table);
      queue.push(mt.metatable);
    } else if get_mutable_type::get_mutable::<ExternType>(ty).is_some() {
      // ExternType 没有需要入队的子类型
    } else if let Some(union_) = get_mutable_type::get_mutable::<UnionType>(ty) {
      queue.extend(union_.options.iter().copied());
    } else if let Some(intersection) = get_mutable_type::get_mutable::<IntersectionType>(ty) {
