...
pub fn dump_cfg_json(cfg: &ControlFlowGraph) -> String {
  ...
  for (i, &block) in cfg.blocks.iter().enumerate() {
    // cfg.blocks 句柄经注册表解析（见 `block_registry` 模块契约，与迁移前的
    // `&*BlockId` 同前提）：仅共享只读借用读前驱列表，`cfg` 全程为 `&` 借用，
    // 单线程无并发写者。
    {
  ...
  for (i, &block) in cfg.blocks.iter().enumerate() {
    // 同一不变量——句柄经注册表解析后为 safe（见 `block_registry` 模块契约），
    // 共享借用只读前驱/后继/指令三张表（指令按 `InstrId` 句柄交 dump_instruction
    // 于注册表内解析），与同层的 `&cfg` 借用均为只读，不存在可变别名，单线程序列化
    // 输出。块作用域仅用于收敛 shadowed 句柄借用的生命周期。
    {
      let block = resolve_block(block).expect("BlockId 为构建期 register_block 发放的存活句柄");
      if i > 0 {
        out.push(',');
