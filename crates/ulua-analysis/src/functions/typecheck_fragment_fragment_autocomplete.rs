...
use crate::{
  ...
  functions::{
    ...
    freeze::freeze, get_module_resolver::get_module_resolver, trace_requires::trace_requires,
    unfreeze::unfreeze,
  },
  methods::{
...
pub unsafe fn typecheck_fragment_(
  ...
  let resolver: *mut dyn ModuleResolver =
    get_module_resolver(frontend, Some(opts.clone())) as *mut _ as *mut dyn ModuleResolver;
