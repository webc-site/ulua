...
/// Mutex 包裹保持 `static` 免 `static mut` 的数据竞争 UB；中毒时沿用内值
/// （对齐 cpp std::function 无中毒语义，见 r1-analysis item 11 的策略）。
...
pub fn set_print_line(pl: Option<PrintLineProc>) {
  *LUAU_PRINT_LINE.lock().unwrap_or_else(|e| e.into_inner()) = pl;
}

/// 当前 print 回调；`None` 表示宿主清空了槽位。
pub fn luau_print_line() -> Option<PrintLineProc> {
  *LUAU_PRINT_LINE.lock().unwrap_or_else(|e| e.into_inner())
}
