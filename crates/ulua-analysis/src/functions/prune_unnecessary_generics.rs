...
use crate::{
  ...
  functions::{
    ...
    follow_type_pack, get_mutable_type, get_tail::get_tail, get_type, get_type_pack,
    invert_polarity::invert,
  },
  records::{
    arena_handle::Handle, builtin_types::BuiltinTypes, counter_state::CounterState,
    ...
    negation_type::NegationType, scope::Scope, table_type::TableType, type_arena::TypeArena,
...
pub unsafe fn prune_unnecessary_generics(
  ...
  if unsafe { (*ty).owning_arena != arena.get().arena_id || (*ty).persistent } {
    return;
  }

  let function_ty = get_mutable_type::get_mutable::<FunctionType>(ty);
  ...
  for generic in function_ty.generics.iter().copied() {
    ...
    if let Some(g) = get_type::get::<GenericType>(generic)
      && !g.explicit_name
    {
  ...
  // 工作表语义：遍历期间 push 进来的 tail 也会被处理（C++ `for` 依赖
  // `size()` 动态求值），因此不能用快照式 for 迭代器，用 get(i) 游标防越界。
  let mut i = 0;
  while let Some(&pack) = function_ty.generic_packs.get(i) {
    let generic_pack = follow_type_pack::follow(pack);
    ...
    if let Some(g) = get_type_pack::get::<GenericTypePack>(tail)
      && !g.explicit_name
    {
  ...
  if !counter.hit_limits {
    for (&generic, state) in counter.generics.iter() {
      if state.count == 1 && state.polarity != Polarity::Mixed {
        ...
        if unsafe { (*generic).owning_arena } != arena.get().arena_id {
          continue;
        }
  ...
  function_ty.generics.retain(|generic| {
    ...
    get_type::get::<GenericType>(generic).is_some()
  });
  ...
  function_ty.generic_packs.retain(|generic_pack| {
    ...
    get_type_pack::get::<GenericTypePack>(generic_pack).is_some()
  });
}
...
impl GenericCounterLocal {
  fn new(_cached_types: *mut DenseHashSet<TypeId>) -> Self {
    // cached_types 在 C++ 侧仅旧 solver 使用，移植保留占位
    Self {
      seen: HashSet::new(),
  ...
  fn dispatch_type_id(&mut self, ty: TypeId) {
    ...
    if let Some(ft) = get_type::get::<FunctionType>(ty) {
    ...
    if let Some(ft) = get_type::get::<FreeType>(ty) {
      self.traverse_type_id(ft.lower_bound);
      self.traverse_type_id(ft.upper_bound);
    } else if let Some(tfit) = get_type::get::<TypeFunctionInstanceType>(ty) {
      let seen_count = self.seen_counts.entry(ty).or_default();
      if *seen_count > 1 {
        return;
      }
    ...
    } else if let Some(ut) = get_type::get::<UnionType>(ty) {
    ...
    } else if let Some(nt) = get_type::get::<NegationType>(ty) {
