...
/// 类型运行期数据。对应 C++ 原生 `static int setFunctionParameters(lua_State* L)`（`cpp/Analysis/src/TypeFunctionRuntime.cpp:1363`）。
pub unsafe fn set_function_parameters(l: *mut LuaState) -> i32 {
  unsafe {
    let vm_l = l as *mut lua_state::LuaState;
    lua_check_args!(vm_l, 1..=3, "type.setparameters: expected 1-3, but got {}");
    ...
    lua_check_not_frozen!(vm_l, self_ty, "type.setparameters");
