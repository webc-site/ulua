...
use crate::{
  ...
  type_aliases::{
    blocked_constraint_id::BlockedConstraintId,
...
pub struct ConstraintSolver {
  // 句柄化：原 C++ NotNull 裸指针字段（构造期由 Frontend 持有的单例接线）。
  ...
  pub uninhabited_type_functions: DenseHashSet<*const ()>,
...
impl ConstraintSolver {
  /// §2 裸指针收口单点：`root_scope` 由构造期经 `NonNull<Scope>` 注入
  /// （C++ `NotNull<Scope>` 直译），与会话同寿且恒非空；解引用只发生在
  /// [`arena_handle::alias_ref`] 一处，业务调用点经本方法取普通引用。
  pub(crate) fn root_scope_ref(&self) -> &Scope {
    alias_ref(self.root_scope)
  }

  /// cpp `ConstraintSolver::moduleResolver` 的读取：字段已是
