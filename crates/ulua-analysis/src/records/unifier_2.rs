...
pub struct Unifier2 {
  ...
  /// C++ `NotNull<InternalErrorReporter> ice`：仅已删除的 occursCheck_DEPRECATED
  /// 读取过，字段按 cpp 布局保留；下划线前缀标注当前无读者。
  pub(crate) _ice: NonNull<InternalErrorReporter>,
  ...
  pub(crate) uninhabited_type_functions: *mut DenseHashSet<*const ()>,
}
