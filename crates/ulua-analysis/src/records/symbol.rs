...
// Safety: `local` 是借用自存活 AST/局部符号 arena 的裸指针，仅作身份比较（`eq`/`hash`
// 只比指针值与 `global` 的字节），从不对它解引用读写；移动 `Symbol` 只搬动这些身份值，
// 不转移 pointee 所有权，故只要被指向对象在该符号存活期内有效即可靠。
unsafe impl Send for Symbol {}
// Safety: `local` 裸指针只被只读访问作身份比较；`global: AstName` 的 `value` 指向全局
