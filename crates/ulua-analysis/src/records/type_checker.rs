...
use crate::{
  records::{
    ...
    frontend_cancellation_token::FrontendCancellationToken, hash_bool_name_pair::HashBoolNamePair,
    instantiation::Instantiation, internal_error_reporter::InternalErrorReporter,
...
pub struct TypeChecker {
  ...
  pub resolver: ModuleResolverRef,
  pub current_module: Option<ModulePtr>,
  pub builtin_types: Handle<BuiltinTypes>, // NotNull<BuiltinTypes>
...
impl TypeChecker {
  /// cpp `TypeChecker::resolver`（`ModuleResolver*`）的读取：字段已是
  /// [`ModuleResolverRef`] 具体分派句柄（`Copy`），无解引用、无 dyn。
  pub fn resolver_ref(&self) -> ModuleResolverRef {
    self.resolver
  }
}
