...
use crate::{
  ...
  records::{
    any_type::AnyType, arena_id::ArenaId, boolean_singleton::BooleanSingleton,
...
impl BuiltinTypes {
  ...
  pub fn new() -> Self {
    ...
    let empty_type_pack =
      add_persistent_pack(arena_ref, TypePackVariant::TypePack(TypePack::empty()));
    ...
    let uninhabitable_type_pack = add_persistent_pack(
      arena_ref,
      TypePackVariant::TypePack(TypePack::new(vec![never_type], Some(never_type_pack))),
    );
