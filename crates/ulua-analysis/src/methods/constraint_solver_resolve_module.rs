...
impl ConstraintSolver {
  pub fn resolve_module(&mut self, info: &ModuleInfo, location: &Location) -> TypeId {
    ...
    let module = unsafe { (*self.module_resolver).get_module(&info.name) };
    if module.is_none() {
      if unsafe { !(*self.module_resolver).module_exists(&info.name) } && !info.optional {
        let human_readable_name =
          unsafe { (*self.module_resolver).get_human_readable_module_name(&info.name) };
