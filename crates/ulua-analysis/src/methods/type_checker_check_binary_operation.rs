...
use crate::{
  ...
  functions::{
    arc_as_mut::arc_as_mut, first::first, follow_type,
    get_identifier_of_base_var_type_infer::get_identifier_of_base_var, get_type,
    op_to_meta_table_entry::op_to_meta_table_entry, to_string_to_string::to_string_type_id,
  ...
  records::{
    ...
    free_type::FreeType, function_type::FunctionType, generic_error::GenericError,
...
impl TypeChecker {
  pub fn check_binary_operation(
    ...
    let lhs_type = follow_type::follow(lhs_type);
    let rhs_type = follow_type::follow(rhs_type);

    if !self.is_nonstrict_mode() && get_type::get::<FreeType>(lhs_type).is_some() {
    ...
    if get_type::get::<FreeType>(rhs_type).is_some() {
    ...
    // left/right 已句柄化：get() 即安全只读借用（parser 保证非空存活）。
    let (left_loc, right_loc) = (
      expr.left.get().base.location,
      expr.right.get().base.location,
    );
  ...
  fn check_binary_operation_metatable_call(
    ...
    if has_errors {
      ...
      let fallback_arguments = self.fresh_type_pack_scope_ptr(scope);
