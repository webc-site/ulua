...
use crate::{
  ...
  functions::{
    arc_as_mut::arc_as_mut, follow_type, get_table_type::get_table_type, get_type,
    maybe_string::maybe_string,
  },
  records::{
    function_type::FunctionType, property_type::Property, table_indexer::TableIndexer,
    table_type::TableType, type_checker::TypeChecker,
  },
...
impl TypeChecker {
  pub fn check_expr_table(
    ...
    if let Some(expected_type) = expected_type
      && let Some(ttv) = get_type::get::<TableType>(follow_type::follow(expected_type))
    ...
    for (i, item) in items.iter().enumerate() {
      ...
      if item.kind == ItemKind::List {
        ...
        if let Some(key_str) = key_str {
          let mut expr_type = follow_type::follow(value_type);
          if self.is_nonstrict_mode()
            && get_table_type(expr_type).is_none()
            && get_type::get::<FunctionType>(expr_type).is_none()
          {
