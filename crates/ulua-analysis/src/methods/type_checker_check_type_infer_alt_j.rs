...
impl TypeChecker {
  pub fn check_scope_ptr_ast_stat_local(
        ...
        if ast_node_try_as::<AstExprTable>(unsafe { &(*rhs).base }).is_some() {
            ...
            && let Some(current_module) = self.current_module.as_ref()
          {
            ...
            if Arc::ptr_eq(scope, &module_scope) {
              // SAFETY: var 指向 AST arena 节点。
              if !unsafe { (*var).name.is_null() } {
                ttv.synthetic_name = Some(unsafe { (*var).name.as_str_or_empty().to_string() });
              }
            }
          }
        } else {
            ...
            && let Some(mtv) = get_mutable_type_id::<MetatableType>(follow_type_id(ty))
          {
            // SAFETY: var 指向 AST arena 节点。
            if !unsafe { (*var).name.is_null() } {
              mtv.synthetic_name = Some(unsafe { (*var).name.as_str_or_empty().to_string() });
            }
          }
        }
      }
    ...
    for (&var, &value) in vars.iter().zip(values.iter()) {
      ...
      let module_info =
        unsafe { (*self.resolver).resolve_module_info(&current_module.name, &*require) };
      ...
      let name: Name = unsafe { (*var).name.as_str_or_empty().to_string() };

      // SAFETY: 同 resolver 契约。
      unsafe {
        if let Some(module) = (*self.resolver).get_module(&module_info.name) {
          scope_mut
