...
impl TypeChecker {
  pub fn check_require(
    ...
    let module = unsafe { (*self.resolver).get_module(&module_info.name) };
    if module.is_none() {
      ...
      if unsafe { !(*self.resolver).module_exists(&module_info.name) } && !module_info.optional {
        let human_readable_name =
          unsafe { (*self.resolver).get_human_readable_module_name(&module_info.name) };
        let error = TypeError::type_error_location_type_error_data(
          *location,
