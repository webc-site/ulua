...
impl TypeChecker2 {
  pub fn check_index_type_from_type(
    ...
    let table_ty = follow_type::follow(table_ty);

    let mut has_cyclic_union = false;
    let mut union_seen: DenseHashSet<TypeId> = DenseHashSet::default();
    let mut union_stack = Vec::from([table_ty]);
    while let Some(candidate) = union_stack.pop() {
      let candidate = follow_type::follow(candidate);
      let Some(union) = get_type::get::<UnionType>(candidate) else {
        continue;
      };
      ...
      for &option in &union.options {
        if get_type::get::<UnionType>(follow_type::follow(option)).is_some() {
    ...
    if !has_cyclic_union {
      ...
      let Some(norm) = self.normalizer.try_normalize(table_ty) else {
    ...
    if let Some(union) = get_type::get::<UnionType>(table_ty) {
      let mut found_prop = false;
      let mut missing_prop = Vec::new();
      let mut hit_limits = false;

      // C++ `for (TypeId option : union)` — UnionTypeIterator 防环并展平嵌套 union。
      for option in begin_union_type(union) {
        let mut option_seen = DenseHashSet::default();
    ...
    if context == ValueContext::LValue {
      let mut seen_read = DenseHashSet::default();
      ...
      } else if get_type::get::<PrimitiveType>(table_ty).is_some()
        || get_type::get::<FunctionType>(table_ty).is_some()
      {
        self.report_error_type_error_data_location(
          TypeErrorData::NotATable(NotATable { ty: table_ty }),
          &location,
        );
      } else if let Some(extern_type) = get_type::get::<ExternType>(table_ty) {
      ...
      let mut seen_write = DenseHashSet::default();
      let mut dummy = Vec::new();
