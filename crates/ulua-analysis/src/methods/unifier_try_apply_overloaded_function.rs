...
use crate::{
  functions::{get_type, has_unification_too_complex::has_unification_too_complex},
  records::{
...
impl Unifier {
  ...
  pub fn unifier_try_apply_overloaded_function(
    ...
    if overloads.is_never() {
      ...
      return self.builtin_types_ref().error_type_pack;
    }
    ...
    for overload in parts {
      let Some(ftv) = get_type::get::<FunctionType>(overload) else {
        continue;
      };
      ...
      if inner_state.errors.is_empty() {
        ...
        if let Some(res) = result {
          ...
          // 泛型 type pack 不支持交集，交集可能失败；此时任意地取第一个匹配的 overload
            ...
            .normalizer_mut()
            .intersection_of_type_packs(res, ftv.ret_types)
          {
        ...
        return self.builtin_types_ref().error_recovery_type_pack(args);
      }
    }
    ...
    if let Some(res) = result {
      ...
      self
        .builtin_types_ref()
        .error_recovery_type_pack(first_fun.ret_types)
    } else {
      self.report_error_location_type_error_data(
        self.location,
        TypeErrorData::CannotCallNonFunction(CannotCallNonFunction { ty: function }),
      );
      self.builtin_types_ref().error_type_pack
    }
  }
}
