...
use crate::{
  ...
  records::{
    error_converter::ErrorConverter, never_type::NeverType,
        ...
        (given_definition_module, wanted_definition_module)
      {
        if let Some(file_resolver) = self.file_resolver {
          ...
          let given_module_name =
            unsafe { (*file_resolver).get_human_readable_module_name(given_mod) };
          let wanted_module_name =
            unsafe { (*file_resolver).get_human_readable_module_name(wanted_mod) };
