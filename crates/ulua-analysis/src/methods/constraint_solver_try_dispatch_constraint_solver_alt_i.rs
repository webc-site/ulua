...
use crate::{
  ...
  functions::{
    extend_type_pack::extend_type_pack, find_blocked_arg_types_in::find_blocked_arg_types_in,
...
impl ConstraintSolver {
  ...
  pub unsafe fn try_dispatch_function_check_constraint_not_null_constraint_bool(
    ...
    let args_slice = unsafe { (*c.call_site).args.as_slice() };
    ...
    let subtyping = NonNull::new(self.subtyping).unwrap();
