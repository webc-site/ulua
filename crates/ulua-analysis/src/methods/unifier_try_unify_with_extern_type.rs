...
use crate::{
  ...
  functions::{
    get_mutable_type, get_type, is_subclass_type::is_subclass_extern_type_extern_type,
    lookup_extern_type_prop::lookup_extern_type_prop,
  },
  records::{
    extern_type::ExternType, generic_error::GenericError, table_type::TableType, unifier::Unifier,
...
impl Unifier {
  pub fn unifier_try_unify_with_extern_type(
    ...
    let Some(super_extern_type) = get_type::get::<ExternType>(super_ty) else {
      self.ice_string("tryUnifyExternType invoked with non-class Type");
      return;
    };

    if let Some(sub_extern_type) = get_type::get::<ExternType>(sub_ty) {
      match self.variance {
        Variance::Covariant => {
  ...
  fn report_extern_type_mismatch(&mut self, sub_ty: TypeId, super_ty: TypeId, reversed: bool) {
    self.unifier_report_type_mismatch(
      if reversed { sub_ty } else { super_ty },
      if reversed { super_ty } else { sub_ty },
    );
  }
