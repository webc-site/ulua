...
use crate::{
  ...
  functions::{flatten_type_pack::flatten_type_pack_id, follow_type, get_mutable_type, get_type},
  records::{
...
impl Unifier2 {
  pub fn unify_free_with_type(&mut self, sub_ty: TypeId, super_ty: TypeId) -> UnifyResult {
    ...
    let sub_free = get_mutable_type::get_mutable::<FreeType>(sub_ty)
      .expect("C++ Unifier2.cpp:321 LUAU_ASSERT(subFree)：入口处断言 sub_ty 为 FreeType");
    let upper_bound = follow_type::follow(sub_free.upper_bound);

    if get_type::get::<FunctionType>(upper_bound).is_some() {
      // C++ 用未 follow 的 upperBound
      let existing_upper = get_mutable_type::get_mutable::<FreeType>(sub_ty)
        .expect("同函数入口判据：sub_ty 为 FreeType 且未改写变体")
        .upper_bound;
    ...
    if fflag::LuauPropagateFreeTypesIntoUnionAndIntersectionBounds.get() {
      ...
      if let Some(super_intersection) = get_type::get::<IntersectionType>(super_ty) {
    ...
    let Some(super_function) = get_type::get::<FunctionType>(super_ty) else {
      return self.do_default_unify_free(sub_ty, super_ty);
    };
    ...
    let Some(upper_bound_intersection) = get_type::get::<IntersectionType>(upper_bound) else {
      return self.do_default_unify_free(sub_ty, super_ty);
    };
    ...
    for part in &upper_bound_intersection.parts {
      let Some(ft) = get_type::get::<FunctionType>(follow_type::follow(*part)) else {
        continue;
      };
  ...
  fn do_default_unify_free(&mut self, sub_ty: TypeId, super_ty: TypeId) -> UnifyResult {
    ...
    let sub_free = get_mutable_type::get_mutable::<FreeType>(sub_ty)
      .expect("调用方 unify_free_with_type 入口已断言 sub_ty 为 FreeType");
  ...
  fn propagate_to_free_members(&mut self, member_range: &[TypeId], sub_ty: TypeId) {
    for &member in member_range {
      let mut m = follow_type::follow(member);
      if let Some(subst) = self.generic_substitutions.find(&m) {
        m = follow_type::follow(*subst);
      }
      if let Some(member_free) = get_mutable_type::get_mutable::<FreeType>(m) {
