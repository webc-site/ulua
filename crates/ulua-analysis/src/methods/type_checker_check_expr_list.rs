...
use crate::{
  functions::{
    arc_as_mut::arc_as_mut, contains_never::contains_never, first::first, follow_type,
    get_mutable_type_pack, get_type,
  },
  records::{
    never_type::NeverType, txn_log::TxnLog, type_checker::TypeChecker, type_pack::TypePack,
...
impl TypeChecker {
  pub fn check_expr_list(
    ...
    let pack = self.add_type_pack_type_pack_var(TypePackVar::from(TypePack::empty()));
    ...
    let tp = get_mutable_type_pack::get_mutable::<TypePack>(pack)
      .expect("pack 刚以 TypePack 变体分配，下转必命中");
      ...
      if i == last_index && is_call_or_varargs {
        ...
        if contains_never(type_pack) {
          ...
          if module.ast_types.find(&key).is_none() {
            *module.ast_types.get_or_insert(key) = follow_type::follow(first_ty);
          }
        }
        ...
        let result = self.check_expr(scope, expr_ref, expected_type, false);
        let r#type = result.r#type;
        predicates.extend(result.predicates);

        if get_type::get::<NeverType>(r#type).is_some() {
          // f(), g() where f() returns (never, string) or (string, never) means this whole TypePackId is uninhabitable, so return (never,
          // ...never)
          uninhabitable = true;
          continue;
