use ulua_ast::records::{
  ...
  ast_stat_block::AstStatBlock, ast_stat_expr::AstStatExpr, ast_stat_if::AstStatIf,
  ast_stat_local::AstStatLocal, ast_stat_while::AstStatWhile,
};
...
impl CfgBuilder {
  pub fn lower_ast_stat(&mut self, statement: *mut AstStat) {
    unsafe {
      ...
      } else if (*p).is::<AstStatExpr>() {
        self.lower_ast_stat_expr(statement as *mut AstStatExpr);
      } else {
        LUAU_ASSERT!(false);
      }
    }
