...
impl TypeChecker2 {
  pub fn type_checker_2_index_expr_metatable_helper(
    ...
    let table_followed = follow_type::follow(meta_table.table());
    if let Some(tt) = get_type::get::<TableType>(table_followed)
    ...
    if let Some(mt) = get_type::get::<MetatableType>(table_followed) {
      self.type_checker_2_index_expr_metatable_helper(index_expr, mt, expr_type, index_type);
      return;
    }

    let metatable_followed = follow_type::follow(meta_table.metatable());
    if let Some(tmt) = get_type::get::<TableType>(metatable_followed)
    ...
    if let Some(mtmt) = get_type::get::<MetatableType>(metatable_followed) {
