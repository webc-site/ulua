...
impl OverloadResolver<'_> {
  pub fn test_function_or_union(
    ...
    LUAU_ASSERT!(fn_ty == follow_type::follow(fn_ty));

    if let Some(ut) = get_type::get::<UnionType>(fn_ty) {
      ...
      // C++ `for (TypeId t : ut)`——UnionTypeIterator 展平嵌套 union 并
      // follow,裸遍历 options 会漏掉嵌套成员。
      let mut count: usize = 0;
      for t in begin_union_type(ut) {
        count += 1;
