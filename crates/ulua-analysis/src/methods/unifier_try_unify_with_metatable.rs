...
use crate::{
  ...
  functions::{
    get_mutable_type, get_type, has_unification_too_complex::has_unification_too_complex,
  },
  records::{
    any_type::AnyType, metatable_type::MetatableType, table_type::TableType, unifier::Unifier,
  },
...
impl Unifier {
  pub fn unifier_try_unify_with_metatable(
    ...
    let Some(super_metatable) = get_type::get::<MetatableType>(super_ty) else {
    ...
    if let Some(sub_metatable) = get_mutable_type::get_mutable::<MetatableType>(sub_ty) {
      ...
      if let Some(e) = has_unification_too_complex(&inner_state.errors) {
        ...
        self.unifier_report_type_mismatch_ext(
          ...
          given,
          String::new(),
          Some(inner_state.errors[0].clone()),
        );
      }

      self.log.concat(inner_state.log);
      self.failure |= inner_state.failure;
    } else if let Some(sub_table) = get_mutable_type::get_mutable::<TableType>(sub_ty) {
      match sub_table.state {
        ...
        TableState::Sealed | TableState::Unsealed | TableState::Generic => {
          self.unifier_report_type_mismatch(wanted, given);
        }
      }
    } else if get_mutable_type::get_mutable::<AnyType>(sub_ty).is_some()
      || get_mutable_type::get_mutable::<ErrorType>(sub_ty).is_some()
    {
    } else {
      self.unifier_report_type_mismatch(wanted, given);
    }
