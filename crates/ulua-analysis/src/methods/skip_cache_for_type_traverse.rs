...
use crate::{
  ...
  type_aliases::{
    collections::HashSet, type_id::TypeId, type_pack_id::TypePackId,
    type_pack_variant::TypePackVariant, type_variant::TypeVariant,
  },
};

impl SkipCacheForType {
  /// SAFETY: 前置条件与 `traverse_skip_cache` 相同（ty 须指向合法 Type），由其
  /// # Safety 契约传递；集中在此，避免逐递归调用点重复 unsafe 块。
  #[inline]
  fn recurse_ty(
    &mut self,
    ty: TypeId,
  ...
  pub unsafe fn traverse_skip_cache(
    ...
    match unsafe { &(*ty).ty } {
      TypeVariant::Bound(bound_to) => {
        ...
        let target = *bound_to;
        self.recurse_ty(target, seen_types, seen_packs);
      }
      TypeVariant::Free(ft) => {
