  ...
  pub unsafe fn check_function_signature(
    ...
    if fflag::DebugLuauUserDefinedClasses.get() {
      if !enclosing_class.is_null() && fn_ref.args.size > 0 {
        ...
        let arg_name = first_arg.name.as_str_or_empty();
      ...
      if has_self {
        ...
        let arg_name = unsafe { (*self_local).name.as_str_or_empty() };
      ...
      if !fn_ref.self_.is_null() {
        ...
        arg_types.push(self_type);
        let arg_name = unsafe { (*fn_ref.self_).name.as_str_or_empty() };
        arg_names.push(Some(FunctionArgument {
          name: arg_name.to_string(),
    ...
    for (i, &local_ptr) in fn_ref.args.as_slice().iter().enumerate() {
