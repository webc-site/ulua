...
use ulua_ast::{
  records::{
    ast_expr_call::AstExprCall, ast_expr_table::AstExprTable, ast_node::AstNode,
...
impl ConstraintGenerator {
  ...
  pub unsafe fn visit_scope_ptr_ast_stat_local(
    ...
    for &local in stat_local_ref.vars.as_slice().iter() {
    ...
    for (i, &local) in stat_local_ref.vars.as_slice().iter().enumerate() {
      ...
      && !has_annotation
    {
      ...
      if should_name {
        let name = unsafe { (*var).name.as_str_or_empty().to_string() };
        self.add_constraint_scope_ptr_location_constraint_v(
          scope,
    ...
    if stat_local_ref.values.size > 0 {
      ...
      for (value, &local) in stat_local_ref
        .values
        .as_slice()
        .iter()
        .zip(stat_local_ref.vars.as_slice().iter())
      {
        ...
        let module_info =
          unsafe { (*self.module_resolver).resolve_module_info(&module_name, &*require) };
        let Some(module_info) = module_info else {
          continue;
        };

        let required_module = unsafe { (*self.module_resolver).get_module(&module_info.name) };
        let Some(required_module) = required_module else {
          continue;
        };

        let name = unsafe { (*local).name.as_str_or_empty().to_string() };
        unsafe {
          (*scope_raw)
