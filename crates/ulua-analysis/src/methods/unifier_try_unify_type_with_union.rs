...
use crate::{
  ...
  records::{
    normalization_too_complex::NormalizationTooComplex, type_error::TypeError,
    type_mismatch::TypeMismatch, unifier::Unifier, union_type::UnionType,
  },
...
impl Unifier {
  ...
  pub unsafe fn unifier_try_unify_type_with_union(
    ...
    if let Some(e) = unification_too_complex {
      ...
      let sub_norm = unsafe { (*self.normalizer).try_normalize(sub_ty) };
      let super_norm = unsafe { (*self.normalizer).try_normalize(super_ty) };
