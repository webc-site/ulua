...
use crate::{
  functions::{
    arc_as_mut::arc_as_mut, begin_type::begin_union_type, flatten_type_pack::flatten_type_pack_id,
    follow_type, follow_type_pack, get_type, get_type_pack, is_prim::is_nil,
...
impl TypeChecker {
  pub fn check_function_signature(
    ...
    if let Some(et) = expected_type {
      let et = follow_type::follow(et);
      if let Some(ftv) = get_type::get::<FunctionType>(et) {
        expected_function_type = Some(ftv);
      } else if let Some(utv) = get_type::get::<UnionType>(et) {
        // C++ `for (auto option : utv)`——UnionTypeIterator 展平嵌套 union
        // 并 follow，裸遍历 options 会漏掉嵌套成员。
        for option in begin_union_type(utv) {
          if let Some(ftv) = get_type::get::<FunctionType>(follow_type::follow(option)) {
    ...
    let generic_defs = self.create_generic_types_nodes(
      &fun_scope,
      None,
      &expr.base.base,
      &expr.generics,
    ...
    let ret_pack = if let Some(ann) = expr.return_annotation.as_ref() {
      ...
      self.any_type_pack
    } else if let Some(ftv) =
      expected_function_type.filter(|ftv| ftv.generics.is_empty() && ftv.generic_packs.is_empty())
    {
      // 双写合一：`is_some_and` 判定与 `unwrap()` 并为 `filter` + `let Some`
      // （Option<&FunctionType> 为 Copy，原句柄照旧可用）。
      ...
      let (head, tail) = flatten_type_pack_id(ftv.ret_types);
      ...
      if tail.is_none() && head.len() == 1 && is_nil(head[0]) {
        self.fresh_type_pack_scope_ptr(&fun_scope)
      } else {
    ...
    if expr.vararg {
      if let Some(vararg_annotation) = expr.vararg_annotation.get() {
        // funScope->varargPack = resolveTypePack(funScope, *expr.varargAnnotation);
        vararg_pack = Some(
          self.resolve_type_pack_scope_ptr_ast_type_pack(fun_scope.clone(), vararg_annotation),
        );
      } else {
          ...
          && !self.is_nonstrict_mode()
        {
          ...
          if expr.args.len() <= head.len() {
            // head.erase(head.begin(), head.begin() + expr.args.size);
            head.drain(0..expr.args.len());
            ...
            if get_type_pack::get::<VariadicTypePack>(followed).is_some() {
              vararg_pack = Some(
        ...
        vararg_pack.get_or_insert(self.any_type_pack);
      }
    ...
    if let Some(self_local) = expr.self_.get() {
      ...
      unsafe {
        (*scope_mut(&fun_scope)).bindings.insert(
          Symbol::from_local(expr.self_.as_ptr()),
          Binding {
            type_id: self_type,
    ...
    for (i, local_node) in expr.args.iter_nodes().enumerate() {
      ...
      let arg_type = if let Some(ann) = unsafe { local.annotation.as_ref() } {
        ...
        if get_type::get::<ErrorType>(follow_type::follow(resolved)).is_some() {
      ...
      unsafe {
        (*scope_mut(&fun_scope)).bindings.insert(
          Symbol::from_local(local_node.as_ptr()),
          Binding {
            type_id: arg_type,
    ...
    for local in expr.args.iter() {
      function_type.arg_names.push(Some(FunctionArgument {
        name: local.name.as_str_or_empty().to_string(),
        location: local.location,
      }));
    }
