...
impl OverloadResolver {
  ...
  fn arg_location_at(
    ...
    if let Some(idx) = idx
      && idx < arg_exprs.len()
    {
      // SAFETY: arg_exprs 元素指向 AST arena 节点。
  ...
  pub fn report_errors(
    ...
    // SAFETY: self.builtin_types 由构造方保证有效（C++ 同契约）。
    ...
    if let Some(fsp) = failed_super_pack
      && get_type_pack_id::<GenericTypePack>(fsp).is_some()
    {
    ...
    if let Some(idx) = argument_index {
      arg_location = Self::arg_location_at(arg_exprs, Some(idx), fn_location);
