...
use crate::{
  ...
  functions::{
    ...
    are_eq_comparable::are_eq_comparable,
  ...
  records::{
    ...
    function_type::FunctionType, generic_error::GenericError, never_type::NeverType,
    primitive_type::PrimitiveType, singleton_type::SingletonType, type_checker::TypeChecker,
...
impl TypeChecker {
  pub fn check_relational_operation(
    ...
    // left 已句柄化：get() 只读借用即 repr(C) 基类视图（base 偏移 0 由类型端保证）。
    let subexp = ast_node_try_as::<AstExprBinary>(&expr.left.get().base);
      ...
      && subexp.op == AstExprBinaryOp::And
    {
      ...
      let right_ty = self
        .check_expr(
          &sub_scope,
          // right 已句柄化：get() 只读借用出自存活 &AstExprBinary。
          subexp.right.get(),
          None,
          false,
        )
    ...
    match expr.op {
      ...
      | AstExprBinaryOp::CompareLe => {
        ...
        if get_type::get::<NeverType>(lhs_type).is_some()
          || get_type::get::<NeverType>(rhs_type).is_some()
        {
          return self.boolean_type;
        }
        ...
        let left_type = follow_type::follow(lhs_type);
        if get_type::get::<PrimitiveType>(left_type).is_some()
          || get_type::get::<AnyType>(left_type).is_some()
          || get_type::get::<ErrorType>(left_type).is_some()
          || get_type::get::<UnionType>(left_type).is_some()
        {
        ...
        let left_metatable: Option<TypeId> = if is_string(lhs_type) {
          ...
          get_metatable_type_id_not_null_builtin_types(left_type, self.builtin_types.get())
        };
        let right_metatable: Option<TypeId> = if is_string(rhs_type) {
          string_no_mt
        } else {
          get_metatable_type_id_not_null_builtin_types(
            follow_type::follow(rhs_type),
        ...
        if left_metatable != right_metatable {
          ...
          if is_equality {
            if let Some(utv) = get_type::get::<UnionType>(left_type)
              && right_metatable.is_some()
            {
              // C++ `for (TypeId leftOption : utv)` — UnionTypeIterator 防环展平并 follow。
              for left_option in begin_union_type(utv) {
                if get_metatable_type_id_not_null_builtin_types(
                  follow_type::follow(left_option),
                  self.builtin_types.get(),
                ) == right_metatable
                {
                  matches = true;
            ...
            if !matches
              && let Some(utv) = get_type::get::<UnionType>(rhs_type)
              && left_metatable.is_some()
            {
              // C++ `for (TypeId rightOption : utv)` — 同上。
              for right_option in begin_union_type(utv) {
                if get_metatable_type_id_not_null_builtin_types(
                  follow_type::follow(right_option),
                  self.builtin_types.get(),
                ) == left_metatable
                {
                  matches = true;
        ...
        if get_type::get::<FreeType>(follow_type::follow(lhs_type)).is_some() && !is_equality {
          // left 已句柄化；get_identifier_of_base_var 为既有裸指针 API，经 as_ptr 桥接。
      ...
      AstExprBinaryOp::And => {
        if lhs_is_any {
          ...
          if get_type::get::<FreeType>(lhs_type).is_some() {
          ...
          if not_never {
            let oty = oty.expect("cpp 不变量：not_never 蕴含 pick_types_from_sense 产出 Some");
  ...
  fn relational_strip_nil(&mut self, ty: TypeId, is_or_op: bool) -> TypeId {
    ...
    if get_type::get::<UnionType>(ty).is_some() {
      ...
      match cleaned {
        ...
        Some(c) => return follow_type::follow(c),
      }
    }
  ...
  fn is_boolean_singleton(&self, ty: TypeId) -> bool {
    get_type::get::<SingletonType>(follow_type::follow(ty)).is_some_and(|stv| {
