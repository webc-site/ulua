...
use crate::{
  ...
  records::{
    constraint_generator::ConstraintGenerator, constraint_graph::ConstraintGraph,
    data_flow_graph_builder::DataFlowGraphBuilder, frontend::Frontend,
    internal_error_reporter::InternalErrorReporter, module::Module,
    module_resolver::ModuleResolver, normalizer::Normalizer,
    null_module_resolver::NullModuleResolver, scope::Scope, type_arena::TypeArena,
...
impl Frontend {
  pub fn parse_type(
    ...
    // 具体解析器 → trait 对象引用，再取 NonNull（引用必非空，免裸指针二次转换）
    let resolver: &mut dyn ModuleResolver = &mut module_resolver;
    ...
    let mut cg = ConstraintGenerator::new(ConstraintGeneratorArgs {
      module,
      normalizer: NonNull::from(&mut normalizer),
      type_function_runtime: NonNull::from(&mut type_function_runtime),
      module_resolver: NonNull::from(resolver),
