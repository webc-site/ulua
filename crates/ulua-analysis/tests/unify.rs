...
impl UnifyFixture {
  fn new() -> Self {
    ...
    let normalizer = Box::new(Normalizer::new(
      ...
      Handle::from_mut(&mut *builtin),
      Some(Handle::from_mut(&mut *shared)),
      SolverMode::Old,
      false,
    ));
    Self {
  ...
  fn unifier(&mut self) -> Unifier {
    Unifier {
      types: Handle::from_mut(&mut *self.arena),
      builtin_types: Handle::from_mut(&mut *self.builtin),
      normalizer: Handle::from_mut(&mut *self.normalizer),
      scope: Handle::from_mut(&mut *self.scope),
      log: TxnLog::new(),
      failure: false,
      errors: Vec::new(),
      ...
      shared_state: Handle::from_mut(&mut *self.shared),
...
fn primitive_wanted_given_are_reported() {
  ...
  assert_eq!(follow_type::follow(tm.wanted_type), s);
  assert_eq!(follow_type::follow(tm.given_type), n);
}
