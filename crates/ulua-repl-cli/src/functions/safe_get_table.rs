...
use crate::functions::complete_partial_matches::MAX_TRAVERSAL_LIMIT;

/// # Safety
///
/// `l` must be a valid, active pointer to a `lua_State`.
/// `table_index` must refer to a table on the stack, and the search key must be at the top of the stack.
