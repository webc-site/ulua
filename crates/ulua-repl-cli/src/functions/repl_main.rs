...
use ulua_cli_lib::functions::{
  assertion_handler::install_assertion_handler, atoi::atoi,
...
use crate::{
  functions::{
    ...
    coverage_init::coverage_init, display_help::display_help, profiler_dump::profiler_dump,
    profiler_start::profiler_start, profiler_stop::profiler_stop, run_file::run_file,
    run_repl::run_repl, setup_state::setup_state,
  },
...
// codegenCold`, `static bool jitInliner`, `static int program_argc`, `char** program_argv`.
static REPL_CODEGEN: AtomicBool = AtomicBool::new(false);
static REPL_CODEGEN_COLD: AtomicBool = AtomicBool::new(false);
static REPL_JIT_INLINER: AtomicBool = AtomicBool::new(false);
...
// `static bool jitInliner` — 仅保持 CLI 解析与 cpp 一致;
// `Luau::JitInliner` 尚未移植 (跨 crate 缺口), setupState 暂无对应读取
...
pub fn repl_main(args: &[impl AsRef<str>]) -> i32 {
  ...
  REPL_JIT_INLINER.store(false, Ordering::Relaxed);
  ...
  for (idx, arg) in args.iter().enumerate().skip(1) {
    ...
    if a == "-h" || a == "--help" {
      ...
      REPL_JIT_INLINER.store(true, Ordering::Relaxed);
