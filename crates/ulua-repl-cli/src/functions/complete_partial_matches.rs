...
use ulua_cli_lib::functions::try_replace_top_with_index::try_replace_top_with_index;
...
// Mirrors MaxTraversalLimit in Repl.cpp, safe_get_table 复用
pub(crate) const MAX_TRAVERSAL_LIMIT: i32 = 50;
...
pub unsafe fn complete_partial_matches(
  ...
  edit_buffer: &str,
  prefix: &str,
  add_completion_callback: impl Fn(&str, &str),
) {
  unsafe {
    let mut i = 0;
      ...
      while lua_next(l, -2) != 0 {
        if lua_type(l, -2) == LuaType::String as i32 {
          ...
          if !key.is_empty() && required_value_type && startsWith(&key, prefix) {
            // lossy key 的 prefix.len() 未必落在字符边界上；越界键跳过
            if let Some(completed_component) = key.get(prefix.len()..) {
              let mut completion =
