...
//! Crashed with SIGSEGV on rustc 1.88 before the pack-unification (4b5c7abd)
//! and definition-module retention (eb2c1d6d) fixes — the UB was
//! layout-dependent（`TypePackId` 被重解释成 `TypeId` 跑类型 unify），
//! 所以同样的用例也值得在 Miri 下走一遍，它会报出执行路径上的第一处 UB：
//!
