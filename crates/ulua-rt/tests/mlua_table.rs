...
fn test_table_insert_remove() -> Result<()> {
  ...
  // Wrong index, tables are 1-indexed：越界位置按 mlua raw_insert 契约拒绝
  // （ulua-rt 文案 "position out of bounds"，承 Lua 5.3 ltablib；注意 cpp 的
  // Lua 层 tinsert 不做此检查——这是 rt API 层自己的边界契约）。
  match table4.raw_insert(0, "123") {
    Err(Error::RuntimeError(ref msg)) => {
      assert!(msg.contains("position out of bounds"), "{msg}")
    }
    r => panic!("raw_insert(0) should reject, got {r:?}"),
  }
...
fn test_table_error() -> Result<()> {
  ...
  // __newindex / __index / __len 元方法抛 error("lua error")：三路都必须以
  // RuntimeError 浮出且保留消息（mlua 原版仅 is_err，此处锁错误形态与文案）。
  for (what, err) in [
    ("set", bad_table.set(1, 1).map(|_| ())),
    ("get", bad_table.get::<i32>(1).map(|_| ())),
    ("len", bad_table.len().map(|_| ())),
  ] {
    match err {
      Err(Error::RuntimeError(ref msg)) => assert!(msg.contains("lua error"), "{what}: {msg}"),
      r => panic!("{what}: expected RuntimeError(\"lua error\"), got {r:?}"),
    }
  }
  // raw 路径绕开元方法：写入后原样读回，raw_len 同步（强于两个孤立 is_ok）。
  bad_table.raw_set(1, 1)?;
  assert_eq!(bad_table.raw_get::<i32>(1)?, 1);
