...
#[test]
fn test_script_raised_zero_length_userdata_error() {
  // `newproxy()` raises a userdata whose data area is zero bytes
  // (cpp/VM/src/lbaselib.cpp:419). Recovering a wrapped Rust error must not read
  // the wrapped-error header out of it.
  let lua = Lua::new();
  let err = lua.load("error(newproxy())").exec().unwrap_err();
  assert!(matches!(err, Error::RuntimeError(_)), "got {err:?}");

  let err = lua
    .load("local ud = newproxy(true) error(ud)")
    .exec()
    .unwrap_err();
  assert!(matches!(err, Error::RuntimeError(_)), "got {err:?}");
}
