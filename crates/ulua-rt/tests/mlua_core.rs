...
//
// This pass grew a large slice of mlua's core `Lua` surface in ulua-rt so the
...
// public `ulua_rt::api` surface, and the `Error::RecursiveMutCallback`/
// `MismatchedRegistryKey`/`PreviouslyResumedPanic` variants.
//
// The ONE import-shape difference from mlua: ulua-rt's `Lua::create_table()`
// is infallible (returns `Table`, not `Result<Table>`), so the few `?` after
// `create_table()` are dropped. No test *logic* is changed.
//
// DEFERRED (kept as `*_deferred` pins documenting a genuine ulua deviation):
//   - `test_load_mode`   -> ulua-rt's high-level loader auto-loads text source
//     and has no binary-bytecode `ChunkMode`; the text-eval path is asserted.
//   - `test_panic`       -> ulua-rt catches a Rust panic in a callback and
...
//   - `test_c_function`  -> ulua's `lua_CFunction` is a pure-Rust
//     `unsafe fn(*mut LuaState) -> c_int` (no C ABI), not mlua's
//     `extern "C-unwind" fn`; the deferred pin uses the ulua-native shape.
//   - `test_inspect_stack` -> ulua-rt's `inspect_stack(level) -> Option<Debug>`
...
use ulua_rt::{
  ChunkMode, Error, ExternalError, Function, Lua, LuaOptions, LuaState, Nil, Result, StdLib, Table,
  UserData, Value, Variadic, api,
};
...
fn test_safe_integers() -> Result<()> {
  ...
  // ulua 在 Luau 之上启用了 LUA_TINTEGER 子类型：超出 f64 安全范围的 i64
  // 经 `lua_pushinteger_64` 无损往返（mlua 的纯 Luau 后端在此会丢精度，
  // ulua 有意增强，与 cpp `lapi.cpp:705` 的 lua_pushinteger64 对齐）。
  {
    assert_eq!(f.call::<i64>(MAX_SAFE_INTEGER + 2)?, MAX_SAFE_INTEGER + 2);
    assert_eq!(f.call::<i64>(MIN_SAFE_INTEGER - 2)?, MIN_SAFE_INTEGER - 2);
    assert_eq!(f.call::<i64>(i64::MAX)?, i64::MAX);
    assert_eq!(f.call::<i64>(i64::MIN)?, i64::MIN);
...
fn test_num_conversion() -> Result<()> {
  ...
  // DEVIATION: ulua normalizes `-0.0` to a positive zero (the sign bit is not
  // preserved), whereas mlua/upstream-Luau keep `-0.0`. We pin the actual
  // ulua behavior — the magnitude (`== 0.0`) is identical, only the sign
  // differs.
...
fn test_drop_registry_value() -> Result<()> {
  ...
  // DEVIATION: mlua runs `collectgarbage("collect")` here; ulua's base library
  ...
  // The ulua-rt analog is `Lua::gc_collect`, which runs a full GC cycle.
  lua.gc_collect()?;
...
fn test_lua_registry_hash() -> Result<()> {
  ...
  let r2 = lua.create_registry_value("value2")?;
  let r1_clone = r1.clone();

  let mut h1 = DefaultHasher::new();
  let mut h2 = DefaultHasher::new();
  let mut h1_clone = DefaultHasher::new();
  r1.hash(&mut h1);
  r2.hash(&mut h2);
  r1_clone.hash(&mut h1_clone);

  assert_eq!(h1.finish(), h1_clone.finish());
  assert_ne!(h1.finish(), h2.finish());
  assert_eq!(r1, r1_clone);
  assert_ne!(r1, r2);
...
fn test_registry_value_reuse_deferred() -> Result<()> {
  ...
  // non-nil value, and a `nil` value never consumes a slot). ulua-rt's
...
fn test_application_data() -> Result<()> {
  ...
  if catch_unwind(AssertUnwindSafe(|| lua.app_data_mut::<&str>().unwrap())).is_ok() {
    panic!("expected panic")
  }
...
fn test_traceback_deferred() -> Result<()> {
  ...
  // the form "in function 'foo'". ulua's `luaL_traceback` emits a different
  ...
  // phrasing). The traceback content ulua *does* produce — message prefix,
  ...
  // Inside a call chain, the traceback names the enclosing functions ulua
  ...
  let tb: String = lua
    ...
    .eval()?;
  // ulua lists the frames by name (without the "in function" phrasing).
...
fn test_multi_states() -> Result<()> {
  ...
  lua
    .load("f(function() coroutine.wrap(function() f() end)() end)")
    .exec()?;
...
fn test_exec_raw() -> Result<()> {
  ...
  let n: i32 = unsafe {
    lua.exec_raw((), |state| {
      api::lua_getglobal(state, c"sum".as_ptr());
      api::lua_pushinteger(state, 1);
      api::lua_pushinteger(state, 7);
      api::lua_call(state, 2, 1);
    })
  }?;
  assert_eq!(n, 8);
...
fn test_multi_thread_deferred() -> Result<()> {
  ...
  // `Lua: Sync`. ulua-rt is deliberately `Send` but **`!Sync`** (the VM is
...
// ---------------------------------------------------------------------------
// DEFERRED pins — each documents a capability where ulua-as-Luau genuinely
...
fn test_load_mode_deferred() -> Result<()> {
  ...
  // precompiled bytecode. ulua-rt's high-level loader auto-loads *text*
...
fn test_panic_deferred() -> Result<()> {
  ...
  //
  // ulua-rt's callback trampoline instead *catches* a Rust panic (inside
  ...
  // `test_coroutine_panic`). We therefore pin the actual ulua behavior: a
  ...
  let rust_panic_function = lua.create_function(|_, ()| -> Result<()> { panic!("rust panic") })?;
  lua
    .globals()
...
fn test_c_function_deferred() -> Result<()> {
  ...
  // `extern "C-unwind" fn(state: *mut LuaState) -> c_int` via
  // `create_c_function`. ulua is a pure-Rust VM: its `lua_CFunction` is a
  // plain `Option<unsafe fn(*mut LuaState) -> c_int>` (no C ABI boundary), so
  // the function uses the ulua-native `unsafe fn` shape instead of
  // `extern "C-unwind"`. The behavior exercised — a raw C function installing
  // a global — is identical.
  let lua = Lua::new();

  unsafe extern "C-unwind" fn c_function(state: *mut LuaState) -> c_int {
    unsafe {
      api::lua_pushboolean(state, 1);
      api::lua_setglobal(state, c"c_function".as_ptr());
    }
    0
  }
...
fn test_inspect_stack_deferred() -> Result<()> {
  ...
  // `debug.function()`. ulua-rt's `inspect_stack(level) -> Option<Debug>` has
  // no closure form and the `Debug` record exposes a smaller field set (no
  // `stack()`/`function()`). We pin the subset ulua-rt backs: resolving a
