...
//
// Still dropped (deferred / out-of-scope ulua-rt features):
...
fn test_string_from_lua() -> Result<()> {
  ...
  // From stack
  let f = lua.create_function(|_, s: ulua_rt::LuaString| Ok(s))?;
...
fn test_thread_from_lua() -> Result<()> {
  ...
  match lua.globals().get::<Thread>("print") {
    Err(err @ Error::FromLuaConversionError { .. }) => {
      // DEVIATION: ulua-rt's conversion errors use the Rust type name
...
fn test_anyuserdata_into_lua() -> Result<()> {
  // Adapted from mlua's `test_anyuserdata_into_lua`: ulua-rt wraps a typed
  ...
  use ulua_rt::{UserData, UserDataMethods};
...
fn test_anyuserdata_from_lua() -> Result<()> {
  ...
  match lua.globals().get::<AnyUserData>("print") {
    Err(err @ Error::FromLuaConversionError { .. }) => {
      // DEVIATION: ulua-rt uses the Rust type name `AnyUserData`.
      assert_eq!(
        err.to_string(),
...
/// 字符串→数值的 FromLua 回退必须走 VM 语义（`lua_tonumberx` → `luaV_tonumber_`
/// → `luaO_str2d`，cpp VM/src/lobject.cpp:86），与 `Lua::coerce_number` 同一条
/// 路径，而不是 Rust `str::parse`（那会把 `"0x10"` 报成 not a number，造成
/// crate 内两套字符串数值语义）。
#[test]
fn test_numeric_string_uses_vm_semantics() -> Result<()> {
  let lua = Lua::new();

  // 与 `Lua::coerce_number` 的对照锚点：同输入同结果。
  assert_eq!(
    lua.coerce_number(Value::String(lua.create_string("0x10")))?,
    Some(16.0)
  );

  // 十六进制：strtod 停在 'x'，luaO_str2d 用 strtoull 基 16 重解析。
  let f = lua.create_function(|_, i: i64| Ok(i))?;
  assert_eq!(f.call::<i64>("0x10")?, 16);
  assert_eq!(f.call::<i32>("0X1f")?, 31);

  // 前导/尾随空白：strtod 跳前导空白，luaO_str2d 容忍尾随空白。
  assert_eq!(f.call::<i32>("  42  ")?, 42);

  // "nan" 是 strtod 认可的 double 形态：f64 接受，整数转换拒绝
  // （无整数表示）。参数转换失败经回调边界包成 RuntimeError，但消息
  // 保留 FromLuaConversionError 的语义（不是 not a number）。
  let g = lua.create_function(|_, x: f64| Ok(x))?;
  assert!(g.call::<f64>("nan")?.is_nan());
  let neg_inf = g.call::<f64>("-inf")?;
  assert!(neg_inf.is_infinite() && neg_inf < 0.0);
  let err = f.call::<i64>("nan").unwrap_err().to_string();
  assert!(
    err.contains("number has no integer representation"),
    "got: {err}"
  );

  // f64 路径同一条 VM 语义。
  assert_eq!(g.call::<f64>("0x10")?, 16.0);
  assert_eq!(g.call::<f64>("  42  ")?, 42.0);

  // 128 位整数的字符串回退走同一条路径。
  let h = lua.create_function(|_, i: i128| Ok(i))?;
  assert_eq!(h.call::<i128>("0x10")?, 16);
  assert_eq!(h.call::<u128>(" 42 ")?, 42);

  // 非数值字符串仍然报 not a number（luaO_str2d 拒绝尾随垃圾）。
  let err = f.call::<i64>("12abc").unwrap_err().to_string();
...
fn test_char_from_lua() -> Result<()> {
  ...
  assert!(
    char::from_lua(Value::Integer(5456324), &lua)
  ...
  assert!(
    char::from_lua(Value::Table(lua.create_table()), &lua)
...
#[test]
fn test_handle_from_lua_propagates_error_value() {
  // `Value::Error` 是可在脚本层流转的一等值：`FromLua` 必须原样透传原变体，
  // 不得被 catch-all 压成 `FromLuaConversionError`（否则 `MemoryError` 这类
  // 宿主判定依据失去可判别性）。`Thread`/`AnyUserData` 与 `Table` 同源，
  // 三者都由 `conversion.rs::impl_lua_handle!` 单点生成。
  for (name, err) in [
    (
      "Thread",
      Thread::from_lua(
        Value::Error(Box::new(Error::MemoryError("oom".to_string()))),
        &Lua::new(),
      )
      .err(),
    ),
    ...
    match err {
      Some(Error::MemoryError(message)) => assert_eq!(message, "oom"),
      other => panic!("{name}: expected `Error::MemoryError`, got {other:?}"),
    }
  }
}
