...
use ulua_rt::{
  Error, Function, Lua, MetaMethod, Result, UserData, UserDataFields, UserDataMethods, Value,
  Variadic,
};
...
fn test_custom_index_metamethod_scoped() -> Result<()> {
  ...
  lua.globals().set("scoped", Value::Nil)?;

  Ok(())
}

// Writes mirror the `__index` chain: with fields registered, the generated
// `__newindex` must fall through field setters -> the custom `__newindex`
// meta-method (mlua's order: setters take priority, the metamethod stays
// reachable, and neither means "unknown field").
#[test]
fn test_custom_newindex_metamethod_with_fields() -> Result<()> {
  struct Record {
    id: i64,
    extra: BTreeMap<String, String>,
  }
  ...
  impl UserData for Record {
    fn add_fields<F: ulua_rt::UserDataFields<Self>>(fields: &mut F) {
      fields.add_field_method_get("id", |_, this| Ok(this.id));
      fields.add_field_method_set("id", |_, this, v: i64| {
        this.id = v;
        Ok(())
      });
    }
    ...
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
      methods.add_meta_method_mut(
        MetaMethod::NewIndex,
        |_, this, (key, val): (String, String)| {
          this.extra.insert(key, val);
          Ok(())
        },
      );
      methods.add_meta_method(MetaMethod::Index, |_, this, key: String| {
  ...
  // A registered field setter wins over the custom __newindex.
  lua.load("rec.id = 42").exec()?;
  assert_eq!(lua.load("return rec.id").eval::<i64>()?, 42);
  // Not a field: the custom __newindex stores it, the custom __index reads it.
  lua.load(r#"rec["note"] = "hello""#).exec()?;
  assert_eq!(lua.load(r#"return rec["note"]"#).eval::<String>()?, "hello");
  // The stored value is visible Rust-side through the same map.
  let ud: ulua_rt::AnyUserData = lua.globals().get("rec")?;
  let r = ud.borrow::<Record>()?;
  assert_eq!(r.extra.get("note").map(String::as_str), Some("hello"));
  assert_eq!(r.id, 42);
