...
//
// Dropped (deferred ulua-rt features): test_string_wrap (LuaString::wrap, a
...
use ulua_rt::{Lua, LuaString, Result};
...
fn test_string_hash() -> Result<()> {
  ...
  let s1 = lua.create_string("hello");
  let s2 = lua.create_string("hello");
  let s3 = lua.create_string("world");
