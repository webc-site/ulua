...
unsafe extern "C-unwind" fn trampoline(state: *mut lua_State) -> c_int {
  unsafe {
    ...
    if ud.is_null() {
      ...
      return raise_lua_error(state, "ulua-rt: missing callback upvalue");
    }
...
/// Shared with `async.rs` (its `raise` calls this).
pub(crate) unsafe fn raise_lua_error(state: *mut lua_State, msg: &str) -> c_int {
  unsafe {
...
/// `interrupt.rs` / `async.rs` (their user-callback panic guards).
