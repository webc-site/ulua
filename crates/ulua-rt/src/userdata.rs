...
use crate::{
  callback::{BoxedCallback, create_callback_function, wrap_callback},
  error::{Error, Result},
  function::Function,
  multi::MultiValue,
  state::{Lua, LuaRef},
...
impl AnyUserData {
  ...
  fn cell<T: 'static>(&self) -> Result<&DataCell<T>> {
    let ptr = unsafe { userdata_ptr(&self.reference) };
    if ptr.is_null() {
      return Err(Error::UserDataTypeMismatch);
    }
    // The wrapper stores the CellKey first; check it before downcasting.
    let header = unsafe { &*(ptr as *const DataHeader) };
    if header.key != CellKey::Typed(TypeId::of::<T>()) {
      return Err(Error::UserDataTypeMismatch);
    }
    Ok(unsafe { &*(ptr as *const DataCell<T>) })
  }
  ...
  pub fn type_id(&self) -> Option<TypeId> {
    ...
    match unsafe { &*(ptr as *const DataHeader) }.key {
...
/// Push a registry-ref'd value and return its raw userdata pointer (then pop).
/// Shared by `AnyUserData::cell` / `type_id`, `recover_cell`, and the scoped
/// userdata neutraliser. The pointer stays valid because the registry ref
/// keeps the userdata alive.
///
/// # Safety
/// `reference` 必须属于有效且存活的 `lua_State`（由持有它的 `XRc<LuaInner>`
/// 保证）；指针指向的内存只有在通过 `CellKey` 检查后才可按 `DataCell<T>` 解释。
unsafe fn userdata_ptr(reference: &LuaRef) -> *mut c_void {
...
fn recover_cell<'a, T>(_lua: &Lua, value: &Value, key: CellKey) -> Result<&'a DataCell<T>> {
  let Value::UserData(ud) = value else {
    return Err(Error::UserDataTypeMismatch);
  };
  let ptr = unsafe { userdata_ptr(&ud.reference) };
  if ptr.is_null() {
    return Err(Error::UserDataTypeMismatch);
  }
  let header = unsafe { &*(ptr as *const DataHeader) };
...
fn cell_callback<T>(
  key: CellKey,
  invoke: impl Fn(&Lua, &DataCell<T>, MultiValue) -> Result<MultiValue> + MaybeSend + 'static,
) -> BoxedCallback {
...
impl<T> Collector<T> {
  ...
  where
    ...
    R: IntoLuaMulti,
  {
    let callback = cell_callback(self.key, move |lua, cell, args| {
  ...
  /// 注册一个字段 getter/setter（字段系 add_* 共用样板）。
  fn push_field(&mut self, name: impl Into<String>, is_get: bool, callback: BoxedCallback) {
...
impl<T> UserDataMethods<T> for Collector<T> {
    ...
    R: IntoLuaMulti,
  {
    self.methods.push(Registered {
      name: name.into(),
      is_meta: false,
      callback: wrap_callback(function),
    });
  }
...
impl<T> UserDataFields<T> for Collector<T> {
    ...
    V: IntoLua + Clone + MaybeSend + 'static,
  {
    ...
    self.push_field(name, true, callback);
  }
  ...
  where
    ...
    R: IntoLua,
  {
    let callback = cell_callback(self.key, move |lua, cell, _args| {
...
///
/// # Safety
/// `state` 必须是有效且存活的 `lua_State`。
unsafe fn alloc_cell<T>(
  lua: &Lua,
  state: *mut lua_State,
  key: CellKey,
  data: T,
...
pub(crate) fn create_scoped_userdata<T: UserData>(
  ...
  let neutralise: Box<dyn FnOnce()> = Box::new(move || {
    let ptr = unsafe { userdata_ptr(&ud_for_dtor.reference) };
    if ptr.is_null() {
      return;
    }
    let cell = unsafe { &*(ptr as *const DataCell<T>) };
