...
impl<T> Collector<T> {
  ...
  /// 注册一个方法/meta-method（`add_method` / `add_meta_method` 共用样板）。
    ...
    R: IntoLuaMulti,
  {
    ...
    let callback: BoxedCallback = Box::new(move |lua, mut args| {
      ...
      let r = with_borrowed(cell, |data| method(lua, data, a))?;
    ...
    R: IntoLuaMulti,
  {
    ...
    let callback: BoxedCallback = Box::new(move |lua, mut args| {
      ...
      let cell = recover_cell::<T>(lua, &this, key)?;
      let a = A::from_lua_multi(args, lua)?;
      let r = with_borrowed_mut(cell, |data| method(lua, data, a))?;
      r.into_lua_multi(lua)
    });
    self.methods.push(Registered {
      name: name.into(),
...
impl<T> UserDataFields<T> for Collector<T> {
    ...
    R: IntoLua,
  {
    ...
    let callback: BoxedCallback = Box::new(move |lua, mut args| {
      let this = args.pop_front().unwrap_or(Value::Nil);
      let cell = recover_cell::<T>(lua, &this, key)?;
      let r = with_borrowed(cell, |data| method(lua, data))?;
      r.into_lua_multi(lua)
    });
    self.push_field(name, true, callback);
  }
