...
impl Lua {
    ...
    F: FnOnce(*mut lua_State),
  {
    ...
    let boxed: RawFn = {
      ...
      // - 正常路径 / `f` 内出错路径：box 已被 trampoline `take()` 并消费，
      //   drop 发生在 `lua_pcall` 内、本函数返回前。
      // - `lua_pcall` 在调用 trampoline 前失败（`ensure_stack` 已保证栈空间，
      //   仅内存不足可达）：`luaD_pcall` 恢复栈顶、函数槽位被 errorobj 覆盖，
      //   upvalue userdata 引用释放；GC sweep 释放该 userdata 的时机可能晚于
      //   本函数返回，但 dtor 只是 drop box——闭包捕获的均是对 `'scope` 帧
      //   数据的引用，引用的 drop glue 是 no-op，不会实际触碰已死数据。
  ...
  /// `unsafe fn` (see [`api::lua_CFunction`](crate::api::lua_CFunction)).
  ///
  /// # Safety
