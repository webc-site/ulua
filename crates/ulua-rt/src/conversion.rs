  ...
  ($($ty:ty),*) => {$(
    ...
    impl FromLua for $ty {
      fn from_lua(value: Value, _lua: &Lua) -> Result<Self> {
        ...
        fn from_float(f: f64, to: &'static str) -> Result<$ty> {
          ...
          let wide = t as i128;
          <$ty>::try_from(wide).map_err(|_| conv_err("number", to, "out of range"))
        }
        match value {
          Value::Integer(i) => {
            <$ty>::try_from(i).map_err(|_| conv_err("number", stringify!($ty), "out of range"))
          }
          Value::Number(f) => {
          ...
          Value::String(ref s) => {
            ...
            if let Ok(i) = trimmed.parse::<i64>() {
              <$ty>::try_from(i).map_err(|_| conv_err("string", stringify!($ty), "out of range"))
              ...
              Err(conv_err("string", stringify!($ty), "not a number"))
            }
          }
      ...
      fn from_lua(value: Value, _lua: &Lua) -> Result<Self> {
        let n: f64 = match value {
          ...
          Value::String(ref s) => s
            ...
            .trim()
            .parse::<f64>()
            .map_err(|_| conv_err("string", stringify!($ty), "not a number"))?,
          other => return Err(Error::FromLuaConversionError {
            from: other.type_name(),
            to: stringify!($ty).to_string(),
            message: None,
          }),
...
impl<T: IntoLua> IntoLua for Vec<T> {
  fn into_lua(self, lua: &Lua) -> Result<Value> {
    ...
    for (i, item) in self.into_iter().enumerate() {
      // Lua sequences are 1-based; fresh table 无元表，raw 写入足够。
...
impl<T: FromLua> FromLua for Vec<T> {
  fn from_lua(value: Value, _lua: &Lua) -> Result<Self> {
    match value {
      Value::Table(t) => {
        let len = t.raw_len();
        (1..=len).map(|i| t.raw_get::<T>(i as i64)).collect()
      }
      other => Err(Error::FromLuaConversionError {
        from: other.type_name(),
        to: "Vec".to_string(),
...
impl FromLua for CString {
  fn from_lua(value: Value, _lua: &Lua) -> Result<Self> {
    let nul_err = |e: NulError| Error::FromLuaConversionError {
      from: "string",
      to: "CString".to_string(),
      message: Some(format!("interior nul byte: {e}")),
    };
    match value {
      Value::String(s) => CString::new(s.as_bytes()).map_err(nul_err),
      other => Err(Error::FromLuaConversionError {
        from: other.type_name(),
        to: "CString".to_string(),
        message: None,
