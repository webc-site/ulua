...
use crate::{
  callback::{
    PANIC_MSG_PREFIX, collect_stack_args, panic_message, raise_lua_error, raise_structured_error,
  },
  error::{Error, Result},
  function::Function,
  multi::MultiValue,
...
// ulua-rt's `Lua` has no such block, so we keep them in a process-wide table
...
/// stable for the VM's lifetime, `Send`). Thin integer view over
/// [`vm_global_key`].
#[inline]
unsafe fn vm_key(state: *mut lua_State) -> usize {
  unsafe { vm_global_key(state) as usize }
}
...
fn noop_waker() -> Waker {
  ...
  const VTABLE: RawWakerVTable = RawWakerVTable::new(
    |_| RawWaker::new(null(), &VTABLE), // clone
    |_| {},                             // wake
    |_| {},                             // wake_by_ref
    |_| {},                             // drop
  );
  // SAFETY: the vtable never dereferences the (null) data pointer.
  unsafe { Waker::from_raw(RawWaker::new(null(), &VTABLE)) }
}
...
unsafe extern "C-unwind" fn poll_upvalue_dtor(ptr: *mut c_void) {
  if !ptr.is_null() {
    unsafe { drop_in_place(ptr as *mut AsyncPollUpvalue) };
  }
}
...
unsafe extern "C-unwind" fn callback_upvalue_dtor(ptr: *mut c_void) {
  if !ptr.is_null() {
    unsafe { drop_in_place(ptr as *mut AsyncCallbackUpvalue) };
  }
}
...
unsafe extern "C-unwind" fn get_future_c(state: *mut lua_State) -> c_int {
  unsafe {
    // Recover the callback from upvalue 1.
    let ud = lua_touserdata(state, lua_upvalueindex(1));
    if ud.is_null() {
      raise_lua_error(state, "ulua-rt: missing async callback upvalue");
    }
    ...
    let args = match collect_stack_args(&lua, lua_gettop(state)) {
      Ok(a) => a,
      Err(e) => raise_lua_error(state, &e.to_string()),
    };

    // Build the future (this runs only the synchronous prologue of the user
    // async fn up to the first await point). A panic in that prologue must not
    // unwind through the VM frames — convert it to a catchable Lua error.
    let fut = match catch_unwind(AssertUnwindSafe(|| (upvalue.callback)(lua.clone(), args))) {
      Ok(fut) => fut,
      Err(payload) => {
    ...
    let storage = lua_newuserdatadtor(
      state,
      size_of::<AsyncPollUpvalue>(),
      Some(poll_upvalue_dtor),
    );
    if storage.is_null() {
      raise_lua_error(state, "ulua-rt: failed to allocate async future userdata");
    }
    write(
...
unsafe extern "C-unwind" fn poll_c(state: *mut lua_State) -> c_int {
  unsafe {
    // The future userdata is always argument 1.
    let ud = lua_touserdata(state, 1);
    if ud.is_null() {
      raise_lua_error(state, "ulua-rt: missing async future argument");
    }
    ...
    let poll = match future.data.as_mut() {
      Some(f) => match catch_unwind(AssertUnwindSafe(|| f.as_mut().poll(&mut cx))) {
        Ok(poll) => poll,
        Err(payload) => {
          future.data.take();
          let msg = panic_message(&payload);
...
unsafe extern "C-unwind" fn unpack_c(state: *mut lua_State) -> c_int {
  unsafe {
    let mut isnum: c_int = 0;
    let n = lua_tointegerx(state, 2, &mut isnum);
    if lua_checkstack(state, n.saturating_add(1)) == 0 {
      raise_lua_error(state, "ulua-rt: stack overflow unpacking async results");
    }
    (1..=n).for_each(|i| {
...
// ---------------------------------------------------------------------------
// Small raise helpers (reusing `callback::raise_lua_error`, kept thin locally)
// ---------------------------------------------------------------------------

/// Raise the structured `CallbackDestructed` error (future polled after drop).
unsafe fn raise_destructed(state: *mut lua_State) -> ! {
...
pub(crate) fn create_async_callback(lua: &Lua, callback: AsyncCallback) -> Result<Function> {
  ...
  let get_future = unsafe {
    ...
    push_c_closure_with_upvalue(state, get_future_c, c"ulua-rt-get-future");
    Function::from_ref(lua.pop_ref())
  };

  // 2. Build the `poll` and `unpack` C closures (no upvalues).
  let poll = unsafe {
    lua_pushcclosurek(state, Some(poll_c), c"ulua-rt-poll".as_ptr(), 0, None);
    Function::from_ref(lua.pop_ref())
  };
  let unpack = unsafe {
    lua_pushcclosurek(state, Some(unpack_c), c"ulua-rt-unpack".as_ptr(), 0, None);
    Function::from_ref(lua.pop_ref())
  };
        ...
        match args.take() {
                  ...
                  Some(v) => {
                    if lua.push_value(v).is_err() {
                      return Poll::Ready(Err(Error::runtime(
                        "ulua-rt: failed to push yield value",
                      )));
                    }
                  }
          ...
          None => {
            ...
            let result = unsafe {
              // We are running inside `poll(future, <resume values>)`, so the
              // coroutine stack is `[future, resume1, resume2, ...]`; the
              // resume values sit above index 1 (the future). Collect them
              // with `base = 1`, which truncates back to the future.
              let _ = lua_checkstack(state, 2);
              lua
                .collect_results_above(state, 1)
...
macro_rules! impl_lua_native_async_fn {
    ($($T:ident: $v:ident),*) => {
            ...
            Fut: Future<Output = R> + MaybeSend + 'static,
        {
            ...
            fn call(&self, args: ($($T,)*)) -> BoxedAsyncFnFuture<R> {
                let ($($v,)*) = args;
                Box::pin(self($($v,)*))
            }
        }
