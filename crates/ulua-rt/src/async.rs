...
use crate::{
  callback::{collect_stack_args, panic_message, raise_lua_error, raise_structured_error},
  error::{Error, Result},
  function::Function,
  multi::MultiValue,
...
unsafe extern "C-unwind" fn get_future_c(state: *mut lua_State) -> c_int {
  unsafe {
    ...
    if ud.is_null() {
      return raise(state, "ulua-rt: missing async callback upvalue");
    }
    ...
    let fut = match catch_unwind(AssertUnwindSafe(|| (upvalue.callback)(lua.clone(), args))) {
      ...
      Err(payload) => {
        ...
        return raise(state, &format!("rust panic: {msg}"));
      }
    };
    ...
    if storage.is_null() {
      return raise(state, "ulua-rt: failed to allocate async future userdata");
    }
    write(
...
unsafe extern "C-unwind" fn poll_c(state: *mut lua_State) -> c_int {
  unsafe {
    ...
    if ud.is_null() {
      return raise(state, "ulua-rt: missing async future argument");
    }
...
unsafe extern "C-unwind" fn unpack_c(state: *mut lua_State) -> c_int {
  unsafe {
    ...
    if lua_checkstack(state, n.saturating_add(1)) == 0 {
      return raise(state, "ulua-rt: stack overflow unpacking async results");
    }
    (1..=n).for_each(|i| {
