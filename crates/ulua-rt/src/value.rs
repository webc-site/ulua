...
impl Value {
  ...
  pub fn to_pointer(&self) -> *const c_void {
    match self {
      Value::LightUserData(lud) => lud.0 as *const c_void,
...
pub(crate) fn push_value(lua: &Lua, value: &Value) -> Result<()> {
  ...
  unsafe {
    match value {
      ...
      Value::Vector(v) => lua_pushvector_lua_state_f32_f32_f32_f32(state, v.x(), v.y(), v.z(), 0.0),
...
pub(crate) fn value_from_stack(lua: &Lua, idx: c_int) -> Result<Value> {
  ...
  unsafe {
    let value = match lua_type(state, idx) {
      ...
      ttype::USERDATA => {
        lua_pushvalue(state, idx);
        Value::UserData(AnyUserData::from_ref(lua.pop_ref()))
      }
      ttype::THREAD => {
        lua_pushvalue(state, idx);
        Value::Thread(Thread::from_ref(lua.pop_ref()))
      }
      ttype::VECTOR => {
        ...
        if p.is_null() {
          ...
          let comps = from_raw_parts(p, Vector::SIZE);
          Value::Vector(Vector::new(comps[0], comps[1], comps[2]))
        }
      }
      ttype::BUFFER => {
        lua_pushvalue(state, idx);
        Value::Buffer(Buffer::from_ref(lua.pop_ref()))
      }
