...
#[cfg(feature = "async")]
use crate::async_support::{implicit_thread_owner, poll_pending, poll_terminate};
#[cfg(feature = "async")]
use crate::sys::{lua_pushlightuserdatatagged, lua_tolightuserdata};
use crate::{
  error::{Error, Result},
  function::Function,
  multi::MultiValue,
...
impl Thread {
  ...
  pub fn resume<R: FromLuaMulti>(&self, args: impl IntoLuaMulti) -> Result<R> {
    ...
    unsafe {
      ...
      ensure_stack(co, nargs.saturating_add(2))?;
  ...
  unsafe fn finish_resume<R: FromLuaMulti>(&self, lua: &Lua, status: c_int) -> Result<R> {
    ...
    unsafe {
      if status != status::OK && status != status::YIELD && status != status::BREAK {
        ...
        // `base` — `pop_error` pops only the error object, so without this
        // the leftovers would leak onto the parent stack.
      ...
      let nres = lua_gettop(co);
      ensure_stack(parent, nres.saturating_add(1))?;
      let base = lua_gettop(parent);
      if nres > 0 {
        lua_xmove(co, parent, nres);
      }
      let results = lua.collect_results_above(parent, base)?;
  ...
  pub(crate) fn resume_for_async(&self, args: MultiValue) -> Result<AsyncResume> {
    ...
    unsafe {
      ...
      if yielded && nres == 1 && lua_tolightuserdata(co, -1) == poll_pending() {
        lua_settop(co, 0);
  ...
  pub(crate) fn terminate_async(&self) {
    ...
    unsafe {
      ...
      lua_pushlightuserdatatagged(parent, poll_terminate(), 0);
      lua_xmove(parent, co, 1);
