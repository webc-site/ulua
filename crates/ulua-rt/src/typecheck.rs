...
use core::{fmt, ptr, result::Result};
...
impl CheckModuleFileResolver {
  ...
  fn resolve_module(&self, context: Option<&ModuleInfo>, expr: &AstExpr) -> Option<ModuleInfo> {
    let node = ptr::from_ref(expr).cast_mut().cast::<AstNode>();

    // 共用前置（下方每个 `ast_node_try_as_ptr` 闸门都引它）：`node` 由 `&expr` 引用
    // 反推，非空、对齐且指向活 `AstExpr`（其 `repr(C)` 首字段链到 `AstNode`，基址重合
    // 使上转合法）；`ast_node_try_as_ptr` 按 RTTI class index 分派——未命中返回 `None`，
    // 命中即类型正确，给出的 `&T` 派生安全；读出的字段（如
    // `name.value`）只经 `ast_name_to_string` / `constant_string_to_string` 的
    // 判空/切片通道使用，不越界。
    // Safety: 共用前置成立——字符串常量字面量闸门。
    if let Some(string) = unsafe { ast_node_try_as_ptr::<AstExprConstantString>(node) } {
      return self.module_info_if_known(constant_string_to_string(string), false);
    }

    // Safety: 共用前置成立——全局名表达式闸门。
    ...
    // Safety: 共用前置成立——索引名表达式（`a.b`）闸门。
    if let Some(index) = unsafe { ast_node_try_as_ptr::<AstExprIndexName>(node) } {
      let context = context?;
    ...
    // Safety: 共用前置成立——索引表达式（`a[...]`）闸门。
    if let Some(index) = unsafe { ast_node_try_as_ptr::<AstExprIndexExpr>(node) } {
      let context = context?;
      // `index.index` 是 `*mut AstExpr`，`.cast::<AstNode>()` 只是裸指针类型转换。
      let index_expr = index.index.cast::<AstNode>();

      // Safety: 共用前置成立——被索引的子表达式再过一次常量串闸门。
      if let Some(index_string) =
        unsafe { ast_node_try_as_ptr::<AstExprConstantString>(index_expr) }
      {
    ...
    // Safety: 共用前置成立——调用表达式（`f(...)`）闸门。
    if let Some(call) = unsafe { ast_node_try_as_ptr::<AstExprCall>(node) } {
      let context = context?;

      if call.self_ && !call.args.is_empty() && context.name == "game" {
        let arg = call.args[0];
        let arg_node = arg.cast::<AstNode>();
        let func_node = call.func.cast::<AstNode>();

        // Safety: 共用前置成立——首实参与被调表达式各过一次 RTTI 闸门。
        let (index_string, func) = unsafe {
          (
            ast_node_try_as_ptr::<AstExprConstantString>(arg_node),
            ast_node_try_as_ptr::<AstExprIndexName>(func_node),
          )
        };
...
fn ast_name_to_string(name: *const u8) -> String {
  if name.is_null() {
    ...
    // 由 arena 持有且比 AST 长寿，from_ptr 读到 NUL 为止不越界。批 2 后 AstName::value
    // 为 `*const u8`，cstr 门面按 cpp `const char*` 收参，cast 仅符号标记、字节域恒等。
