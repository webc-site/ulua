...
/// 构造 + 自指针布线一步完成（`Frontend` 构造 + [`Frontend::wire_self_pointers`]，
/// `run_check` / `run_check_modules` / [`Checker::new`] 三处调用点共用）。
///
/// frontend 入 `Box`（地址恒定）后立即布线；之后 `Box` 只被按指针移动
/// （pointee 留在堆上），自指针在 frontend 整个生命周期内恒有效。
/// `file_resolver` / `config` 必须存活至 frontend 使用结束（构造函数会把它们
/// 以裸指针存入 frontend）。
///
/// # Safety
/// `file_resolver` / `config` 必须取自有效且存活的对象（构造时以裸指针保存）。
unsafe fn make_wired_frontend<R: FileResolver + 'static>(
  file_resolver: &mut R,
  config: &mut ConfigResolver,
) -> Box<Frontend> {
  let options = FrontendOptions::default();
  // 泛型入参保证 `R` 是 `'static` 具体类型（不含借用），`*mut R → *mut dyn`
  // 只做类型级强转，与借用生命周期无关；对象存活期由调用方保证。
  let mut frontend = Box::new(
    Frontend::frontend_file_resolver_config_resolver_frontend_options(
      file_resolver as *mut R as *mut dyn FileResolver,
      config,
      &options,
    ),
  );
  // SAFETY: frontend 已入 `Box`、地址恒定，布线后不再移动。
  unsafe {
...
fn run_check(source: &str, definitions: Option<&str>) -> Vec<TypeDiagnostic> {
  ...
  let mut frontend = unsafe { make_wired_frontend(&mut file_resolver, &mut config_resolver.base) };
  ...
  diagnostics.extend(check_result.errors.iter().map(check_error_diagnostic));
...
impl Checker {
  ...
  pub fn new() -> Self {
    ...
    // 入 Box（地址稳定）后由 [`make_wired_frontend`] 布线：Box 只被按指针移动，
    // pointee 留在堆上，自指针在 checker 整个生命周期内恒有效。
    let mut frontend =
      unsafe { make_wired_frontend(&mut *file_resolver, &mut config_resolver.base) };
    frontend.set_luau_solver_mode(SolverMode::Old);
