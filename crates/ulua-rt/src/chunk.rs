...
impl Chunk {
  ...
  fn compile(&self) -> Result<Vec<u8>> {
    ...
    let blob = compiler_compile(&self.source, &options, &parse_options, NoopEncoder);
  ...
  pub fn eval<R: FromLuaMulti>(mut self) -> Result<R> {
    // 表达式优先：改写为 `return <src>` 试编译一次（原地改写，不整块克隆
    // Chunk）；命中直接加载该字节码，失败恢复原源码走语句块路径。
    let source = mem::take(&mut self.source);
    self.source = format!("return {source}");
    let expression = self.compile().ok();
    self.source = source;
    let f = match expression {
  ...
  #[cfg(feature = "async")]
  #[cfg_attr(docsrs, doc(cfg(feature = "async")))]
  pub async fn eval_async<R: FromLuaMulti>(mut self) -> Result<R> {
    // 表达式优先（同 [`Chunk::eval`]：原地改写，命中复用字节码不二次编译）。
    let source = mem::take(&mut self.source);
    self.source = format!("return {source}");
    let expression = self.compile().ok();
    self.source = source;
