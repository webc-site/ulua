...
impl Chunk {
  ...
  /// The expression form of this chunk (`return ` prepended), sharing the
  /// chunk's name/environment/compiler. Shared by [`Chunk::eval`] and
  /// [`Chunk::eval_async`].
  fn as_expression(&self) -> Chunk {
    Chunk {
      lua: self.lua.clone(),
  ...
  pub fn eval<R: FromLuaMulti>(self) -> Result<R> {
    ...
    if let Ok(f) = self.as_expression().into_function() {
      return f.call::<R>(());
    }
