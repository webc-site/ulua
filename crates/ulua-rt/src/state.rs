...
use crate::{
  ...
  buffer::{self, Buffer},
  callback::{create_callback_function, recover_wrapped_error, wrap_callback},
  chunk::Chunk,
  error::{Error, Result},
  function::Function,
...
const MEMCAT_MAIN: c_int = 0;

/// 每个 VM 的身份键：`global_State` 指针（进程内唯一、VM 存活期内稳定、可跨
/// 该 VM 的所有协程状态访问）。app-data、async 状态、per-VM 编译器、沙箱标志、
/// 内存控制、中断闭包等 side-store 全部用它做键。
///
/// # Safety
/// `state` 必须是有效且存活的 `lua_State`。
pub(crate) unsafe fn vm_global_key(state: *mut lua_State) -> *mut c_void {
  unsafe { (*state).global as *mut c_void }
}

/// 把 VM 返回的 `(ptr, len)` 字节串转成 Rust `String`（lossy UTF-8）。
/// [`Lua::value_to_string`] 与 [`Lua::pop_error`] 共用。
///
/// # Safety
/// `p` 必须非 null 且指向至少 `len` 个有效字节。
pub(crate) unsafe fn lossy_bytes(p: *const c_char, len: usize) -> String {
  unsafe { String::from_utf8_lossy(from_raw_parts(p as *const u8, len)).into_owned() }
}
...
impl Drop for LuaInner {
  fn drop(&mut self) {
    if self.owned && !self.state.is_null() {
      ...
      let mem_key = unsafe { vm_global_key(self.state) };
      unsafe {
        ...
        lua_setmemcat(self.state, MEMCAT_MAIN);
...
impl Lua {
  ...
  where
    ...
    R: IntoLuaMulti,
  {
    create_callback_function(self, wrap_callback(func))
  }
