...
///
/// 与同目录 `is_directory_path` 一样用 `symlink_metadata` 取 Win32 属性位，避免符号
/// 链接被 `metadata` 跟随后与上游 lstat 语义脱节。
#[cfg(windows)]
pub(crate) fn is_file_path(path: &Path) -> bool {
...
/// `&str` 便捷入口（公开 API 保持不变），内部统一收敛到 `is_file_path`。
pub fn is_file(path: &str) -> bool {
  is_file_path(Path::new(path))
}
