// ulua playground — boots the WebAssembly engine and wires the editor + buttons.
//
// The Luau compiler, VM and type checker are compiled to wasm (crates/ulua-web,
...
async function loadEngine() {
  engineGen += 1;
  const mod = await import(`./pkg/ulua_web.js?gen=${engineGen}`);
...
async function doRun() {
  ...
  try {
    ...
    if (isTrap(e)) {
      ...
      // handed us the recovered error text via globalThis.__uluaOnRuntimeError
