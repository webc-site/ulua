...
# i18n / examples follow for good measure. index.html is deliberately excluded.
rsync -a --delete website/pkg/ "$DEPLOY_DIR/pkg/"
for f in app.js codemirror.bundle.js env.js style.css i18n.js; do
  [[ -f "website/$f" ]] && cp "website/$f" "$DEPLOY_DIR/$f"
done
rsync -a --delete website/public/locales/ "$DEPLOY_DIR/locales/"
...
git -C "$DEPLOY_DIR/.." add \
  ulua/pkg ulua/app.js ulua/codemirror.bundle.js ulua/env.js ulua/i18n.js \
  ulua/locales ulua/style.css ulua/examples 2>/dev/null || true
