...
# 不用 mapfile：macOS 自带 bash 3.2 没有它；while-read 逐行收集。
files=()
while IFS= read -r f; do files+=("$f"); done < <(find "$CORPUS" -name '*.luau' -type f)
# bash 3.2 在 set -u 下引用空数组会报 unbound，用 ${files[@]+…} 兜底。
n_files=${#files[@]}
echo ">> scanning $n_files scripts (steps=$STEPS, timeout=${TIMEOUT}s/file)"
...
scan() { # $1 = target binary name
  ...
  for f in ${files[@]+"${files[@]}"}; do
    ...
    n=$((n+1))
    (( n % 500 == 0 )) && echo "   $1: $n/$n_files ($bad findings)"
  done
