...
#   ULUA_FUZZ_ASAN=1 make fuzz-typeck   # AFL + AddressSanitizer build
...
#
#   Throughput knob — ULUA_FUZZ_STEPS caps the VM interrupt budget for the
...
#     ULUA_FUZZ_STEPS=20000  make fuzz-loop   # breadth: ~6x more execs/sec
#     ULUA_FUZZ_STEPS=1000000 make fuzz-run   # depth: exercise long finite loops
#
...
# Set ULUA_FUZZ_ASAN=1 for an AddressSanitizer build. By default a target fuzzes
...
# targets with --no-default-features and runs each over ULUA_FUZZ_ITERS
...
# 产物目录不硬写 ./target：build.target-dir 可在 ~/.cargo/config.toml 里全局重定向
# （本机即指向仓库外），届时下面 17 条 standalone 规则全部 127。问 cargo 要真实目录
# ——默认配置下它返回 fuzz/target，重定向时返回重定向后的位置。
FUZZ_BIN := $(shell cd fuzz && cargo metadata --format-version 1 --no-deps 2>/dev/null | sed -n 's/.*"target_directory":"\([^"]*\)".*/\1/p')/debug
FUZZ_BIN := $(if $(FUZZ_BIN),$(FUZZ_BIN),$(CURDIR)/fuzz/target/debug)

fuzz-standalone-compile:
	cd fuzz && cargo build --no-default-features --bin compile
	cd fuzz && ULUA_FUZZ_ITERS=$(ITERS) ULUA_FUZZ_SEED=$(SEED) $(FUZZ_BIN)/compile
fuzz-standalone-run:
	cd fuzz && cargo build --no-default-features --bin run
	cd fuzz && ULUA_FUZZ_ITERS=$(ITERS) ULUA_FUZZ_SEED=$(SEED) $(FUZZ_BIN)/run
fuzz-standalone-typeck:
	cd fuzz && cargo build --no-default-features --bin typeck
	cd fuzz && ULUA_FUZZ_ITERS=$(ITERS) ULUA_FUZZ_SEED=$(SEED) $(FUZZ_BIN)/typeck
fuzz-standalone-typeck_typed:
	cd fuzz && cargo build --no-default-features --bin typeck_typed
	cd fuzz && ULUA_FUZZ_ITERS=$(ITERS) ULUA_FUZZ_SEED=$(SEED) $(FUZZ_BIN)/typeck_typed
fuzz-standalone-number:
	cd fuzz && cargo build --no-default-features --bin number
	cd fuzz && ULUA_FUZZ_ITERS=$(ITERS) ULUA_FUZZ_SEED=$(SEED) $(FUZZ_BIN)/number
fuzz-standalone-structured:
	cd fuzz && cargo build --no-default-features --bin structured
	cd fuzz && ULUA_FUZZ_ITERS=$(ITERS) ULUA_FUZZ_SEED=$(SEED) $(FUZZ_BIN)/structured
fuzz-standalone-typeck_defs:
	cd fuzz && cargo build --no-default-features --bin typeck_defs
	cd fuzz && ULUA_FUZZ_ITERS=$(ITERS) ULUA_FUZZ_SEED=$(SEED) $(FUZZ_BIN)/typeck_defs
fuzz-standalone-determinism:
	cd fuzz && cargo build --no-default-features --bin determinism
	cd fuzz && ULUA_FUZZ_ITERS=$(ITERS) ULUA_FUZZ_SEED=$(SEED) $(FUZZ_BIN)/determinism
fuzz-standalone-roundtrip:
	cd fuzz && cargo build --no-default-features --bin roundtrip
	cd fuzz && ULUA_FUZZ_ITERS=$(ITERS) ULUA_FUZZ_SEED=$(SEED) $(FUZZ_BIN)/roundtrip
fuzz-standalone-splice:
	cd fuzz && cargo build --no-default-features --bin splice
	cd fuzz && ULUA_FUZZ_ITERS=$(ITERS) ULUA_FUZZ_SEED=$(SEED) $(FUZZ_BIN)/splice
fuzz-standalone-optdiff:
	cd fuzz && cargo build --no-default-features --bin optdiff
	cd fuzz && ULUA_FUZZ_ITERS=$(ITERS) ULUA_FUZZ_SEED=$(SEED) $(FUZZ_BIN)/optdiff
fuzz-standalone-metamorphic:
	cd fuzz && cargo build --no-default-features --bin metamorphic
	cd fuzz && ULUA_FUZZ_ITERS=$(ITERS) ULUA_FUZZ_SEED=$(SEED) $(FUZZ_BIN)/metamorphic
fuzz-standalone-spans:
	cd fuzz && cargo build --no-default-features --bin spans
	cd fuzz && ULUA_FUZZ_ITERS=$(ITERS) ULUA_FUZZ_SEED=$(SEED) $(FUZZ_BIN)/spans
fuzz-standalone-api:
	cd fuzz && cargo build --no-default-features --bin api
	cd fuzz && ULUA_FUZZ_ITERS=$(ITERS) ULUA_FUZZ_SEED=$(SEED) $(FUZZ_BIN)/api
fuzz-standalone-gcstress:
	cd fuzz && cargo build --no-default-features --bin gcstress
	cd fuzz && ULUA_FUZZ_ITERS=$(ITERS) ULUA_FUZZ_SEED=$(SEED) $(FUZZ_BIN)/gcstress
fuzz-standalone-host:
	cd fuzz && cargo build --no-default-features --bin host
	cd fuzz && ULUA_FUZZ_ITERS=$(ITERS) ULUA_FUZZ_SEED=$(SEED) $(FUZZ_BIN)/host
fuzz-standalone-serde_roundtrip:
	cd fuzz && cargo build --no-default-features --bin serde_roundtrip
	cd fuzz && ULUA_FUZZ_ITERS=$(ITERS) ULUA_FUZZ_SEED=$(SEED) $(FUZZ_BIN)/serde_roundtrip
